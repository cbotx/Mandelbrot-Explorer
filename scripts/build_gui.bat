...
if not exist "%VCPKG%\include\mpfr.h" (
  echo Missing static MPFR headers in %VCPKG%
  goto :err
)
if not exist "%VCPKG%\lib\mpfr.lib" (
  echo Missing static MPFR library in %VCPKG%
  goto :err
)

rc /nologo /fo build\mandel_gui.res src\gui\mandel_gui.rc || goto :err

REM Static GMP/MPFR (x64-windows-static, /MT) -> no gmp/mpfr DLLs.
cl /nologo /O2 /EHsc /openmp /std:c++17 /arch:AVX2 /MT /DMANDEL_ENABLE_ASMJIT ^
   ...
   build\mandel_gui.res ^
   /Fe:build\mandel_gui.exe ^
   /link /MANIFEST:NO /SUBSYSTEM:WINDOWS /LIBPATH:"%VCPKG%\lib" ^
