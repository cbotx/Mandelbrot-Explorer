...
cl /nologo /O2 /EHsc /openmp /std:c++17 /arch:AVX2 /MT /DMANDEL_ENABLE_ASMJIT ^
   ...
   src\engine\compute_backend.cpp src\engine\mandel_perturbation.cpp src\engine\mandel_expression.cpp src\engine\formula_expression.cpp src\engine\formula_expression_jit.cpp src\engine\float_math.cpp src\engine\interpolate.cpp src\engine\color.cpp ^
   /Fe:build\mandel_gui.exe ^
   /link /SUBSYSTEM:WINDOWS /LIBPATH:"%VCPKG%\lib" ^
   asmjit.lib gmp.lib user32.lib gdi32.lib comctl32.lib comdlg32.lib windowscodecs.lib ole32.lib || goto :err
