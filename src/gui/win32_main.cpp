...
struct Preset {
    ...
    float density;
    bool ss;
    int coloring; // 0 Smooth, 1 Distance(EDE), 2 Feather
    const wchar_t* palette;
    int maxIter;
};
static const std::vector<Preset>& galleryPresets() {
    static const std::vector<Preset> p = {
        {L"Night City  (1.7e40)", "-1.768628917759850520844734198472848718821423994141176532908", "0.001395534274228826510747662517373603005419245032078944176", "1.691960e40", 83.4f, true, 0, L"Sunrise", 500000},
        {L"Golden Phoenix  (5.1e292)", "-1.", "0.", "5.119695e292", 169.0f, true, 2, L"Sunrise", 500000},
    };
    return p;
}
...
struct PaletteEditor {
    ...
    void rebuild() {
        ...
        for (int i = 0; i < n; ++i) rgb2lab(stops[i].r, stops[i].g, stops[i].b, lab[i][0], lab[i][1], lab[i][2]);
        ...
        for (int p = -1; p <= 1; ++p)
            for (int i = 0; i < n; ++i) xs[(p + 1) * n + i] = stops[i].pos + p;
        for (int c = 0; c < 3; ++c) {
...
static std::wstring truncLocation(const std::string& raw, int cpl) {
    ...
    auto flush = [&](bool last) {
        std::string L = acc;
        acc.clear();
        if ((L.rfind("x:", 0) == 0 || L.rfind("y:", 0) == 0) && (int)L.size() > cpl) { L = L.substr(0, std::max(0, cpl - 3)) + "..."; }
...
static std::string expandSci(const std::string& in) {
    ...
    if (shift >= 0) {
        ...
        if (point <= 0)
            out = "0." + std::string((size_t)(-point), '0') + mant;
        else
            out = mant.substr(0, (size_t)point) + "." + mant.substr((size_t)point);
    }
    return sign + out;
}
...
class App {
  public:
    ...
    bool panelSbDrag = false;
    int panelSbGrabY = 0, panelSbGrabScroll = 0;
    ...
    int fpsLogSamples = 0;
    double fpsLogSum = 0, fpsLogMin = 1e9;
    ...
    void rebuildOrbitThumbnail() {
        ...
        orbitThumbnailGeneration = orbitThumbnailWorker->request(ORBIT_W, ORBIT_H, ORBIT_X0, ORBIT_X1, ORBIT_Y0, ORBIT_Y1, 160, std::move(expression));
    ...
    POINT orbitMap(double re, double im, const RECT& r) const {
        double x = r.left + (re - ORBIT_X0) / (ORBIT_X1 - ORBIT_X0) * (r.right - r.left - 1);
        double y = r.top + (ORBIT_Y1 - im) / (ORBIT_Y1 - ORBIT_Y0) * (r.bottom - r.top - 1);
        constexpr double coordinateLimit = 10000000.0;
        return {(LONG)std::clamp(x, -coordinateLimit, coordinateLimit), (LONG)std::clamp(y, -coordinateLimit, coordinateLimit)};
    }
    ...
    void drawOrbitThumbnail(HDC dc) {
        ...
        if (nav && nav->IsExpression()) {
            ...
            FormulaParameter pixelParameter = expression ? expression->pixelParameter : formulaConfig.pixelParameter;
            thumbnailLabel = pixelParameter == FormulaParameter::InitialZ ? L"Custom z0 initial-value plane" : L"Custom c parameter plane";
        }
        label(dc, rcOrbitThumb.left, rcOrbitThumb.top - S(20), thumbnailLabel);
        fillRound(dc, rcOrbitThumb, CLR_CARD, CLR_BORDER, S(8));
        RECT image = rcOrbitThumb;
        InflateRect(&image, -S(6), -S(6));
        const bool hasBackground = orbitThumbnail.size() == (size_t)ORBIT_W * ORBIT_H * 3;
        if (hasBackground) {
            BITMAPINFO bi{};
            bi.bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
            bi.bmiHeader.biWidth = ORBIT_W;
            bi.bmiHeader.biHeight = -ORBIT_H;
            bi.bmiHeader.biPlanes = 1;
            bi.bmiHeader.biBitCount = 24;
            bi.bmiHeader.biCompression = BI_RGB;
            SetStretchBltMode(dc, HALFTONE);
            StretchDIBits(dc, image.left, image.top, image.right - image.left, image.bottom - image.top, 0, 0, ORBIT_W, ORBIT_H, orbitThumbnail.data(), &bi, DIB_RGB_COLORS, SRCCOPY);
        } else {
        ...
        if (hasBackground && orbitResult.points.size() >= 2) {
            ...
            for (const OrbitPoint& p : orbitResult.points) {
                if (!std::isfinite(p.re) || !std::isfinite(p.im)) break;
            ...
            if (points.size() >= 2) Polyline(dc, points.data(), (int)points.size());
            SelectObject(dc, old);
            DeleteObject(line);
        }
        ...
        if (!hasBackground) {
            drawText(dc, image, orbitThumbnailBuilding ? L"Building orbit background..." : L"Background unavailable", CLR_TEXT, fSmall, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
        } else if (orbitResult.generation == 0) {
            drawText(dc, image, L"Move over the main image", CLR_TEXT, fSmall, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
        } else {
            wchar_t text[96];
            swprintf_s(text, L"%d iter%s   %.1f ms", orbitResult.iterations, orbitResult.escaped ? L"  escaped" : L"", orbitResult.computeMs);
            RECT info = image;
            info.left += S(6);
            info.bottom -= S(4);
            drawText(dc, info, text, CLR_TEXT, fSmall, DT_LEFT | DT_BOTTOM | DT_SINGLELINE);
        }
    }
    ...
    void requestOrbit(int x, int y, bool applyBoundary = true) {
        ...
        if (lastOrbitRequest.time_since_epoch().count()) {
            double ms = std::chrono::duration<double, std::milli>(now - lastOrbitRequest).count();
            if (ms < 12.0 && std::abs(x - lastOrbitX) < 2 && std::abs(y - lastOrbitY) < 2) return;
        }
        POINT p = mapToRender(x, y);
        if (applyBoundary) {
            auto boundary = nav->GetBoundary();
            double fx = boundary[0][0] + (boundary[2][0] - boundary[0][0]) * (p.x + 0.5) / renderW;
            double fy = boundary[0][1] + (boundary[2][1] - boundary[0][1]) * ((renderH - 1 - p.y) + 0.5) / renderH;
        ...
        orbitWorker->request(re, im, scale, p.x, p.y, renderW, renderH, maxIter, nav->GetFormulaContext(), std::move(expression), nav->GetCustomDeepZoomPlan());
        mpf_clear(re);
        mpf_clear(im);
        mpf_clear(scale);
        lastOrbitRequest = now;
        lastOrbitX = x;
        lastOrbitY = y;
    }
    ...
    void requestCurrentOrbit() {
        ...
        RECT vr = viewRect();
        int x = inRect(vr, lastOrbitX, lastOrbitY) ? lastOrbitX : (vr.left + vr.right) / 2;
        int y = inRect(vr, lastOrbitX, lastOrbitY) ? lastOrbitY : (vr.top + vr.bottom) / 2;
        lastOrbitRequest = {};
        requestOrbit(x, y);
    }
    ...
    void finishGenericDeepSmoke(bool passed, const char* detail) {
        FILE* file = nullptr;
        fopen_s(&file, "build\\generic_deep_gui_smoke.txt", "a");
        if (file) {
            fprintf(file, "%s: %s\n", passed ? "PASS" : "FAIL", detail);
            fclose(file);
        }
    ...
    bool startGenericDeepSmokeStage(int stage) {
        ...
        candidate.source = stage == 0 ? "z*z+c+0" : "sin(z)+c";
        candidate.pixelParameter = FormulaParameter::C;
        candidate.bailout = 4.0;
        if (!applyFormulaConfig(candidate, false)) return false;
        maxIter = 32;
        nav->SetMxit(maxIter);
        if (!nav->SetLocation("0", "0", "1e500")) return false;
        genericDeepSmokeStage = stage;
        genericDeepSmokeDeadline = std::chrono::steady_clock::now() + std::chrono::seconds(45);
        startRender();
    ...
    void checkGenericDeepSmoke() {
        ...
        if (file) {
            fprintf(file,
                    ...
                    genericDeepSmokeStage == 0 ? "arithmetic" : "transcendental", info.used ? 1 : 0, info.success ? 1 : 0, info.status.c_str(), (unsigned long long)info.pixelCount, (unsigned long long)info.fastPixelCount, (unsigned long long)info.fallbackPixelCount, (unsigned long long)info.preflightFallbackCount, (unsigned long long)info.preflightSampleCount, info.predictedPath.c_str(), (unsigned long long)info.preflightAvoidedFastPixelCount, info.totalSeconds, info.referenceSeconds, info.preflightSeconds, info.taylorSeconds, info.fallbackSeconds, nav->GetExpressionAccelerationText().c_str());
            fclose(file);
        }
        if (!info.used || !info.settled || !info.success || !nav->LastComputeUsedGenericDeepPath()) {
            finishGenericDeepSmoke(false, "generic-deep path/status mismatch");
            return;
        }
        if (genericDeepSmokeStage == 0) {
            if (!startGenericDeepSmokeStage(1)) finishGenericDeepSmoke(false, "transcendental setup failed");
            return;
        }
        finishGenericDeepSmoke(true, "arithmetic and transcendental e500 renders completed");
    }
    ...
    void startOrbitThumbnailSmoke() {
        ...
        if (orbitThumbnailSmokeExpectedGeneration <= orbitThumbnailSmokeFirstGeneration) {
            finishOrbitThumbnailSmoke(false, "generation did not advance");
            return;
        }
        orbitThumbnailSmokeStage = 0;
        orbitThumbnailSmokeDeadline = std::chrono::steady_clock::now() + std::chrono::seconds(20);
    }

    void checkOrbitThumbnailSmoke(const OrbitThumbnailResult& result) {
        if (!orbitThumbnailSmoke) return;
        const bool complete = orbitThumbnail.size() == (size_t)ORBIT_W * ORBIT_H * 3 && result.width == ORBIT_W && result.height == ORBIT_H;
        if (orbitThumbnailSmokeStage == 0) {
            if (!complete || !result.expression || result.pixelParameter != FormulaParameter::InitialZ || result.generation != orbitThumbnailSmokeExpectedGeneration) {
                finishOrbitThumbnailSmoke(false, "latest Custom z0 background mismatch");
                return;
            }
        ...
        if (!complete || result.expression || result.generation != orbitThumbnailSmokeExpectedGeneration) {
            finishOrbitThumbnailSmoke(false, "Mandelbrot background restore mismatch");
            return;
        }
        finishOrbitThumbnailSmoke(true, "Custom latest-generation z0 and Mandel restore completed");
    }
    ...
    void setReliefAzFromX(int x) {
        ...
        if (de_overlay_on)
            de_k = (float)(DE_K_MIN + t * (DE_K_MAX - DE_K_MIN));
        else
    ...
    void setReliefStrFromX(int x) {
        ...
        if (de_overlay_on)
            de_scale = (float)(DE_SCALE_MIN + t * (DE_SCALE_MAX - DE_SCALE_MIN));
        else
    ...
    void recolorPhaseNow() {
        color_phase = std::fmod(color_phase, (float)colP);
        if (color_phase < 0) color_phase += colP;
        if (nav) {
            nav->RecolorPhase(bitmap.data());
            buildDisplay();
        }
    ...
    // ---- geometry ----
    int formulaDockWidth() const { return formulaEditor && formulaEditor->visible() ? S(FormulaEditorPanel::DESIGN_WIDTH) : 0; }
    ...
    void layout() {
        ...
        if (formulaEditor && formulaEditor->visible()) formulaEditor->move({panelR, 0, rc.right, rc.bottom});
        ...
        rcLocation = {px, y, px + w, y + locationH};
        y += locationH + S(16);
        rcMaxTrack = {px, y + S(24), px + w, y + S(38)};
        y += S(44);
        rcDensTrack = {px, y + S(24), px + w, y + S(38)};
        y += S(44);
        rcPhaseTrack = {px, y + S(24), px + w, y + S(38)};
        y += S(44);
        rcSpeedTrack = {px, y + S(24), px + w, y + S(38)};
        y += S(44);
        // Unlabeled toggle: box sits at y (no label above), then a uniform 14px gap.
        rcSS = {px, y, px + w, y + bh};
        y += bh + S(14);
        rcFormula = {px, y, px + w, y + bh};
        y += bh + S(14);
        if (juliaUiEnabled) {
            rcJulia = {px, y, px + w, y + bh};
            y += bh + S(14);
        } else {
            rcJulia = RECT{};
        }
        rcOrbitToggle = {px, y, px + w, y + bh};
        y += bh + S(14);
        if (orbitOn) {
        ...
        rcColoringDD = {px, y + S(30), px + w, y + S(30) + bh};
        y += S(30) + bh + S(14);
        ...
        if (relief_on || normal_light_on) {
            rcReliefAz = {px, y + S(24), px + w, y + S(38)};
            y += S(52);
            rcReliefEl = {px, y + S(24), px + w, y + S(38)};
            y += S(52);
            rcReliefStr = {px, y + S(24), px + w, y + S(38)};
            y += S(52);
        ...
        rcPaletteDD = {px, y + S(30), px + w, y + S(30) + bh};
        y += S(30) + bh + S(14);
        rcGradient = {px, y - S(4), px + w, y + S(30)};
        y += S(44);
        ...
        if (panelScroll != 0) {
            RECT* all[] = {&rcReset, &rcRender, &rcSave, &rcCopy, &rcPaste, &rcLocation, &rcMaxTrack, &rcDensTrack, &rcPhaseTrack, &rcSpeedTrack, &rcReliefAz, &rcReliefEl, &rcReliefStr, &rcSS, &rcFormula, &rcJulia, &rcOrbitToggle, &rcOrbitThumb, &rcColoringDD, &rcPaletteDD, &rcGradient, &rcColor, &rcGalleryDD};
    ...
    bool applyFormulaConfig(const FormulaDialogConfig& candidate, bool render = true) {
        ...
        if (formulaEditor && formulaEditor->visible()) formulaEditor->setConfig(formulaConfig);
        if (orbitWorker) orbitWorker->cancel();
        orbitResult = OrbitResult{};
        rebuildOrbitThumbnail();
        ssOn = false;
        if (entering || !expressionColoringIndexSupported(coloringIdx, nav->ExpressionSupportsDistance())) coloringIdx = 0;
    ...
    void expandWindowForFormulaEditor() {
        ...
        int grow = std::min(desired, std::max(0, (int)(mi.rcWork.right - mi.rcWork.left) - currentW));
        int growHeight = std::min(std::max(0, S(907) - currentH), std::max(0, (int)(mi.rcWork.bottom - mi.rcWork.top) - currentH));
        if (grow <= 0 && growHeight <= 0) return;
        int newW = currentW + grow;
        int newH = currentH + growHeight;
        int newX = std::clamp(wr.left - grow / 2, mi.rcWork.left, mi.rcWork.right - newW);
        int newY = std::clamp(wr.top - growHeight / 2, mi.rcWork.top, mi.rcWork.bottom - newH);
    ...
    void closeFormulaEditor() {
        ...
        formulaEditor->hide();
        if ((formulaEditorExpansionDip > 0 || formulaEditorHeightExpansionDip > 0)) {
            bool maximized = IsZoomed(hwnd) != FALSE;
            WINDOWPLACEMENT placement{sizeof(placement)};
            RECT wr{};
            if (maximized) {
                if (GetWindowPlacement(hwnd, &placement)) wr = placement.rcNormalPosition;
            } else {
                GetWindowRect(hwnd, &wr);
            }
            HMONITOR monitor = MonitorFromRect(&wr, MONITOR_DEFAULTTONEAREST);
            MONITORINFO mi{sizeof(mi)};
            bool restored = false;
            if (wr.right > wr.left && wr.bottom > wr.top && GetMonitorInfoW(monitor, &mi)) {
                int width = wr.right - wr.left;
                int height = wr.bottom - wr.top;
                int shrink = std::min(S(formulaEditorExpansionDip), std::max(0, width - S(900)));
                int shrinkHeight = std::min(S(formulaEditorHeightExpansionDip), std::max(0, height - S(640)));
                if (shrink > 0 || shrinkHeight > 0) {
                    int newWidth = width - shrink;
                    int newHeight = height - shrinkHeight;
                    int newX = std::clamp(wr.left + shrink / 2, mi.rcWork.left, mi.rcWork.right - newWidth);
                    int newY = std::clamp(wr.top + shrinkHeight / 2, mi.rcWork.top, mi.rcWork.bottom - newHeight);
                    if (maximized) {
                        placement.rcNormalPosition = {newX, newY, newX + newWidth, newY + newHeight};
                        restored = SetWindowPlacement(hwnd, &placement) != FALSE;
                    } else {
                        restored = SetWindowPos(hwnd, nullptr, newX, newY, newWidth, newHeight, SWP_NOZORDER | SWP_NOACTIVATE) != FALSE;
                    }
                } else {
                    restored = true;
                }
    ...
    void createFonts() {
        ...
        fUi = CreateFontW(-S(12), 0, 0, 0, FW_NORMAL, 0, 0, 0, DEFAULT_CHARSET, 0, 0, CLEARTYPE_QUALITY, 0, L"Segoe UI");
        fBold = CreateFontW(-S(12), 0, 0, 0, FW_SEMIBOLD, 0, 0, 0, DEFAULT_CHARSET, 0, 0, CLEARTYPE_QUALITY, 0, L"Segoe UI");
        fSmall = CreateFontW(-S(11), 0, 0, 0, FW_NORMAL, 0, 0, 0, DEFAULT_CHARSET, 0, 0, CLEARTYPE_QUALITY, 0, L"Segoe UI");
        fMono = CreateFontW(-S(12), 0, 0, 0, FW_MEDIUM, 0, 0, 0, DEFAULT_CHARSET, 0, 0, CLEARTYPE_QUALITY, 0, L"Cascadia Mono");
        fMicro = CreateFontW(-S(11), 0, 0, 0, FW_NORMAL, 0, 0, 0, DEFAULT_CHARSET, 0, 0, CLEARTYPE_QUALITY, 0, L"Segoe UI");
    }
...
#pragma omp parallel for schedule(static)
        for (int y = 0; y < renderH; ++y) {
            ...
            for (int x = 0; x < renderW; ++x) {
                ...
                if (sx < 0 || sx >= renderW || sy < 0 || sy >= renderH) {
                    ...
                    display[d] = bitmap[s + 2];
                    display[d + 1] = bitmap[s + 1];
                    display[d + 2] = bitmap[s];
                }
            }
        }
    }
    ...
    void scaleDisplay(int vW, int vH) {
        ...
        if (scaledW != vW || scaledH != vH || (int)scaled.size() != stride * vH) {
            scaled.assign((size_t)stride * vH, 0);
            scaledW = vW;
            scaledH = vH;
            scaledStride = stride;
        }
...
#pragma omp parallel for schedule(static)
        for (int y = 0; y < vH; ++y) {
            ...
            for (int x = 0; x < vW; ++x) {
                ...
                for (int ch = 0; ch < 3; ++ch) d[x * 3 + ch] = (uint8_t)(r0[x0c + ch] * w00 + r0[x1c + ch] * w10 + r1[x0c + ch] * w01 + r1[x1c + ch] * w11 + 0.5);
            }
        }
    }
    void commitView() {
    ...
    void gradientDown(int x) {
        ...
        if (n >= 0)
            ...
            auto it = std::lower_bound(palette.stops.begin(), palette.stops.end(), p, [](const Stop& a, float v) { return a.pos < v; });
            palette.selected = (int)(it - palette.stops.begin());
            palette.stops.insert(it, s);
        }
        palette.dragging = true;
        SetCapture(hwnd);
        InvalidateRect(hwnd, nullptr, FALSE);
    }
    ...
    void gradientDeleteSelected() {
        if (palette.stops.size() <= 2 || palette.selected < 0 || palette.selected >= (int)palette.stops.size()) return;
        int i = palette.selected;
    ...
    void copyLocation() {
        ...
        if (nav->IsExpression()) {
            ...
            snprintf(line, sizeof(line),
                     ...
                     "\r\nfixed_c_re: %.17g\r\nfixed_c_im: %.17g"
                     "\r\nbailout: %.17g",
                     formulaConfig.fixedZ0.real(), formulaConfig.fixedZ0.imag(), formulaConfig.fixedC.real(), formulaConfig.fixedC.imag(), formulaConfig.bailout);
            location += line;
            for (int i = 0; i < 8; ++i) {
                snprintf(line, sizeof(line), "\r\np%d_re: %.17g\r\np%d_im: %.17g", i, formulaConfig.parameters[i].real(), i, formulaConfig.parameters[i].imag());
                location += line;
            }
        }
        ...
        memcpy(GlobalLock(h), t.c_str(), bytes);
        GlobalUnlock(h);
    ...
    void pasteLocation() {
        ...
        auto val = [&](const char* key) -> std::string {
            ...
            size_t e = p;
            while (e < txt.size() && txt[e] != '\r' && txt[e] != '\n') ++e;
        ...
        auto validNumbers = [](const std::vector<std::string>& values, int positiveIndex = -1) {
            size_t chars = 0;
            for (const std::string& value : values) chars = std::max(chars, value.size());
            mp_bitcnt_t precision = std::max<mp_bitcnt_t>(64, (mp_bitcnt_t)(chars * 3.3219) + 40);
            mpf_t parsed;
            mpf_init2(parsed, precision);
            bool ok = true;
            for (int i = 0; i < (int)values.size(); ++i) {
                if (mpf_set_str(parsed, values[i].c_str(), 10) != 0 || (i == positiveIndex && mpf_sgn(parsed) <= 0)) {
                    ok = false;
                    break;
                }
            }
        ...
        if (!precisionText.empty()) {
            char* end = nullptr;
            unsigned long long parsed = strtoull(precisionText.c_str(), &end, 10);
            if (!end || *end || parsed < 64 || parsed > (1u << 20)) return;
        ...
        if (mode == "expression") {
            ...
            candidate.pixelParameter = pixel == "z0" ? FormulaParameter::InitialZ : FormulaParameter::C;
            std::vector<std::string> values = {val("fixed_z0_re:"), val("fixed_z0_im:"), val("fixed_c_re:"), val("fixed_c_im:"), val("bailout:")};
            for (int i = 0; i < 8; ++i) {
                values.push_back(val(("p" + std::to_string(i) + "_re:").c_str()));
                values.push_back(val(("p" + std::to_string(i) + "_im:").c_str()));
            }
            if (candidate.source.empty() || std::any_of(values.begin(), values.end(), [](const std::string& value) { return value.empty(); }) || !validNumbers(values)) return;
            ...
            for (int i = 0; i < 8; ++i) candidate.parameters[i] = {atof(values[5 + 2 * i].c_str()), atof(values[6 + 2 * i].c_str())};
            if (!applyFormulaConfig(candidate, false)) return;
            std::string coloring = val("coloring:");
            if (!coloring.empty()) {
                char* end = nullptr;
                long index = strtol(coloring.c_str(), &end, 10);
                if (!end || *end || index < 0 || index > 6 || !setColoringState((int)index, false)) return;
            }
        } else if (mode == "julia") {
            ...
            if (!nav->SetJuliaC(cr, ci, precisionHint)) {
        ...
        if (nav->SetLocation(xs, ys, scale, precisionHint)) {
            needFull = true;
            startRender();
        }
    }
    void saveImage() {
        ...
        wcscpy_s(file, defaultSaveName().c_str());
        ...
        if (GetSaveFileNameW(&ofn)) {
            ...
            for (size_t i = 0; i < (size_t)renderW * renderH; ++i) {
                rgb[i * 3] = display[i * 3 + 2];
                rgb[i * 3 + 1] = display[i * 3 + 1];
                rgb[i * 3 + 2] = display[i * 3];
            }
    ...
    // ---- widget drawing ----
    void drawButton(HDC dc, RECT r, const std::wstring& s, Hit id, bool accent, bool activeOutline = false) {
        bool hov = hover == id, prs = pressed == id;
        COLORREF bg = accent ? (prs ? RGB(70, 120, 220) : hov ? CLR_ACCENT_HI
                                                              : CLR_ACCENT)
                             : (prs || hov ? CLR_CARD_HOV : CLR_CARD);
        COLORREF border = activeOutline ? CLR_ACCENT : (accent ? bg : CLR_BORDER);
        fillRound(dc, r, bg, border, S(8));
        if (activeOutline) {
            RECT stripe = {r.left + S(1), r.top + S(5), r.left + S(4), r.bottom - S(5)};
            fillRound(dc, stripe, CLR_ACCENT, CLR_ACCENT, S(3));
        }
        HFONT font = id >= H_RESET && id <= H_PASTE ? fMicro : fUi;
        drawText(dc, r, s, accent ? RGB(255, 255, 255) : CLR_TEXT, font, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
    }
    void drawSlider(HDC dc, RECT track, double t, Hit id, const double* snaps = nullptr, int nsnaps = 0) {
    ...
    void drawFormulaButton(HDC dc) {
        ...
        fillRound(dc, rcFormula, prs || hov ? CLR_CARD_HOV : CLR_CARD, active ? CLR_ACCENT : CLR_BORDER, S(8));
        if (active) {
            RECT stripe = {rcFormula.left + S(1), rcFormula.top + S(5), rcFormula.left + S(4), rcFormula.bottom - S(5)};
            fillRound(dc, stripe, CLR_ACCENT, CLR_ACCENT, S(3));
        }
        RECT title = rcFormula;
        title.left += S(12);
        title.right -= S(72);
        drawText(dc, title, L"Formula editor", CLR_TEXT, fUi, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
        const wchar_t* state = open ? L"OPEN" : (nav->IsExpression() ? L"CUSTOM" : L"");
        if (*state) {
            RECT status = rcFormula;
            status.left = status.right - S(68);
            status.right -= S(12);
            drawText(dc, status, state, CLR_ACCENT_HI, fMicro, DT_RIGHT | DT_VCENTER | DT_SINGLELINE);
        }
    }
    void drawSeg(HDC dc, RECT r, const std::wstring& s, bool sel, Hit id) {
        bool hov = hover == id;
        fillRound(dc, r, sel ? CLR_ACCENT : (hov ? CLR_CARD_HOV : CLR_CARD), sel ? CLR_ACCENT : CLR_BORDER, S(8));
        drawText(dc, r, s, sel ? RGB(255, 255, 255) : CLR_TEXT, fUi, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
    }
    ...
    static std::array<int, 3> sampleStops(const std::vector<Stop>& st, float t) {
        ...
        for (size_t i = 0; i + 1 < st.size(); ++i)
            if (t >= st[i].pos && t <= st[i + 1].pos) {
                ...
                return {(int)(st[i].r + (st[i + 1].r - st[i].r) * f), (int)(st[i].g + (st[i + 1].g - st[i].g) * f), (int)(st[i].b + (st[i + 1].b - st[i].b) * f)};
            }
    ...
    RECT paletteListRect() const {
        ...
        return {rcPaletteDD.left, rcPaletteDD.bottom + S(2), rcPaletteDD.right, rcPaletteDD.bottom + S(2) + n * paletteItemH()};
    }
    ...
    RECT coloringListRect() const {
        int n = 7;
        return {rcColoringDD.left, rcColoringDD.bottom + S(2), rcColoringDD.right, rcColoringDD.bottom + S(2) + n * coloringItemH()};
    }
    ...
    void drawColoringDD(HDC dc) {
        ...
        fillRound(dc, rcColoringDD, hov ? CLR_CARD_HOV : CLR_CARD, coloringOpen ? CLR_ACCENT : CLR_BORDER, S(8));
        RECT tr = rcColoringDD;
        tr.left += S(12);
        tr.right -= S(28);
        const wchar_t* currentName = nav->IsExpression() && coloringIdx == 0 && !nav->ExpressionSupportsDistance() ? L"Iteration (raw)" : coloringName(coloringIdx);
        drawText(dc, tr, currentName, CLR_TEXT, fUi, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
        RECT cr = rcColoringDD;
        cr.right -= S(12);
    ...
    void drawColoringList(HDC dc) {
        ...
        for (int i = 0; i < 7; ++i) {
            ...
            RECT tr = ir;
            tr.left += S(14);
            COLORREF color = ((nav->IsJulia() && !nav->IsExpression() && i > 1) || (nav->IsExpression() && !expressionColoringIndexSupported(i, nav->ExpressionSupportsDistance()))) ? CLR_TEXT_DIM : (i == coloringIdx ? CLR_ACCENT : CLR_TEXT);
            const wchar_t* name = nav->IsExpression() && i == 0 && !nav->ExpressionSupportsDistance() ? L"Iteration (raw)" : coloringName(i);
            drawText(dc, tr, name, color, fUi, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
        }
    }
    bool setColoringState(int idx, bool render) {
        if (idx < 0 || idx > 6) return false;
        if (nav->IsJulia() && !nav->IsExpression() && idx > 1) return false;
        if (nav->IsExpression() && !expressionColoringIndexSupported(idx, nav->ExpressionSupportsDistance())) return false;
        coloringIdx = idx;
        relief_on = (idx == 3) ? 1 : 0;
        ...
        if (idx == 1)
            m |= ColoringMethod::EXTERIOR_DIST_EST;
        else if (idx == 2)
            m |= ColoringMethod::STRIPE_AVERAGE;
        else if (idx == 4)
            m |= ColoringMethod::NORMAL_MAP;
        else if (idx == 5)
            m |= ColoringMethod::ORBIT_TRAP;
        else if (idx == 6)
            m |= ColoringMethod::DE_OVERLAY;
    ...
    void selectColoring(int idx) { setColoringState(idx, true); }
    ...
    void drawGalleryDD(HDC dc) {
        ...
        fillRound(dc, rcGalleryDD, hov ? CLR_CARD_HOV : CLR_CARD, galleryOpen ? CLR_ACCENT : CLR_BORDER, S(8));
        RECT tr = rcGalleryDD;
        tr.left += S(12);
        tr.right -= S(28);
    ...
    void applyPreset(int idx) {
        ...
        maxIter = std::clamp(p.maxIter, 100, 5000000);
        nav->SetMxit(maxIter);
        ...
        if (coloringIdx == 1)
            ...
            m |= ColoringMethod::STRIPE_AVERAGE;
        else if (coloringIdx == 4)
            m |= ColoringMethod::NORMAL_MAP;
        else if (coloringIdx == 5)
            m |= ColoringMethod::ORBIT_TRAP;
        else if (coloringIdx == 6)
            m |= ColoringMethod::DE_OVERLAY;
        nav->SetCMethod(m);
        // palette by name
        const auto& pr = palettePresets();
        for (int i = 0; i < (int)pr.size(); ++i)
            if (wcscmp(pr[i].name, p.palette) == 0) {
                paletteIdx = i;
                palette.load(i);
                break;
            }
    ...
    void drawPaletteDD(HDC dc) {
        ...
        fillRound(dc, rcPaletteDD, hov ? CLR_CARD_HOV : CLR_CARD, paletteOpen ? CLR_ACCENT : CLR_BORDER, S(8));
    ...
    void paint() {
        ...
        if (stretchEnv < 0) {
            const char* e = getenv("MANDEL_STRETCH");
            stretchEnv = e ? atoi(e) : 1;
        }
        if (vW == renderW && vH == renderH) {
            bi.bmiHeader.biWidth = renderW;
            bi.bmiHeader.biHeight = -renderH;
            StretchDIBits(dc, vr.left, vr.top, vW, vH, 0, 0, renderW, renderH, display.data(), &bi, DIB_RGB_COLORS, SRCCOPY); // 1:1 copy
        } else if (stretchEnv == 1 || stretchEnv == 3) {
            scaleDisplay(vW, vH);
            bi.bmiHeader.biWidth = vW;
            bi.bmiHeader.biHeight = -vH;
            StretchDIBits(dc, vr.left, vr.top, vW, vH, 0, 0, vW, vH, scaled.data(), &bi, DIB_RGB_COLORS, SRCCOPY); // 1:1 copy
        } else {
            bi.bmiHeader.biWidth = renderW;
            bi.bmiHeader.biHeight = -renderH;
            SetStretchBltMode(dc, stretchEnv == 2 ? HALFTONE : COLORONCOLOR);
            StretchDIBits(dc, vr.left, vr.top, vW, vH, 0, 0, renderW, renderH, display.data(), &bi, DIB_RGB_COLORS, SRCCOPY);
        }
        ...
        if (full) {
            ...
            RECT panelRail = {panel.right - S(11), panel.top, panel.right, panel.bottom};
            fillRect(dc, panelRail, RGB(21, 24, 33));
            HPEN sep = CreatePen(PS_SOLID, 1, CLR_BORDER);
            HGDIOBJ osep = SelectObject(dc, sep);
            MoveToEx(dc, panel.left, 0, nullptr);
            LineTo(dc, panel.left, rc.bottom);
            SelectObject(dc, osep);
            DeleteObject(sep);
            ...
            lt.bottom -= S(6);
            {
                ...
                if (nav->IsExpression()) {
                    ...
                    if (formulaLine != std::string::npos) location.resize(formulaLine);
                }
                HGDIOBJ ofm = SelectObject(dc, fMono);
                SIZE cs{};
                GetTextExtentPoint32W(dc, L"0", 1, &cs);
                SelectObject(dc, ofm);
                int cpl = cs.cx > 0 ? (int)((lt.right - lt.left) / cs.cx) : 40;
                drawText(dc, lt, truncLocation(location, cpl), CLR_TEXT, fMono, DT_LEFT | DT_TOP | DT_NOPREFIX);
            }
            ...
            wchar_t db[32];
            swprintf_s(db, L"%.1f", color_density);
            ...
            wchar_t pb[32];
            swprintf_s(pb, L"%.2f", color_phase / (float)colP);
            ...
            wchar_t sb[32];
            if (std::fabs(animSpeed) < 1e-3f)
                swprintf_s(sb, L"paused");
            else
                swprintf_s(sb, L"%+.2f", animSpeed);
            labelRow(dc, rcSpeedTrack, L"Cycle speed", sb);
            const double speedSnaps[] = {0.5};
            drawSlider(dc, rcSpeedTrack, std::clamp((animSpeed + 1.0) / 2.0, 0.0, 1.0), H_SPEEDTRACK, speedSnaps, 1);

            drawToggle(dc, rcSS, L"5x supersampling", ssOn, H_SS);
            drawFormulaButton(dc);
            if (juliaUiEnabled) drawToggle(dc, rcJulia, L"Julia set (experimental)", nav->IsJulia(), H_JULIA);
            ...
            if (relief_on || normal_light_on) {
                wchar_t rab[32];
                swprintf_s(rab, L"%.0f\u00B0", relief_light_az * 180.0f / 3.14159265f);
                ...
                wchar_t reb[32];
                swprintf_s(reb, L"%.0f\u00B0", relief_light_el * 180.0f / 3.14159265f);
                ...
                wchar_t rsb[32];
                swprintf_s(rsb, L"%.2f", relief_strength);
            ...
            for (int x = rcGradient.left; x < rcGradient.right; ++x) {
                ...
                MoveToEx(dc, x, rcGradient.top, nullptr);
                LineTo(dc, x, rcGradient.bottom);
                SelectObject(dc, op);
                DeleteObject(pen);
            }
            for (int i = 0; i < (int)palette.stops.size(); ++i) {
            ...
            // selected color button (shows swatch)
            {
                ...
                RECT tr = rcColor;
                tr.left += S(44);
                drawText(dc, tr, L"Edit color  \u00b7  Delete removes stop", CLR_TEXT, fUi, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
            }
        ...
        RECT status = {0, rc.bottom - S(STATUS_H), panelRight() - S(PANEL_W), rc.bottom};
        fillRect(dc, status, CLR_PANEL);
        const bool computing = nav->IsComputing();
        std::wstring st = computing ? L"  Rendering " + std::to_wstring((int)std::lround(100.0f * nav->GetComputeProgress())) + L"%..." : L"  Ready   last render " + std::to_wstring((int)lastRenderMs) + L" ms";
        st += L"   present " + std::to_wstring((int)(lastPresentMs + 0.5)) + L" ms";
        {
            ...
            if (backend.fallback)
                st += L" (fallback)";
            else if (backend.hardwareAccelerated && !nav->IsComputing() && !nav->LastComputeUsedGpuPath())
                st += L" (CPU frame)";
        }
        if (nav->IsExpression()) {
        ...
        if (!nav->IsComputing()) st += L"    Drag pan   Wheel zoom   R reset   Space render   S save   C copy";
        RECT str = status;
        str.left += S(10);
        ...
        if (full) {
            ...
            BitBlt(wdc, status.left, status.top, status.right - status.left, status.bottom - status.top, dc, status.left, status.top, SRCCOPY);
            if (orbitOn) {
                RECT orb = rcOrbitThumb;
                orb.top -= S(20);
                BitBlt(wdc, orb.left, orb.top, orb.right - orb.left, orb.bottom - orb.top, dc, orb.left, orb.top, SRCCOPY);
            }
        }
        EndPaint(hwnd, &ps);
        lastPresentMs = std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - _pt0).count();
    }

    Hit hitTest(int x, int y) {
        {
            RECT sb = panelScrollbarRect();
            if (sb.right > sb.left && inRect(sb, x, y)) return H_PANELSB;
        }
        ...
        mt.top -= S(8);
        mt.bottom += S(8);
        if (inRect(mt, x, y)) return H_MAXTRACK;
        RECT dt = rcDensTrack;
        dt.top -= S(8);
        dt.bottom += S(8);
        if (inRect(dt, x, y)) return H_DENSTRACK;
        RECT pt = rcPhaseTrack;
        pt.top -= S(8);
        pt.bottom += S(8);
        if (inRect(pt, x, y)) return H_PHASETRACK;
        RECT sp = rcSpeedTrack;
        sp.top -= S(8);
        sp.bottom += S(8);
        if (inRect(sp, x, y)) return H_SPEEDTRACK;
        if (relief_on || normal_light_on || de_overlay_on) {
            RECT ra = rcReliefAz;
            ra.top -= S(8);
            ra.bottom += S(8);
            if (inRect(ra, x, y)) return H_RELAZ;
            RECT re = rcReliefEl;
            re.top -= S(8);
            re.bottom += S(8);
            if (inRect(re, x, y)) return H_RELEL;
            RECT rs = rcReliefStr;
            rs.top -= S(8);
            rs.bottom += S(8);
            if (inRect(rs, x, y)) return H_RELSTR;
        }
        ...
        RECT vr = viewRect();
        if (inRect(vr, x, y)) return H_VIEW;
        return H_NONE;
    }
    ...
    void runRecolorBench() {
        ...
        nav->SetRedisplay();
        nav->UpdateBitmap(bitmap.data()); // ensure settled cache is built
        const int N = 60;
        ...
        if (getenv("MANDEL_GUI_DUMP")) {
            ...
            scaleDisplay(vW, vH);
            FILE* bf = nullptr;
            fopen_s(&bf, "build\\gui_scaled.bmp", "wb");
            if (bf) {
            ...
            FILE* ff = nullptr;
            fopen_s(&ff, "build\\gui_frame.bmp", "wb");
            if (ff) {
                int stride = (renderW * 3 + 3) & ~3;
                uint32_t ds = (uint32_t)stride * renderH, fs = 54 + ds, off = 54;
                uint8_t fh[14] = {'B', 'M'};
                memcpy(fh + 2, &fs, 4);
                memcpy(fh + 10, &off, 4);
                uint8_t ih[40] = {0};
                ...
                v = 40;
                memcpy(ih, &v, 4);
                int32_t iw = renderW, ihh = renderH;
                memcpy(ih + 4, &iw, 4);
                memcpy(ih + 8, &ihh, 4);
                uint16_t pl = 1, bp = 24;
                memcpy(ih + 12, &pl, 2);
            ...
            if (memDC && memBmp) {
                ...
                pbi.bmiHeader.biWidth = memW;
                pbi.bmiHeader.biHeight = memH;
                pbi.bmiHeader.biPlanes = 1;
                pbi.bmiHeader.biBitCount = 24;
                pbi.bmiHeader.biCompression = BI_RGB;
                int pst = (memW * 3 + 3) & ~3;
                std::vector<uint8_t> pbuf((size_t)pst * memH);
                if (GetDIBits(memDC, memBmp, 0, memH, pbuf.data(), &pbi, DIB_RGB_COLORS)) {
                    FILE* pf = nullptr;
                    fopen_s(&pf, "build\\gui_present.bmp", "wb");
                    if (pf) {
        ...
        FILE* f = nullptr;
        fopen_s(&f, "build\\gui_bench.txt", "a");
        if (f) {
            const GenericDeepInfo deep = nav->GetLastGenericDeepInfo();
            fprintf(f, "SS=%d  render=%.1f ms  recolor=%.3f ms  full=%.3f  buildDisplay=%.3f  present(paint)=%.3f  view=%ldx%ld win=%ldx%ld\n", ssOn ? 1 : 0, renderMs, rp, ub, bd, pt, vr.right - vr.left, vr.bottom - vr.top, rcw.right, rcw.bottom);
            if (deep.used) {
                fprintf(f, "generic status=%s pixels=%llu fast=%llu fallback=%llu Taylor=%llu preflight=%llu/%llu predicted=%s avoided=%llu specialized_mpfr=%d bigfixed=%d total_iter=%llu mpfr_pixels=%llu mpfr_iter=%llu periodic=%llu generic_periodic=%llu bf_pixels=%llu bf_iter=%llu precision=%llu/%llu total=%.6f reference=%.6f preflight_time=%.6f build+eval=%.6f fast_time=%.6f fallback_time=%.6f preflight_iter=%llu preflight_ops=%llu preflight_folds=%llu first_uncertain=", deep.status.c_str(), (unsigned long long)deep.pixelCount, (unsigned long long)deep.fastPixelCount, (unsigned long long)deep.fallbackPixelCount, (unsigned long long)deep.taylorPixelCoverage, (unsigned long long)deep.preflightFallbackCount, (unsigned long long)deep.preflightSampleCount, deep.predictedPath.c_str(), (unsigned long long)deep.preflightAvoidedFastPixelCount, deep.specializedPiecewiseMpfr ? 1 : 0, deep.piecewiseBigFixed ? 1 : 0, (unsigned long long)deep.totalIterationCount, (unsigned long long)deep.specializedPiecewiseMpfrPixelCount, (unsigned long long)deep.specializedPiecewiseMpfrIterationCount,
                        (unsigned long long)deep.specializedPiecewiseMpfrPeriodicPixelCount, (unsigned long long)deep.genericMpfrPeriodicPixelCount, (unsigned long long)deep.piecewiseBigFixedPixelCount, (unsigned long long)deep.piecewiseBigFixedIterationCount, (unsigned long long)deep.selectedPrecision, (unsigned long long)deep.fallbackPrecision, deep.totalSeconds, deep.referenceSeconds, deep.preflightSeconds, deep.taylorSeconds, deep.fastSeconds, deep.fallbackSeconds, (unsigned long long)deep.preflightIterationCount, (unsigned long long)deep.preflightOperationCount, (unsigned long long)deep.preflightFoldOperationCount);
                for (size_t bin = 0; bin < deep.preflightFirstUncertainHistogram.size(); ++bin) {
                    if (bin != 0) fputc(',', f);
    ...
    void timer() {
        ...
        if (orbitThumbnailSmoke && std::chrono::steady_clock::now() > orbitThumbnailSmokeDeadline) {
            finishOrbitThumbnailSmoke(false, "thumbnail worker timed out");
            return;
        }
        if (orbitOn && orbitWorker) {
            ...
            if (orbitWorker->takeLatest(latest)) {
                ...
                if (orbitBench) {
                    FILE* f = nullptr;
                    fopen_s(&f, "build\\orbit_bench.txt", "a");
                    if (f) {
                        fprintf(f, "orbit=%.3f ms iterations=%d points=%zu escaped=%d pixel=(%.17g,%.17g)\n", orbitResult.computeMs, orbitResult.iterations, orbitResult.points.size(), orbitResult.escaped ? 1 : 0, orbitResult.pixelRe, orbitResult.pixelIm);
                        fclose(f);
                    }
                    PostQuitMessage(0);
                }
        ...
        if (!genericDeepSmoke && getenv("MANDEL_GUI_GENERIC_DEEP_SMOKE")) return;
        if (benchMode && !fpsLogMode && !computing) {
            runRecolorBench();
            return;
        }
        bool anim = std::fabs(animSpeed) > 1e-4f;
        bool active = computing || wasComputing || navDragging || palette.dragging || pressed == H_MAXTRACK || pressed == H_DENSTRACK || pressed == H_PHASETRACK || pressed == H_SPEEDTRACK || pressed == H_RELAZ || pressed == H_RELEL || pressed == H_RELSTR || liveFrames > 0 || anim;
        auto now = std::chrono::steady_clock::now();
        if (!active) {
            lastAnimTick = now;
            return;
        } // idle: no repaint, no CPU spin
        ...
        if (lastRenderTs.time_since_epoch().count()) {
            ...
            if (capMs < 0) {
                const char* c = getenv("MANDEL_GUI_CAPMS");
                capMs = c ? atof(c) : 12.0;
            }
        ...
        bool phaseOnly = !computing && !wasComputing && !navDragging && pressed != H_MAXTRACK && pressed != H_DENSTRACK && (anim || pressed == H_PHASETRACK || pressed == H_SPEEDTRACK || pressed == H_RELAZ || pressed == H_RELEL || pressed == H_RELSTR);
        auto _rc0 = std::chrono::steady_clock::now();
        if (phaseOnly)
            nav->RecolorPhase(bitmap.data());
        else
            nav->UpdateBitmap(bitmap.data());
        lastRecolorMs = std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - _rc0).count();
        // Smoothed frame rate while animating (measured cadence, not the timer set point).
        if (anim || pressed == H_PHASETRACK || pressed == H_RELAZ || pressed == H_RELEL || pressed == H_RELSTR) {
            if (lastFrameTs.time_since_epoch().count()) {
                double dtf = std::chrono::duration<double>(now - lastFrameTs).count();
                if (dtf > 0) {
                    double f = 1.0 / dtf;
                    animFps = animFps ? animFps * 0.85 + f * 0.15 : f;
                }
            }
            lastFrameTs = now;
        } else {
        ...
        if (wasComputing && !computing) lastRenderMs = std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - renderStart).count();
        ...
        bool uiInteract = palette.dragging || pressed == H_MAXTRACK || pressed == H_DENSTRACK || pressed == H_PHASETRACK || pressed == H_SPEEDTRACK || pressed == H_RELAZ || pressed == H_RELEL || pressed == H_RELSTR;
        ...
        if (fpsLogMode) {
            ...
            if (!computing) {
                ...
                if (fpsLogStart.time_since_epoch().count() == 0) {
                    fpsLogStart = tnow;
                    fpsLogLast = tnow;
                }
                ...
                if (sinceLast >= 0.5) {
                    ...
                    if (animFps > 0) {
                        fpsLogSum += animFps;
                        ++fpsLogSamples;
                        if (animFps < fpsLogMin) fpsLogMin = animFps;
                    }
                    RECT vr = viewRect();
                    FILE* f = nullptr;
                    fopen_s(&f, "build\\gui_fpslog.txt", "a");
                    if (f) {
                        fprintf(f, "[fpslog] anim=%.1f fps (%.1f ms/frame)  handler=%.2f ms  recolor=%.2f  present=%.2f  view=%ldx%ld ss=%d\n", animFps, animFps > 0 ? 1000.0 / animFps : 0.0, handlerMs, lastRecolorMs, lastPresentMs, vr.right - vr.left, vr.bottom - vr.top, ssOn ? 1 : 0);
                        fclose(f);
                    }
                }
    ...
    LRESULT message(UINT msg, WPARAM wp, LPARAM lp) {
        ...
        case WM_CREATE: {
            ...
            nav->SetMxit(maxIter);
            nav->SetCMethod(coloringIdx == 1 ? ColoringMethod::EXTERIOR_DIST_EST : coloringIdx == 2 ? ColoringMethod::STRIPE_AVERAGE
                                                                                                    : 0);
            ...
            juliaUiEnabled = getenv("MANDEL_EXPERIMENTAL_JULIA") != nullptr || getenv("MANDEL_GUI_JULIA") != nullptr;
            orbitThumbnailWorker = std::make_unique<OrbitThumbnailWorker>();
            orbitThumbnailSmoke = getenv("MANDEL_GUI_ORBIT_THUMBNAIL_SMOKE") != nullptr;
            genericDeepSmoke = getenv("MANDEL_GUI_GENERIC_DEEP_SMOKE") != nullptr;
            if (genericDeepSmoke) DeleteFileW(L"build\\generic_deep_gui_smoke.txt");
            ...
            const bool anyBench = getenv("MANDEL_GUI_BENCH") || orbitBench || orbitThumbnailSmoke || genericDeepSmoke;
            if (anyBench) {
                ...
                if (gx && gy && gz) {
                    ...
                    if (const char* value = getenv("MANDEL_GUI_PRECISION")) precision = static_cast<mp_bitcnt_t>(strtoull(value, nullptr, 10));
                    if (!sc.empty()) nav->SetLocation(gx, gy, sc, precision);
                }
                if (const char* ww = getenv("MANDEL_GUI_WINW")) {
                    int winw = atoi(ww);
                    const char* wh = getenv("MANDEL_GUI_WINH");
                    int winh = wh ? atoi(wh) : (int)(winw * 0.66);
                    if (winw > 200 && winh > 200) MoveWindow(hwnd, 40, 40, winw, winh, TRUE);
                }
            }
            if (const char* e = getenv("MANDEL_GUI_JULIA"); e && atoi(e)) { switchJuliaMode(true, false); }
            if (const char* expression = getenv("MANDEL_GUI_FORMULA")) {
                ...
                config.pixelParameter = plane && strcmp(plane, "z0") == 0 ? FormulaParameter::InitialZ : FormulaParameter::C;
                ...
                if (getenv("MANDEL_GUI_FORMULA_RESTORE")) restoreMandelbrotUi(false);
            ...
            if (anyBench) {
                if (const char* value = getenv("MANDEL_GUI_MXIT")) {
                    maxIter = std::clamp(atoi(value), 1, 5000000);
                    nav->SetMxit(maxIter);
                }
            }
            if (orbitThumbnailSmoke) startOrbitThumbnailSmoke();
            if (getenv("MANDEL_GUI_BENCH")) {
                ...
                if (const char* pp = getenv("MANDEL_GUI_PAL")) {
                    paletteIdx = atoi(pp);
                    palette.load(paletteIdx);
                }
                ...
                if (getenv("MANDEL_GUI_OPENPAL")) {
                    paletteOpen = true;
                    layout();
                    int over = (int)paletteListRect().bottom - panelViewH + S(8);
                    if (over > 0) panelScrollBy(over);
                }
            ...
            if (genericDeepSmoke) {
                ...
                display.assign((size_t)renderW * renderH * 3, 0);
                nav->Resize(renderW, renderH);
                if (!startGenericDeepSmokeStage(0)) finishGenericDeepSmoke(false, "arithmetic setup failed");
                ...
                const char* requestedWidth = getenv("MANDEL_GUI_RENDERW");
                const char* requestedHeight = getenv("MANDEL_GUI_RENDERH");
                const int explicitWidth = requestedWidth ? atoi(requestedWidth) : 0;
                const int explicitHeight = requestedHeight ? atoi(requestedHeight) : 0;
                if (explicitWidth > 1 && explicitHeight > 1) {
                    renderW = explicitWidth;
                    renderH = explicitHeight;
                    bitmap.assign(static_cast<size_t>(renderW) * renderH * 3, 0);
                    display.assign(static_cast<size_t>(renderW) * renderH * 3, 0);
                    nav->Resize(renderW, renderH);
                    startRender();
                } else {
            ...
            {
                int tms = 4;
                if (const char* t = getenv("MANDEL_GUI_TIMERMS")) {
                    tms = atoi(t);
                    if (tms < 1) tms = 1;
                }
                SetTimer(hwnd, TIMER_ID, tms, nullptr);
            }
            if (getenv("MANDEL_GUI_OPEN_FORMULA")) PostMessageW(hwnd, WM_OPEN_FORMULA_TEST, 0, 0);
            return 0;
        }
        case WM_OPEN_FORMULA_TEST: showFormulaEditor(); return 0;
        case WM_DPICHANGED: {
            dpi = HIWORD(wp);
            createFonts();
        ...
        case WM_MOUSEMOVE: {
            ...
            if (pressed == H_MAXTRACK) {
                setMaxFromT((double)(x - rcMaxTrack.left) / (rcMaxTrack.right - rcMaxTrack.left), false);
                InvalidateRect(hwnd, nullptr, FALSE);
                return 0;
            }
            if (pressed == H_DENSTRACK) {
                color_density = (float)std::round(std::clamp(10.0 + 190.0 * (x - rcDensTrack.left) / (rcDensTrack.right - rcDensTrack.left), 10.0, 200.0));
                nav->SetRedisplay();
                InvalidateRect(hwnd, nullptr, FALSE);
                return 0;
            }
            if (pressed == H_PHASETRACK) {
                setPhaseFromX(x);
                return 0;
            }
            if (pressed == H_SPEEDTRACK) {
                setSpeedFromX(x);
                InvalidateRect(hwnd, nullptr, FALSE);
                return 0;
            }
        ...
        case WM_LBUTTONDOWN: {
            int x = GET_X_LPARAM(lp), y = GET_Y_LPARAM(lp);
            if (formulaEditor && formulaEditor->visible()) formulaEditor->dismissPopups();
            ...
            if (h == H_EDE) {
                ...
                pressed = H_NONE;
                layout();
                int over = (int)coloringListRect().bottom - panelViewH + S(8);
                if (over > 0) panelScrollBy(over);
                InvalidateRect(hwnd, nullptr, FALSE);
                return 0;
            }
            if (h == H_GALLERY_DD) {
                ...
                pressed = H_NONE;
                layout();
                int over = (int)galleryListRect().bottom - panelViewH + S(8);
                if (over > 0) panelScrollBy(over);
                InvalidateRect(hwnd, nullptr, FALSE);
                return 0;
            }
        ...
        case WM_LBUTTONUP: {
            ...
            if (palette.dragging) {
                ...
                if (pressed == H_MAXTRACK)
                    ...
                    startRender();
            } else if (pressed == H_PHASETRACK || pressed == H_SPEEDTRACK || pressed == H_RELAZ || pressed == H_RELEL || pressed == H_RELSTR) {
                ...
                case H_SS:
                    ...
                    break;
                case H_FORMULA: showFormulaEditor(); break;
                case H_JULIA: {
        ...
        case WM_MOUSEWHEEL: {
            POINT q{GET_X_LPARAM(lp), GET_Y_LPARAM(lp)};
            ...
            int panelR = panelRight();
            if (q.x >= panelR - S(PANEL_W) && q.x < panelR && panelMaxScroll() > 0) panelScrollBy(wd > 0 ? -S(48) : S(48));
            return 0;
        }
        case WM_KEYDOWN:
            if (wp == 'R') {
                ...
                pasteLocation();
            else if ((wp == VK_DELETE || wp == VK_BACK) && gradientFocused)
                gradientDeleteSelected();
            else if (wp == VK_ADD || wp == VK_OEM_PLUS) {
                nav->ZoomIn(renderW / 2, renderH / 2);
                keepLive();
            } else if (wp == VK_SUBTRACT || wp == VK_OEM_MINUS) {
                nav->ZoomOut(renderW / 2, renderH / 2);
                keepLive();
            }
            return 0;
        case WM_GETMINMAXINFO: {
            auto* m = (MINMAXINFO*)lp;
            m->ptMinTrackSize.x = S(formulaEditor && formulaEditor->visible() ? 1100 : 900);
...
App* g_app = nullptr;
void App::fixCallback() {
    if (g_app) g_app->commitView();
}
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int show) {
    ...
    const bool benchHeadless = (getenv("MANDEL_GUI_BENCH") || getenv("MANDEL_GUI_ORBIT_BENCH") || getenv("MANDEL_GUI_ORBIT_THUMBNAIL_SMOKE") || getenv("MANDEL_GUI_GENERIC_DEEP_SMOKE")) && !getenv("MANDEL_GUI_SHOW");
    HWND hwnd = CreateWindowExW(0, wc.lpszClassName, L"Mandelbrot Explorer", WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | (benchHeadless ? 0 : WS_VISIBLE), wx, wy, winW, winH, nullptr, nullptr, instance, &app);
    if (!hwnd) return 1;
    ShowWindow(hwnd, benchHeadless ? SW_HIDE : show);
    UpdateWindow(hwnd);
