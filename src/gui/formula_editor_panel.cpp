...
const std::array<Preset, 8>& presets() {
    static const std::array<Preset, 8> values = {{
        {L"Custom", nullptr, false, {}},
        {L"Quadratic", "z*z+c", false, {}},
        {L"Cubic", "z*z*z+c", false, {}},
        {L"Burning Ship",
         "sqr(complex(abs(real(z)),abs(imag(z))))+c", false, {}},
        {L"Sine", "sin(z)+c", false, {}},
        {L"Parameter polynomial", "z*z+c+p0*z", true, {0.25, 0.0}},
        {L"Iteration drift", "z*z+c+0.0001*n", false, {}},
        {L"Branch power", "exp(p0*log(z))+c", true, {2.0, 0.0}}
    }};
    return values;
}
...
const std::array<std::array<const wchar_t*, 16>, 3>& functionSets() {
    static const std::array<std::array<const wchar_t*, 16>, 3> sets = {{
        {{
            L"sqr", L"^", L"sqrt", L"abs",
            L"sin", L"cos", L"tan", L"exp",
            L"log", L"conj", L"real", L"imag",
            L"norm", L"arg", L"polar", L"complex"
        }},
        {{
            L"conj", L"real", L"imag", L"norm",
            L"arg", L"abs", L"sqrt", L"complex",
            L"polar", L"exp", L"log", L"log10",
            L"sinh", L"cosh", L"tanh", L"pow"
        }},
        {{
            L"sqr", L"^", L"sqrt", L"abs",
            L"sin", L"cos", L"tan", L"sinh",
            L"cosh", L"tanh", L"exp", L"log",
            L"log10", L"conj", L"complex", L"polar"
        }}
    }};
    return sets;
}
...
size_t wideIndexForUtf8Offset(const std::string& source, size_t offset) {
    ...
    int count = MultiByteToWideChar(
        CP_UTF8, 0, source.data(), static_cast<int>(offset), nullptr, 0);
...
bool parseFiniteNumber(const std::wstring& text, double& value) {
    ...
    if (errno == ERANGE || !end || end == begin || *end ||
        !std::isfinite(parsed)) {
        return false;
    }
    value = parsed;
...
bool inspectorForName(const std::wstring& name, InspectorValue& value) {
    if (name == L"z") value = InspectorValue::Z;
    else if (name == L"c") value = InspectorValue::C;
    else if (name == L"z0") value = InspectorValue::Z0;
    else if (name == L"n") value = InspectorValue::N;
    else if (name.size() == 2 && name[0] == L'p' &&
             name[1] >= L'0' && name[1] <= L'7') {
        value = static_cast<InspectorValue>(
            static_cast<int>(InspectorValue::P0) + name[1] - L'0');
    } else {
        return false;
    }
    return true;
...
InspectorValue variableFromButton(size_t index) {
    static const std::array<InspectorValue, 12> values = {
        InspectorValue::Z, InspectorValue::C,
        InspectorValue::Z0, InspectorValue::N,
        InspectorValue::P0, InspectorValue::P1,
        InspectorValue::P2, InspectorValue::P3,
        InspectorValue::P4, InspectorValue::P5,
        InspectorValue::P6, InspectorValue::P7
    };
    return values[index];
}
...
int parameterIndex(InspectorValue value) {
    int index = static_cast<int>(value) -
                static_cast<int>(InspectorValue::P0);
...
bool sameConfig(const FormulaDialogConfig& a,
                const FormulaDialogConfig& b) {
    if (a.source != b.source ||
        a.pixelParameter != b.pixelParameter ||
        a.fixedZ0 != b.fixedZ0 ||
        a.fixedC != b.fixedC ||
        a.bailout != b.bailout) {
        return false;
...
std::vector<Token> tokenizeFormula(const std::wstring& text) {
    ...
    while (i < text.size()) {
        ...
        if (std::iswspace(ch)) {
            ...
            token.normalized =
                lowerAscii(text.substr(token.first, i - token.first));
            size_t probe = i;
            while (probe < text.size() && std::iswspace(text[probe])) ++probe;
            token.function = probe < text.size() && text[probe] == L'(';
            token.selectable =
                inspectorForName(token.normalized, token.inspector);
...
bool writeSelfTestResult(const std::wstring& path,
                         const std::vector<std::wstring>& failures) {
    if (path.empty()) return false;
    std::wstring report = failures.empty()
        ? L"PASS\r\n"
        : L"FAIL\r\n";
    ...
    int length = WideCharToMultiByte(
        CP_UTF8, 0, report.data(), static_cast<int>(report.size()),
    ...
    DWORD written = 0;
    BOOL okay = WriteFile(
        file, utf8.data(), static_cast<DWORD>(utf8.size()),
        &written, nullptr);
    CloseHandle(file);
    return okay && written == utf8.size();
}

HFONT createFont(int dpi, int dipHeight, int weight,
                 const wchar_t* face) {
    return CreateFontW(
        -std::max(1, MulDiv(dipHeight, dpi, 96)), 0, 0, 0, weight,
        FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
        CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY,
        DEFAULT_PITCH | FF_DONTCARE, face);
}

void drawSeparator(HDC dc, int x1, int y1, int x2, int y2,
                   COLORREF color) {
...
void drawCircle(HDC dc, int centerX, int centerY, int radius,
    ...
    Ellipse(dc, centerX - radius, centerY - radius,
...
struct FormulaEditorPanel::Impl {
    ...
    std::shared_ptr<AccessibilityGuard> accessibilityGuard =
        std::make_shared<AccessibilityGuard>();
    ...
    RECT pixelRect(int left, int top, int right, int bottom) const {
        return {scale(left), scale(top), scale(right), scale(bottom)};
    }

    RECT contentPixelRect(int left, int top,
                          int right, int bottom) const {
        int origin = HEADER_HEIGHT + BODY_PADDING -
                     scrollbar.position();
        return pixelRect(left, origin + top, right, origin + bottom);
    }

    RECT contentLocalRect(int left, int top,
                          int right, int bottom) const {
    ...
    std::complex<double>* selectedValue(FormulaDialogConfig& config) const {
        int parameter = parameterIndex(selected);
        if (parameter >= 0)
            return &config.parameters[static_cast<size_t>(parameter)];
        if (selected == InspectorValue::C) return &config.fixedC;
        if (selected == InspectorValue::Z0) return &config.fixedZ0;
        return nullptr;
    }

    const std::complex<double>* selectedValue(
        const FormulaDialogConfig& config) const {
        int parameter = parameterIndex(selected);
        if (parameter >= 0)
            return &config.parameters[static_cast<size_t>(parameter)];
        if (selected == InspectorValue::C) return &config.fixedC;
    ...
    std::wstring inspectorTitle() const {
        int parameter = parameterIndex(selected);
        if (parameter >= 0)
            return std::wstring(L"Parameter ") + inspectorName(selected);
    ...
    std::wstring inspectorDescription() const {
        if (parameterIndex(selected) >= 0)
            return L"Drag in the square or enter exact values";
        if (selected == InspectorValue::Z)
            return L"Computed orbit value; read-only";
        if (selected == InspectorValue::N)
            return L"Current iteration index; read-only";
        if (!selectedEditable(working))
            return L"Bound to each pixel in the selected plane";
    ...
    bool dirty() const {
        return !sameConfig(working, applied);
    }
    ...
    void notifyAccessibility(DWORD event, LONG key) {
        notifyFormulaEditorAccessibility(accessibility, event, key);
    }
    ...
    InspectorValue initialInspector() const {
        for (int i = 0; i < 8; ++i) {
            InspectorValue value = static_cast<InspectorValue>(
    ...
    void syncAllControls() {
        ...
        presetDropdown.setSelectedIndex(
            presetIndexForSource(working.source));
        syncing = false;
    ...
    void onFormulaEvent(ui::TextField::Event event,
        ...
        if (event == ui::TextField::Event::Changed && !syncing) {
            ...
            if (!narrowUtf8(source, utf8)) {
                formulaValid = false;
                setStatus(L"Formula contains text that is not valid UTF-8.",
                          true);
            } else {
                ...
                if (formulaValid) {
                    ...
                    std::wstring error;
                    if (!widenUtf8(formulaError.message, error))
    ...
    void onNumericEvent(int focus, ui::TextField::Event event,
                        const std::wstring& detail) {
        if (event == ui::TextField::Event::ClipboardError) {
            setStatus(detail, true);
            return;
        ...
        if (event == ui::TextField::Event::FocusGained) {
            focusedId = focus;
            presetDropdown.setFocused(false);
            LONG key = focus == FOCUS_BAILOUT
                ? FORMULA_ACC_BAILOUT
                : (focus == FOCUS_REAL
                    ? FORMULA_ACC_REAL
                    : FORMULA_ACC_IMAGINARY);
    ...
    bool createControls() {
        if (!formulaField.create(
                hwnd, ID_FORMULA_SERVICE,
                formula::ExpressionProgram::MAX_SOURCE,
                [this](ui::TextField::Event event,
                       const std::wstring& detail) {
                    onFormulaEvent(event, detail);
                })) {
            return false;
        }
        if (!bailoutField.create(
                hwnd, ID_BAILOUT_SERVICE, 79,
                [this](ui::TextField::Event event,
                       const std::wstring& detail) {
                    onNumericEvent(FOCUS_BAILOUT, event, detail);
                })) {
            return false;
        }
        if (!realField.create(
                hwnd, ID_REAL_SERVICE, 79,
                [this](ui::TextField::Event event,
                       const std::wstring& detail) {
                    onNumericEvent(FOCUS_REAL, event, detail);
                })) {
            return false;
        }
        if (!imaginaryField.create(
                hwnd, ID_IMAGINARY_SERVICE, 79,
                [this](ui::TextField::Event event,
                       const std::wstring& detail) {
                    onNumericEvent(FOCUS_IMAGINARY, event, detail);
                })) {
            return false;
        }
        ...
        for (const Preset& preset : presets())
            presetItems.emplace_back(preset.label);
        presetDropdown.setItems(std::move(presetItems));
        presetDropdown.setCallback(
            [this](int index) { choosePreset(index); });
        scrollbar.setCallback([this](int) {
            updateLayout();
            if (hwnd) InvalidateRect(hwnd, nullptr, FALSE);
            notifyAccessibility(EVENT_OBJECT_VALUECHANGE,
                                FORMULA_ACC_SCROLLBAR);
        });
        return true;
    }
    ...
    bool create(HWND newOwner, int newDpi,
                FormulaEditorCallbacks newCallbacks) {
        destroy();
    ...
    void addButton(RECT rect, int hit, const wchar_t* label,
                   ButtonKind kind, bool enabled, bool active,
                   bool content, RECT local = {}) {
        ButtonSpec button{rect, hit, label, kind, enabled, active, content};
        buttons.push_back(std::move(button));
        RECT hitRect = content
            ? intersected(rect, layout.contentClip) : rect;
        if (nonempty(hitRect)) hitRouter.add(hit, hitRect, enabled);
        focusEntries.push_back(
            {hit, rect, local, content, enabled});
    }

    void addFocus(int id, RECT rect, RECT local,
                  bool content, bool enabled) {
        focusEntries.push_back({id, rect, local, content, enabled});
    }
    ...
    void updateLayout() {
        ...
        layout.contentOriginDip =
            HEADER_HEIGHT + BODY_PADDING - scrollbar.position();
        ...
        scrollbar.setBounds(pixelRect(
            widthDip - 10, bodyTop, widthDip, bodyBottom));

        buttons.clear();
        focusEntries.clear();
        hitRouter.clear();

        addButton(pixelRect(widthDip - 46, 16, widthDip - 16, 46),
        ...
        int functionGap = 4;
        int functionWidth =
            (functionRight - functionLeft - functionGap * 3) / 4;
        const auto& labels =
            functionSets()[static_cast<size_t>(functionTab)];
        for (int i = 0; i < 16; ++i) {
            int row = i / 4;
            int column = i % 4;
            int left =
        ...
        addButton(layout.picker, HIT_PICKER, L"",
                  ButtonKind::Normal, true, false, true,
                  contentLocalRect(
                      pickerLeft, pickerTop,
                      pickerLeft + pickerSize, pickerTop + pickerSize));
        ...
        addButton(pixelRect(mandelbrotLeft, buttonTop,
                            mandelbrotRight, buttonBottom),
                  HIT_MANDELBROT, L"Mandelbrot", ButtonKind::Normal,
                  true, false, false);
        addButton(pixelRect(revertLeft, buttonTop,
                            revertRight, buttonBottom),
                  HIT_REVERT, L"Revert", ButtonKind::Normal,
                  true, false, false);
        addButton(pixelRect(applyLeft, buttonTop,
    ...
    RECT accessibleScreenRect(RECT rect, bool content,
                              bool& visible) const {
    ...
    void appendAccessible(
        std::vector<FormulaAccessibleItem>& items, LONG key,
        std::wstring name, LONG role, RECT rect, bool content,
        bool enabled, bool selectedState, bool focusedState,
        std::wstring value, std::wstring description,
        std::wstring defaultAction, bool forceInvisible = false) const {
        bool visible = false;
        RECT screen = accessibleScreenRect(rect, content, visible);
        LONG state = STATE_SYSTEM_FOCUSABLE;
        if (role == ROLE_SYSTEM_RADIOBUTTON ||
        ...
        items.push_back({
            key, std::move(name), std::move(value),
            std::move(description), std::move(defaultAction),
            role, state, screen
        });
    }

    void accessibleSnapshot(
        std::vector<FormulaAccessibleItem>& items) const {
        items.clear();
        ...
        bool panelHasFocus =
            keyboardFocus == hwnd ||
            (keyboardFocus && IsChild(hwnd, keyboardFocus));
        auto focused = [this, panelHasFocus](int id) {
            return panelHasFocus && focusedId == id;
        };

        appendAccessible(
            items, FORMULA_ACC_EXPRESSION, L"Formula expression",
            ROLE_SYSTEM_TEXT, layout.formulaField, true, true, false,
            focused(FOCUS_FORMULA), formulaField.text(),
            L"Orbit expression. Editing stages the formula until Apply.",
            L"Edit");
        int presetIndex = presetDropdown.selectedIndex();
        std::wstring presetValue =
            presetIndex >= 0 &&
            presetIndex < static_cast<int>(presets().size())
                ? presets()[static_cast<size_t>(presetIndex)].label
                : L"";
        appendAccessible(
            items, FORMULA_ACC_PRESET, L"Formula preset",
            ROLE_SYSTEM_COMBOBOX, layout.preset, true, true, false,
            focused(FOCUS_PRESET), std::move(presetValue),
            presetDropdown.open()
                ? L"Preset list expanded. Activate to accept the current item."
                : L"Preset list collapsed. Activate to open it.",
            presetDropdown.open() ? L"Accept" : L"Open");
        ...
        appendAccessible(
            items, FORMULA_ACC_COPY, L"Copy complete formula",
            ROLE_SYSTEM_PUSHBUTTON, buttonRect(HIT_COPY), true, true,
            false, focused(HIT_COPY), L"",
            L"Copies the entire formula expression.", L"Press");
        appendAccessible(
            items, FORMULA_ACC_PASTE, L"Paste complete formula",
            ROLE_SYSTEM_PUSHBUTTON, buttonRect(HIT_PASTE), true, true,
            false, focused(HIT_PASTE), L"",
            L"Replaces the entire formula from the clipboard.", L"Press");
        ...
        for (int i = 0; i < 12; ++i) {
            ...
            std::wstring description =
                L"Inserts variable " + label +
                L" and selects its value inspector.";
            appendAccessible(
                items, FORMULA_ACC_VARIABLE_BASE + i,
                L"Variable " + label, ROLE_SYSTEM_PUSHBUTTON,
                buttonRect(HIT_VARIABLE_BASE + i), true, true,
                selected == value, focused(HIT_VARIABLE_BASE + i),
                usageCount(value) == 0
                    ? L"not used"
                    : std::to_wstring(usageCount(value)) + L" uses",
                std::move(description), L"Insert");
        }
        ...
        for (int i = 0; i < 3; ++i) {
            appendAccessible(
                items, FORMULA_ACC_TAB_BASE + i,
                std::wstring(tabLabels[static_cast<size_t>(i)]) +
                    L" functions tab",
                ROLE_SYSTEM_PAGETAB,
                buttonRect(HIT_FUNCTION_TAB_BASE + i), true, true,
                functionTab == i,
                focused(HIT_FUNCTION_TAB_BASE + i), L"",
                L"Selects the visible function button set.", L"Select");
        }
        ...
        for (int i = 0; i < 16; ++i) {
            ...
            appendAccessible(
                items, FORMULA_ACC_FUNCTION_BASE + i,
                L"Function " + label, ROLE_SYSTEM_PUSHBUTTON,
                buttonRect(HIT_FUNCTION_BASE + i), true, true, false,
                focused(HIT_FUNCTION_BASE + i), L"",
                L"Inserts this function into the formula.", L"Insert");
        }

        bool cPlane =
            working.pixelParameter != FormulaParameter::InitialZ;
        appendAccessible(
            items, FORMULA_ACC_C_PLANE, L"c-plane",
            ROLE_SYSTEM_RADIOBUTTON, buttonRect(HIT_C_PLANE), true,
            true, cPlane, focused(HIT_C_PLANE), L"",
            L"Each pixel supplies c; z0 is a fixed editable value.",
            L"Select");
        appendAccessible(
            items, FORMULA_ACC_Z0_PLANE, L"z0-plane",
            ROLE_SYSTEM_RADIOBUTTON, buttonRect(HIT_Z0_PLANE), true,
            true, !cPlane, focused(HIT_Z0_PLANE), L"",
            L"Each pixel supplies z0; c is a fixed editable value.",
            L"Select");
        appendAccessible(
            items, FORMULA_ACC_BAILOUT, L"Bailout value",
            ROLE_SYSTEM_TEXT, layout.bailout, true, true, false,
            focused(FOCUS_BAILOUT), bailoutField.text(),
            L"Positive escape radius.", L"Edit");

        const std::complex<double>* complexValue = selectedValue(working);
        std::wstring pickerValue = complexValue
            ? formatNumber(complexValue->real()) + L", " +
                  formatNumber(complexValue->imag())
            : L"Read only";
        bool pickerEnabled = selectedEditable(working);
        appendAccessible(
            items, FORMULA_ACC_PICKER, L"Complex value picker",
            ROLE_SYSTEM_SLIDER, layout.picker, true, pickerEnabled,
            false, focused(HIT_PICKER), std::move(pickerValue),
            L"Complex plane picker. Arrow keys move by one fiftieth "
            L"of the current range.",
            L"Focus");
        appendAccessible(
            items, FORMULA_ACC_RANGE_OUT, L"Increase picker range",
            ROLE_SYSTEM_PUSHBUTTON, buttonRect(HIT_RANGE_OUT), true,
            true, false, focused(HIT_RANGE_OUT),
            formatNumber(pickerRange), L"Doubles the complex picker range.",
            L"Press");
        appendAccessible(
            items, FORMULA_ACC_RANGE_RESET, L"Reset picker range",
            ROLE_SYSTEM_PUSHBUTTON, buttonRect(HIT_RANGE_RESET), true,
            true, false, focused(HIT_RANGE_RESET),
            formatNumber(pickerRange), L"Resets the picker range to 2.",
            L"Press");
        appendAccessible(
            items, FORMULA_ACC_RANGE_IN, L"Decrease picker range",
            ROLE_SYSTEM_PUSHBUTTON, buttonRect(HIT_RANGE_IN), true,
            true, false, focused(HIT_RANGE_IN),
            formatNumber(pickerRange), L"Halves the complex picker range.",
            L"Press");
        appendAccessible(
            items, FORMULA_ACC_REAL, L"Real value",
            ROLE_SYSTEM_TEXT, layout.realField, true, realField.enabled(),
            false, focused(FOCUS_REAL), realField.text(),
            L"Real component of the selected fixed complex value.",
            L"Edit");
        appendAccessible(
            items, FORMULA_ACC_IMAGINARY, L"Imaginary value",
            ROLE_SYSTEM_TEXT, layout.imaginaryField, true,
            imaginaryField.enabled(), false, focused(FOCUS_IMAGINARY),
            imaginaryField.text(),
            L"Imaginary component of the selected fixed complex value.",
            L"Edit");

        wchar_t scrollValue[64]{};
        swprintf_s(
            scrollValue, L"%d of %d", scrollbar.position(),
            scrollbar.maximumPosition());
        appendAccessible(
            items, FORMULA_ACC_SCROLLBAR, L"Formula editor scrollbar",
            ROLE_SYSTEM_SCROLLBAR, scrollbar.bounds(), false,
            scrollbar.visible(), false, focused(FOCUS_SCROLLBAR),
            scrollValue,
        ...
        appendAccessible(
            items, FORMULA_ACC_MANDELBROT, L"Mandelbrot",
            ROLE_SYSTEM_PUSHBUTTON, buttonRect(HIT_MANDELBROT), false,
            true, false, focused(HIT_MANDELBROT), L"",
            L"Switches back to the built-in Mandelbrot formula.", L"Press");
        appendAccessible(
            items, FORMULA_ACC_REVERT, L"Revert",
            ROLE_SYSTEM_PUSHBUTTON, buttonRect(HIT_REVERT), false,
            true, false, focused(HIT_REVERT), L"",
            L"Restores the last applied formula configuration.", L"Press");
        appendAccessible(
            items, FORMULA_ACC_APPLY, L"Apply and render",
            ROLE_SYSTEM_PUSHBUTTON, buttonRect(HIT_APPLY), false,
            true, false, focused(HIT_APPLY), L"",
            L"Validates and applies staged changes, then renders.", L"Press");
        appendAccessible(
            items, FORMULA_ACC_CLOSE, L"Close formula editor",
            ROLE_SYSTEM_PUSHBUTTON, buttonRect(HIT_CLOSE), false,
            true, false, focused(HIT_CLOSE), L"",
            L"Closes the Formula editor dock.", L"Press");

        for (int i = 0; i < static_cast<int>(presets().size()); ++i) {
            RECT itemRect =
                presetDropdown.itemBounds(i, layout.contentClip);
            appendAccessible(
                items, FORMULA_ACC_PRESET_ITEM_BASE + i,
                std::wstring(L"Preset ") +
                    presets()[static_cast<size_t>(i)].label,
                ROLE_SYSTEM_LISTITEM, itemRect, false, true,
                presetIndex == i, false,
                presets()[static_cast<size_t>(i)].label,
    ...
    LONG accessibleKeyForFocus(int id) const {
        ...
        if (id >= HIT_VARIABLE_BASE &&
            ...
            return FORMULA_ACC_VARIABLE_BASE +
                   id - HIT_VARIABLE_BASE;
        }
        if (id >= HIT_FUNCTION_TAB_BASE &&
            id < HIT_FUNCTION_TAB_BASE + 3) {
            return FORMULA_ACC_TAB_BASE +
                   id - HIT_FUNCTION_TAB_BASE;
        }
        if (id >= HIT_FUNCTION_BASE &&
            id < HIT_FUNCTION_BASE + 16) {
            return FORMULA_ACC_FUNCTION_BASE +
                   id - HIT_FUNCTION_BASE;
        }
    ...
    void notifyDropdownTransition(bool wasOpen) {
        ...
        notifyAccessibility(
            isOpen ? EVENT_SYSTEM_MENUPOPUPSTART
    ...
    bool accessibilityFocusKey(LONG key, bool selectAction) {
        ...
        if (key >= FORMULA_ACC_PRESET_ITEM_BASE &&
            key < FORMULA_ACC_PRESET_ITEM_BASE +
                      static_cast<LONG>(presets().size())) {
            setFocused(FOCUS_PRESET);
            if (selectAction) {
                ...
                notifyAccessibility(EVENT_OBJECT_SELECTION,
                                    FORMULA_ACC_PRESET_ITEM_BASE + index);
                notifyAccessibility(EVENT_OBJECT_VALUECHANGE,
                                    FORMULA_ACC_PRESET);
            }
            return true;
        ...
        int focus = HIT_NONE;
        if (key == FORMULA_ACC_COPY) focus = HIT_COPY;
        else if (key == FORMULA_ACC_PASTE) focus = HIT_PASTE;
        else if (key == FORMULA_ACC_C_PLANE) focus = HIT_C_PLANE;
        else if (key == FORMULA_ACC_Z0_PLANE) focus = HIT_Z0_PLANE;
        else if (key == FORMULA_ACC_PICKER) {
            if (!selectedEditable(working)) return false;
            focus = HIT_PICKER;
        } else if (key == FORMULA_ACC_RANGE_OUT) focus = HIT_RANGE_OUT;
        else if (key == FORMULA_ACC_RANGE_RESET) focus = HIT_RANGE_RESET;
        else if (key == FORMULA_ACC_RANGE_IN) focus = HIT_RANGE_IN;
        else if (key == FORMULA_ACC_MANDELBROT) focus = HIT_MANDELBROT;
        else if (key == FORMULA_ACC_REVERT) focus = HIT_REVERT;
        else if (key == FORMULA_ACC_APPLY) focus = HIT_APPLY;
        else if (key == FORMULA_ACC_CLOSE) focus = HIT_CLOSE;
        else if (key >= FORMULA_ACC_VARIABLE_BASE &&
                 key < FORMULA_ACC_VARIABLE_BASE + 12) {
            focus = HIT_VARIABLE_BASE +
                    static_cast<int>(key - FORMULA_ACC_VARIABLE_BASE);
            if (selectAction) {
                selectInspector(variableFromButton(
                    static_cast<size_t>(
                        key - FORMULA_ACC_VARIABLE_BASE)));
            }
        } else if (key >= FORMULA_ACC_TAB_BASE &&
                   key < FORMULA_ACC_TAB_BASE + 3) {
            focus = HIT_FUNCTION_TAB_BASE +
                    static_cast<int>(key - FORMULA_ACC_TAB_BASE);
            if (selectAction) {
                int oldTab = functionTab;
                functionTab = static_cast<int>(
                    key - FORMULA_ACC_TAB_BASE);
                updateLayout();
                InvalidateRect(hwnd, nullptr, FALSE);
                notifyAccessibility(EVENT_OBJECT_SELECTION,
                                    FORMULA_ACC_TAB_BASE + functionTab);
                if (oldTab != functionTab)
                    notifyAccessibility(EVENT_OBJECT_REORDER,
                                        FORMULA_ACC_SELF);
            }

        } else if (key >= FORMULA_ACC_FUNCTION_BASE &&
                   key < FORMULA_ACC_FUNCTION_BASE + 16) {
            focus = HIT_FUNCTION_BASE +
                    static_cast<int>(key - FORMULA_ACC_FUNCTION_BASE);
        }
    ...
    bool accessibilityDefaultAction(LONG key) {
        if (key == FORMULA_ACC_EXPRESSION ||
            key == FORMULA_ACC_BAILOUT ||
            key == FORMULA_ACC_REAL ||
            key == FORMULA_ACC_IMAGINARY) {
            return accessibilityFocusKey(key, true);
        }
        if (key == FORMULA_ACC_PRESET) {
            setFocused(FOCUS_PRESET);
            bool wasOpen = presetDropdown.open();
            presetDropdown.keyDown(
                VK_RETURN, false, layout.contentClip);
            notifyDropdownTransition(wasOpen);
            InvalidateRect(hwnd, nullptr, FALSE);
            return true;
        }
        ...
        if (key == FORMULA_ACC_PICKER)
            return accessibilityFocusKey(key, false);
        if (key == FORMULA_ACC_SCROLLBAR) {
            if (!scrollbar.visible()) return false;
            int page = std::max(
                48, clientHeightDip() - HEADER_HEIGHT -
                        FOOTER_HEIGHT - BODY_PADDING * 2);
            scrollbar.scrollBy(page);
            return true;
        }

        int hit = HIT_NONE;
        if (key == FORMULA_ACC_COPY) hit = HIT_COPY;
        else if (key == FORMULA_ACC_PASTE) hit = HIT_PASTE;
        else if (key == FORMULA_ACC_C_PLANE) hit = HIT_C_PLANE;
        else if (key == FORMULA_ACC_Z0_PLANE) hit = HIT_Z0_PLANE;
        else if (key == FORMULA_ACC_RANGE_OUT) hit = HIT_RANGE_OUT;
        else if (key == FORMULA_ACC_RANGE_RESET) hit = HIT_RANGE_RESET;
        else if (key == FORMULA_ACC_RANGE_IN) hit = HIT_RANGE_IN;
        else if (key == FORMULA_ACC_MANDELBROT) hit = HIT_MANDELBROT;
        else if (key == FORMULA_ACC_REVERT) hit = HIT_REVERT;
        else if (key == FORMULA_ACC_APPLY) hit = HIT_APPLY;
        else if (key == FORMULA_ACC_CLOSE) hit = HIT_CLOSE;
        else if (key >= FORMULA_ACC_VARIABLE_BASE &&
                 key < FORMULA_ACC_VARIABLE_BASE + 12) {
            hit = HIT_VARIABLE_BASE +
                  static_cast<int>(key - FORMULA_ACC_VARIABLE_BASE);
        } else if (key >= FORMULA_ACC_TAB_BASE &&
                   key < FORMULA_ACC_TAB_BASE + 3) {
            hit = HIT_FUNCTION_TAB_BASE +
                  static_cast<int>(key - FORMULA_ACC_TAB_BASE);
        } else if (key >= FORMULA_ACC_FUNCTION_BASE &&
                   key < FORMULA_ACC_FUNCTION_BASE + 16) {
            hit = HIT_FUNCTION_BASE +
                  static_cast<int>(key - FORMULA_ACC_FUNCTION_BASE);
        }
    ...
    bool performAccessibilityAction(
        LONG key, FormulaAccessibilityAction action) {
        if (action == FormulaAccessibilityAction::Default)
            return accessibilityDefaultAction(key);
        return accessibilityFocusKey(
            key, action == FormulaAccessibilityAction::Select);
    }

    void ensureContentVisible(RECT contentRect) {
        int viewport = std::max(
            1, clientHeightDip() - HEADER_HEIGHT -
               FOOTER_HEIGHT - BODY_PADDING * 2);
    ...
    void focusNext(bool reverse) {
        ...
        for (int step = 1; step <= count; ++step) {
            int index = current < 0
                ? (reverse ? count - 1 : 0)
                : (current + (reverse ? -step : step)) % count;
    ...
    const Token* tokenAt(size_t position) const {
        for (const Token& token : tokens) {
            if (position >= token.first && position < token.last)
                return &token;
        }
        if (position > 0) {
            for (const Token& token : tokens) {
                if (position == token.last &&
    ...
    void selectInspectorAtCaret(bool selectToken) {
        ...
        if (selectToken)
            formulaField.setSelection(token->first, token->last);
    }
    ...
    bool insertFormulaText(const std::wstring& token, bool function) {
        ...
        if (function) {
            if (last > first) {
                insertion = token + L"(" +
                    current.substr(first, last - first) + L")";
        ...
        if (current.size() - (last - first) + insertion.size() >
            formula::ExpressionProgram::MAX_SOURCE) {
    ...
    void choosePreset(int index) {
        ...
        syncing = false;
        refreshFormulaAnalysis();
        if (selected == InspectorValue::P0 && preset.setP0)
            syncInspectorFields();
        setStatus(std::wstring(preset.label) +
                  L" preset loaded; changes are staged.", false);
        notifyAccessibility(EVENT_OBJECT_SELECTION,
                            FORMULA_ACC_PRESET_ITEM_BASE + index);
        notifyAccessibility(EVENT_OBJECT_VALUECHANGE,
                            FORMULA_ACC_PRESET);
    ...
    void copyCompleteFormula() {
        if (!writeClipboard(hwnd, formulaField.text())) {
            setStatus(L"Copy failed because the clipboard is unavailable.",
                      true);
            return;
        }
    ...
    void pasteCompleteFormula() {
        ...
        if (!readClipboard(hwnd, clipboard)) {
            setStatus(L"Paste failed because the clipboard has no readable text.",
                      true);
            return;
        }
        if (clipboard.size() > formula::ExpressionProgram::MAX_SOURCE) {
            setStatus(L"Paste failed: the formula is longer than 4096 characters.",
                      true);
            return;
        }
    ...
    void choosePlane(FormulaParameter plane) {
        ...
        updateLayout();
        setStatus(
            plane == FormulaParameter::InitialZ
                ? L"z0 plane selected; fixed c is editable."
                : L"c plane selected; fixed z0 is editable.",
            false);
        notifyAccessibility(
            EVENT_OBJECT_SELECTION,
            plane == FormulaParameter::InitialZ
                ? FORMULA_ACC_Z0_PLANE
                : FORMULA_ACC_C_PLANE);
        notifyAccessibility(EVENT_OBJECT_VALUECHANGE,
                            FORMULA_ACC_REAL);
        notifyAccessibility(EVENT_OBJECT_VALUECHANGE,
    ...
    void changeRange(double factor) {
        pickerRange =
            std::clamp(pickerRange * factor, 1.0e-12, 1.0e12);
        if (hwnd) InvalidateRect(hwnd, nullptr, FALSE);
        notifyAccessibility(EVENT_OBJECT_VALUECHANGE,
                            FORMULA_ACC_PICKER);
    }
    ...
    void updatePickerFromMouse(POINT point) {
        ...
        double x = std::clamp(
            (point.x - rect.left) / width, 0.0, 1.0);
        double y = std::clamp(
            (point.y - rect.top) / height, 0.0, 1.0);
    ...
    bool validateComplexes(FormulaDialogConfig& candidate) {
        std::array<std::pair<std::complex<double>*, InspectorValue>, 9>
            values{};
        values[0] =
            candidate.pixelParameter == FormulaParameter::InitialZ
                ? std::make_pair(&candidate.fixedC, InspectorValue::C)
                : std::make_pair(&candidate.fixedZ0, InspectorValue::Z0);
        for (int i = 0; i < 8; ++i) {
        ...
        for (const auto& entry : values) {
            ...
            syncInspectorFields();
            setStatus(std::wstring(inspectorName(selected)) +
                      L" must contain finite values.", true);
    ...
    bool readCandidate(FormulaDialogConfig& candidate) {
        ...
        double bailout = 0.0;
        if (!parseFiniteNumber(bailoutField.text(), bailout) ||
            !(bailout > 0.0)) {
        ...
        if (!program.compile(candidate.source, &error)) {
            std::wstring detail;
            if (!widenUtf8(error.message, detail))
                detail = L"Invalid formula";
            wchar_t prefix[96]{};
            swprintf_s(prefix, L"Formula error at character %zu: ",
                       error.position + 1u);
            setStatus(std::wstring(prefix) + detail, true);
            size_t caret =
                wideIndexForUtf8Offset(candidate.source, error.position);
    ...
    void applyChanges() {
        ...
        if (!accepted) {
            setStatus(L"The formula was not accepted; staged edits were kept.",
                      true);
            return;
        }
        working = candidate;
    ...
    void invokeHit(int hit) {
        ...
        if (hit >= HIT_FUNCTION_TAB_BASE &&
            hit < HIT_FUNCTION_TAB_BASE + 3) {
        ...
        if (hit >= HIT_FUNCTION_BASE &&
            hit < HIT_FUNCTION_BASE + 16) {
            size_t index =
                static_cast<size_t>(hit - HIT_FUNCTION_BASE);
            const wchar_t* label =
                functionSets()[static_cast<size_t>(functionTab)][index];
    ...
    bool runSelfTest() {
        ...
        auto check = [&failures](bool condition,
                                 const wchar_t* name) {
        ...
        HWND formulaBackend =
            GetDlgItem(hwnd, ID_FORMULA_SERVICE);
        check(formulaBackend != nullptr,
              L"text backend exists");
        ...
        std::wstring clipboard;
        check(readClipboard(hwnd, clipboard) &&
                  clipboard == L"b",
              L"selection copy");
        SendMessageW(formulaBackend, WM_CUT, 0, 0);
        check(formulaField.text() == L"ac",
              L"selection cut");
        check(formulaField.undo() &&
                  formulaField.text() == L"abc",
              L"text undo");
        check(writeClipboard(hwnd, L"X"),
              L"clipboard setup");
        formulaField.setSelection(1, 2);
        SendMessageW(formulaBackend, WM_PASTE, 0, 0);
        check(formulaField.text() == L"aXc",
              L"selection paste");
        ...
        check(readClipboard(hwnd, clipboard) &&
                  clipboard == L"z*z+c",
              L"complete copy");
        check(writeClipboard(hwnd, L"sin(z)+c"),
              L"complete paste setup");
        pasteCompleteFormula();
        check(formulaField.text() == L"sin(z)+c",
              L"complete paste");
        ...
        HWND bailoutBackend =
            GetDlgItem(hwnd, ID_BAILOUT_SERVICE);
        SendMessageW(bailoutBackend, WM_CHAR, L'2', 1);
        check(bailoutField.text().size() == maximum.size(),
        ...
        refreshFormulaAnalysis();
        presetDropdown.setSelectedIndex(0);
        check(presetDropdown.keyDown(
                  VK_SPACE, false, layout.contentClip) &&
                  presetDropdown.open(),
              L"dropdown keyboard open");
        presetDropdown.keyDown(
            VK_DOWN, false, layout.contentClip);
        presetDropdown.keyDown(
            VK_RETURN, false, layout.contentClip);
        check(presetDropdown.selectedIndex() == 1 &&
                  working.source == "z*z+c",
              L"dropdown keyboard selection");
        presetDropdown.keyDown(
            VK_SPACE, false, layout.contentClip);
        presetDropdown.mouseDown(
            {layout.body.left + 2, layout.body.bottom - 2},
            layout.contentClip);
        check(!presetDropdown.open(),
              L"dropdown outside dismissal");
        presetDropdown.close();
        POINT presetCenter{
            (layout.preset.left + layout.preset.right) / 2,
            (layout.preset.top + layout.preset.bottom) / 2
        };
        onLeftButtonDown(presetCenter);
        check(presetDropdown.open() && GetCapture() != hwnd,
              L"dropdown opens without mouse capture");
        dismissPopups();
        check(!presetDropdown.open(),
              L"parent-driven popup dismissal");
        ...
        HDC memoryDc = windowDc
            ? CreateCompatibleDC(windowDc) : nullptr;
        HBITMAP bitmap = windowDc
            ? CreateCompatibleBitmap(
                  windowDc, std::max(1L, layout.client.right),
                  std::max(1L, layout.client.bottom))
            : nullptr;
        HGDIOBJ oldBitmap =
            memoryDc && bitmap ? SelectObject(memoryDc, bitmap) : nullptr;
        if (memoryDc && bitmap) paintTo(memoryDc);
        check(memoryDc && bitmap,
              L"offscreen validation surface");

        if (memoryDc && bitmap) {
            HGDIOBJ oldFont =
                SelectObject(memoryDc, formulaFont);
            SIZE prefix{};
            SIZE tokenSize{};
            GetTextExtentPoint32W(
                memoryDc, L"z+", 2, &prefix);
            GetTextExtentPoint32W(
                memoryDc, L"p0", 2, &tokenSize);
            SelectObject(memoryDc, oldFont);
            POINT tokenPoint{
                layout.formulaField.left + prefix.cx +
                    tokenSize.cx / 2,
                (layout.formulaField.top +
                 layout.formulaField.bottom) / 2
            };
        ...
        check(insertFormulaText(L"p0", false) &&
                  formulaField.text() == L"p0+c",
              L"variable insertion");
        formulaField.setText(L"z+");
        working.source = "z+";
        formulaField.setSelection(2, 2);
        check(insertFormulaText(L"sin", true) &&
                  formulaField.text() == L"z+sin()" &&
                  formulaField.selection().first == 6,
              L"function insertion caret");
        formulaField.setText(L"z+c");
        working.source = "z+c";
        formulaField.setSelection(0, 3);
        check(insertFormulaText(L"sqr", true) &&
                  formulaField.text() == L"sqr(z+c)",
              L"function selection wrapping");
        ...
        POINT pickerPoint{
            layout.picker.left +
                (layout.picker.right - layout.picker.left) * 3 / 4,
            layout.picker.top +
                (layout.picker.bottom - layout.picker.top) / 4
        };
        updatePickerFromMouse(pickerPoint);
        check(std::abs(working.parameters[0].real() - 1.0) < 0.04 &&
                  std::abs(working.parameters[0].imag() - 1.0) < 0.04,
              L"picker synchronization");
        realField.setText(L"0.25");
        imaginaryField.setText(L"-0.5");
        check(commitInspector(true) &&
                  working.parameters[0] ==
                      std::complex<double>(0.25, -0.5),
              L"manual complex synchronization");

        working.pixelParameter = FormulaParameter::C;
        selected = InspectorValue::C;
        syncInspectorFields();
        check(!selectedEditable(working),
              L"pixel-bound value read-only");
        choosePlane(FormulaParameter::InitialZ);
        check(selectedEditable(working),
              L"fixed plane value editable");
        ...
        callbacks.apply =
            [&applyCalled, &callbackCandidate](
                const FormulaDialogConfig& candidate) {
                applyCalled = true;
                callbackCandidate = candidate;
                return true;
            };
        callbacks.useMandelbrot =
            [&mandelbrotCalled]() {
                mandelbrotCalled = true;
            };
        callbacks.close =
            [&closeCalled]() {
                closeCalled = true;
        ...
        applyChanges();
        check(applyCalled &&
                  callbackCandidate.source == "z*z+c" &&
                  applied.source == "z*z+c",
              L"transactional apply");
        working.source = "z*z*z+c";
        formulaField.setText(L"z*z*z+c");
        revertChanges();
        check(working.source == "z*z+c" &&
                  formulaField.text() == L"z*z+c",
              L"transactional revert");
        useMandelbrot();
        check(mandelbrotCalled,
              L"Mandelbrot callback");
        ...
        check(!applyCalled && !formulaValid &&
                  errorSelection.first == 2 &&
                  errorSelection.second == 3,
              L"parse error focus selection");

        ShowWindow(hwnd, SW_SHOW);
        closePanel();
        check(closeCalled &&
                  IsWindowVisible(hwnd) == FALSE,
              L"transactional close");
        ShowWindow(hwnd, SW_SHOW);

        SetWindowPos(
            hwnd, nullptr, 0, 0,
            scale(FormulaEditorPanel::DESIGN_WIDTH), scale(640),
            SWP_NOMOVE | SWP_NOZORDER | SWP_NOACTIVATE);
        updateLayout();
        check(scrollbar.visible() &&
                  layout.header.bottom == scale(HEADER_HEIGHT) &&
                  layout.footer.bottom == scale(640),
        ...
        for (int testDpi : {96, 144, 192}) {
            ...
            updateLayout();
            int pickerWidth =
                layout.picker.right - layout.picker.left;
            int pickerHeight =
                layout.picker.bottom - layout.picker.top;
            check(std::abs(clientWidthDip() -
                      ...
                      !scrollbar.visible(),
                  testDpi == 96
                      ? L"96 DPI deterministic geometry"
                      : (testDpi == 144
                          ? L"144 DPI deterministic geometry"
                          : L"192 DPI deterministic geometry"));
        }
        setDpi(savedDpi);
        SetWindowPos(
            hwnd, nullptr, 0, 0,
        ...
        EnumChildWindows(
            ...
            [](HWND child, LPARAM parameter) -> BOOL {
                auto* result =
                    reinterpret_cast<NativeControlCheck*>(parameter);
        ...
        check(nativeCheck.okay && nativeCheck.edits == 4,
              L"no visible native control chrome");

        IAccessible* accessible = nullptr;
        HRESULT accessibleResult = AccessibleObjectFromWindow(
            hwnd, OBJID_CLIENT, IID_IAccessible,
            reinterpret_cast<void**>(&accessible));
        check(SUCCEEDED(accessibleResult) && accessible,
              L"OBJID_CLIENT accessibility provider");
        LONG accessibleChildren = 0;
        if (accessible) {
            check(SUCCEEDED(accessible->get_accChildCount(
                      &accessibleChildren)) &&
                      accessibleChildren >= 50,
                  L"accessible child inventory");
        }
        auto findAccessibleChild =
            [accessible, accessibleChildren](
                const wchar_t* expectedName) -> LONG {
                if (!accessible) return 0;
                for (LONG childId = 1;
                     childId <= accessibleChildren; ++childId) {
                    VARIANT child{};
                    child.vt = VT_I4;
                    child.lVal = childId;
                    BSTR name = nullptr;
                    if (SUCCEEDED(accessible->get_accName(
                            child, &name)) &&
                        name && std::wcscmp(name, expectedName) == 0) {
                        SysFreeString(name);
                        return childId;
                    }
        ...
        LONG expressionChild =
            findAccessibleChild(L"Formula expression");
        LONG presetChild =
            findAccessibleChild(L"Formula preset");
        LONG bailoutChild =
            findAccessibleChild(L"Bailout value");
        LONG pickerChild =
            findAccessibleChild(L"Complex value picker");
        LONG variableChild =
            findAccessibleChild(L"Variable z");
        std::wstring accessibleFunctionName =
            std::wstring(L"Function ") +
            functionSets()[static_cast<size_t>(functionTab)][0];
        LONG functionChild =
            findAccessibleChild(accessibleFunctionName.c_str());
        LONG scrollbarChild =
            findAccessibleChild(L"Formula editor scrollbar");
        LONG applyChild =
            findAccessibleChild(L"Apply and render");
        LONG closeChild =
            findAccessibleChild(L"Close formula editor");
        LONG presetItemChild =
            findAccessibleChild(L"Preset Custom");
        check(expressionChild && presetChild && bailoutChild &&
                  pickerChild && variableChild && functionChild &&
                  scrollbarChild && applyChild && closeChild &&
                  presetItemChild,
              L"accessible key control names");

        auto checkAccessibleRoleState =
            [&check, accessible](
                LONG childId, LONG expectedRole,
                LONG requiredState, const wchar_t* failure) {
                ...
                VARIANT state{};
                bool okay =
                    SUCCEEDED(accessible->get_accRole(child, &role)) &&
                    role.vt == VT_I4 &&
                    role.lVal == expectedRole &&
                    SUCCEEDED(accessible->get_accState(child, &state)) &&
                    state.vt == VT_I4 &&
                    (state.lVal & requiredState) == requiredState;
                VariantClear(&role);
                VariantClear(&state);
                check(okay, failure);
            };
        checkAccessibleRoleState(
            expressionChild, ROLE_SYSTEM_TEXT,
            STATE_SYSTEM_FOCUSABLE,
            L"accessible formula role and state");
        checkAccessibleRoleState(
            presetChild, ROLE_SYSTEM_COMBOBOX,
            STATE_SYSTEM_FOCUSABLE | STATE_SYSTEM_COLLAPSED,
            L"accessible preset role and state");
        checkAccessibleRoleState(
            applyChild, ROLE_SYSTEM_PUSHBUTTON,
            STATE_SYSTEM_FOCUSABLE,
            L"accessible action role and state");

        if (accessible) {
            VARIANT selection{};
            bool multipleSelection = false;
            if (SUCCEEDED(accessible->get_accSelection(&selection)) &&
                selection.vt == VT_UNKNOWN && selection.punkVal) {
                IEnumVARIANT* enumerator = nullptr;
                if (SUCCEEDED(selection.punkVal->QueryInterface(
                        IID_IEnumVARIANT,
                    ...
                    multipleSelection =
                        (next == S_OK || next == S_FALSE) && fetched >= 3;
            ...
            check(multipleSelection,
                  L"accessible multiple selection enumeration");
            ...
            bool locationOkay = SUCCEEDED(accessible->accLocation(
                &left, &top, &width, &height, child));
            RECT expected =
                presetDropdown.itemBounds(0, layout.contentClip);
            ...
            locationOkay = locationOkay &&
                left == expectedTop.x && top == expectedTop.y &&
                width == expectedBottom.x - expectedTop.x &&
                height == expectedBottom.y - expectedTop.y;
            check(locationOkay,
                  L"accessible DPI-scaled preset item location");
            presetDropdown.close();
        }
        ...
        if (accessible && expressionChild) {
            ...
            check(SUCCEEDED(accessible->accNavigate(
                      NAVDIR_FIRSTCHILD, self, &first)) &&
                      first.vt == VT_I4 &&
                      first.lVal == expressionChild &&
            ...
            check(SUCCEEDED(accessible->accSelect(
                      SELFLAG_TAKEFOCUS, child)) &&
                      formulaField.focused(),
                  L"accessible formula focus action");
            BSTR value = nullptr;
            check(SUCCEEDED(accessible->get_accValue(
                      child, &value)) && value,
                  L"accessible formula value");
        ...
        if (accessible && scrollbarChild) {
            ...
            VARIANT state{};
            check(SUCCEEDED(accessible->get_accState(child, &state)) &&
                      state.vt == VT_I4 &&
                      (state.lVal & STATE_SYSTEM_INVISIBLE) == 0,
                  L"accessible visible custom scrollbar");
            VariantClear(&state);
        }
        SetWindowPos(
            hwnd, nullptr, 0, 0,
        ...
        DWORD detachedGdiBefore = GetGuiResources(
            GetCurrentProcess(), GR_GDIOBJECTS);
        DWORD detachedUserBefore = GetGuiResources(
            GetCurrentProcess(), GR_USEROBJECTS);
        bool detachOkay = true;
        for (int i = 0; i < 32; ++i) {
            FormulaEditorAccessibilityProvider* temporary =
                createFormulaEditorAccessibility(hwnd);
            IAccessible* disconnected =
                acquireFormulaEditorAccessibility(temporary);
            detachFormulaEditorAccessibility(temporary);
            LONG count = -1;
            if (!disconnected ||
                disconnected->get_accChildCount(&count) !=
        ...
        DWORD detachedGdiAfter = GetGuiResources(
            GetCurrentProcess(), GR_GDIOBJECTS);
        DWORD detachedUserAfter = GetGuiResources(
            GetCurrentProcess(), GR_USEROBJECTS);
        check(detachOkay &&
                  (detachedGdiBefore == 0 ||
                   detachedGdiAfter <= detachedGdiBefore + 1) &&
                  (detachedUserBefore == 0 ||
                   detachedUserAfter <= detachedUserBefore + 1),
              L"accessible provider detach resource stability");
        if (accessible) accessible->Release();

        if (memoryDc && bitmap) {
            DWORD before = GetGuiResources(
                GetCurrentProcess(), GR_GDIOBJECTS);
            for (int i = 0; i < 96; ++i) paintTo(memoryDc);
            DWORD after = GetGuiResources(
                GetCurrentProcess(), GR_GDIOBJECTS);
            check(before == 0 || after <= before + 1,
                  L"GDI resource stability");
        }
        ...
        SetWindowPos(
            hwnd, nullptr, 0, 0,
            savedClient.right - savedClient.left,
            savedClient.bottom - savedClient.top,
            SWP_NOMOVE | SWP_NOZORDER | SWP_NOACTIVATE);
        syncAllControls();
        ...
        std::wstring resultPath = environmentValue(
            L"MANDEL_FORMULA_EDITOR_SELFTEST_RESULT");
        if (!writeSelfTestResult(resultPath, failures)) {
            writeSelfTestResult(
                L"build\\formula_editor_selftest.txt", failures);
        }
        if (failures.empty()) {
            OutputDebugStringW(
                L"Formula editor self-test: PASS\n");
        } else {
            OutputDebugStringW(
                L"Formula editor self-test: FAIL\n");
            for (const std::wstring& failure : failures) {
    ...
    void runSelfTestIfRequested() {
        if (selfTestRan ||
            environmentValue(
                L"MANDEL_FORMULA_EDITOR_SELFTEST").empty()) {
            return;
        }
        selfTestRan = true;
        runSelfTest();
        if (!environmentValue(
                L"MANDEL_FORMULA_EDITOR_SELFTEST_EXIT").empty()) {
    ...
    void drawHeader(HDC dc) {
        ...
        drawSeparator(dc, layout.header.left, layout.header.bottom - 1,
                      layout.header.right, layout.header.bottom - 1,
                      SOFT_BORDER);
        ...
        drawText(dc, subtitle,
                 L"Build the orbit, then bind values visually",
                 CLR_TEXT_DIM, resources.small(),
                 DT_LEFT | DT_VCENTER | DT_SINGLELINE);

        int widthDip = clientWidthDip();
        RECT badge = pixelRect(widthDip - 97, 20,
                               widthDip - 56, 43);
        fillRound(dc, badge, RGB(24, 47, 43),
                  RGB(45, 99, 80), scale(11));
        drawText(dc, badge, L"DOCKED", CLR_GREEN, tinyFont,
    ...
    void drawFormulaSection(HDC dc) {
        ...
        drawSeparator(dc, layout.formulaMeta.left,
                      layout.formulaMeta.top,
                      layout.formulaMeta.right,
                      layout.formulaMeta.top,
                      RGB(37, 42, 53));
        int dotX = contentPixelRect(left + 14, 0,
                                    left + 14, 0).left;
        int centerY =
            (layout.formulaMeta.top + layout.formulaMeta.bottom) / 2;
        drawCircle(dc, dotX, centerY, scale(2),
                   formulaValid ? CLR_GREEN : DANGER,
                   formulaValid ? CLR_GREEN : DANGER);

        std::wstring meta;
        if (formulaValid) {
            wchar_t buffer[96]{};
            swprintf_s(buffer, L"Valid formula \u00b7 %zu operations",
                       formulaProgram.instructionCount());
            meta = buffer;
        } else {
            std::wstring error;
            if (!widenUtf8(formulaError.message, error))
                error = L"Invalid formula";
            meta = error;
        }
        RECT metaText = layout.formulaMeta;
        metaText.left += scale(22);
        metaText.right -= scale(128);
        drawText(dc, metaText, meta,
                 formulaValid ? CLR_TEXT_DIM : DANGER,
                 tinyFont,
                 DT_LEFT | DT_VCENTER | DT_SINGLELINE |
                     DT_END_ELLIPSIS);
        ...
        std::wstring badge;
        if (formulaValid &&
            formulaProgram.fastPath() ==
                formula::ExpressionProgram::FastPath::
                    IntegerPowerPlusC) {
            wchar_t buffer[48]{};
            swprintf_s(buffer, L"FAST z^%d+c",
                       formulaProgram.fastIntegerPower());
            badge = buffer;
        } else if (formulaValid &&
                   formulaProgram.avx2Compatible()) {
            badge = L"AVX2 READY";
        } else if (formulaValid) {
            badge = L"SCALAR";
        }
        ...
        drawText(dc, badgeRect, badge, TOKEN_VARIABLE, tinyFont,
                 DT_RIGHT | DT_VCENTER | DT_SINGLELINE);
    }

    static void drawCardWithColor(HDC dc, RECT rect, COLORREF fill,
                                  COLORREF border, int radius) {
        fillRound(dc, rect, fill, border, radius);
    }
    ...
    void drawLeftColumn(HDC dc) {
        drawCardWithColor(dc, layout.variableCard, VALUE_CARD,
                          SOFT_BORDER, scale(9));
        drawSectionTitle(
            dc, contentPixelRect(
    ...
    void drawPicker(HDC dc) {
        RECT rect = layout.picker;
        drawCardWithColor(dc, rect, RGB(17, 21, 28),
                          RGB(83, 96, 118), scale(7));
        ...
        if (grid && axis) {
            ...
            for (int i = 1; i < 4; ++i) {
                int x = rect.left +
                    MulDiv(rect.right - rect.left, i, 4);
                int y = rect.top +
                    MulDiv(rect.bottom - rect.top, i, 4);
        ...
        int centerX = (rect.left + rect.right) / 2;
        int centerY = (rect.top + rect.bottom) / 2;
        drawCircle(dc, centerX, centerY, scale(2),
                   RGB(107, 115, 130), RGB(107, 115, 130));

        const std::complex<double>* value = selectedValue(working);
        std::complex<double> point = value ? *value :
            std::complex<double>{};
        if (finiteComplex(point)) {
            double normalizedX =
                point.real() / pickerRange * 0.5 + 0.5;
            double normalizedY =
                0.5 - point.imag() / pickerRange * 0.5;
            normalizedX = std::clamp(normalizedX, 0.0, 1.0);
            normalizedY = std::clamp(normalizedY, 0.0, 1.0);
            int x = rect.left + static_cast<int>(
                normalizedX * (rect.right - rect.left - 1));
            int y = rect.top + static_cast<int>(
                normalizedY * (rect.bottom - rect.top - 1));
            drawCircle(dc, x, y, scale(10),
                       RGB(40, 59, 92), RGB(40, 59, 92));
            drawCircle(dc, x, y, scale(6),
                       selectedEditable(working)
                           ? CLR_ACCENT : CLR_TEXT_DIM,
                       RGB(255, 255, 255));
        }
        ...
        if (!selectedEditable(working)) {
            ...
            fillRound(dc, overlay, RGB(24, 27, 35),
                      CLR_BORDER, scale(6));
            drawText(dc, overlay, L"Read-only", CLR_TEXT_DIM,
                     resources.small(),
                     DT_CENTER | DT_VCENTER | DT_SINGLELINE);
        }
    ...
    void drawValueColumn(HDC dc) {
        drawCardWithColor(dc, layout.valueCard, VALUE_CARD,
                          SOFT_BORDER, scale(9));
        ...
        RECT title = contentPixelRect(
            rightLeft + 56, 164, contentRight - 68, 183);
        drawText(dc, title, inspectorTitle(), CLR_TEXT, chipFont,
                 DT_LEFT | DT_VCENTER | DT_SINGLELINE |
                     DT_END_ELLIPSIS);
        RECT description = contentPixelRect(
            rightLeft + 56, 181, contentRight - 12, 202);
        drawText(dc, description, inspectorDescription(),
                 CLR_TEXT_DIM, tinyFont,
                 DT_LEFT | DT_VCENTER | DT_SINGLELINE |
                     DT_END_ELLIPSIS);

        RECT typeBadge = contentPixelRect(
            contentRight - 68, 166, contentRight - 13, 187);
        fillRound(dc, typeBadge, RGB(35, 48, 73),
                  RGB(35, 48, 73), scale(8));
        drawText(dc, typeBadge,
                 selectedEditable(working) ? L"COMPLEX" : L"READ ONLY",
                 RGB(169, 197, 251), tinyFont,
                 DT_CENTER | DT_VCENTER | DT_SINGLELINE);
        ...
        yAxis.left -= scale(22);
        drawText(dc, yAxis, L"Im", CLR_TEXT_DIM, tinyFont,
                 DT_CENTER | DT_VCENTER | DT_SINGLELINE);
        RECT xAxis{
            layout.picker.left,
            layout.picker.bottom,
            layout.picker.right,
            layout.picker.bottom + scale(14)
        };
        drawText(dc, xAxis, L"Re", CLR_TEXT_DIM, tinyFont,
                 DT_CENTER | DT_VCENTER | DT_SINGLELINE);
    ...
    void drawContent(HDC dc) {
        int saved = SaveDC(dc);
        IntersectClipRect(dc, layout.contentClip.left,
                         layout.contentClip.top,
                         layout.contentClip.right,
                         layout.contentClip.bottom);
        drawFormulaSection(dc);
        drawLeftColumn(dc);
        drawValueColumn(dc);
        for (const ButtonSpec& button : buttons) {
            if (button.content && button.hit != HIT_PICKER)
        ...
        presetDropdown.drawPopup(
            dc, resources.small(), dropdownStyle, layout.contentClip);
        RestoreDC(dc, saved);
        scrollbar.draw(dc, {});
    }
    ...
    void drawFooter(HDC dc) {
        ...
        drawText(dc, headline, headlineText, headlineColor,
                 tinyFont,
                 DT_LEFT | DT_VCENTER | DT_SINGLELINE);

        RECT detail = pixelRect(
            16, footerTopDip + 28,
            std::max(90, buttonBoundary), footerTopDip + 51);
        std::wstring detailText = status.empty()
            ? (dirty() ? L"Changes are staged until Apply"
                       : L"Configuration matches the last Apply")
            : status;
        drawText(dc, detail, detailText,
                 statusError ? DANGER : CLR_TEXT_DIM,
                 tinyFont,
    ...
    bool pointInContent(POINT point) const {
        return containsPoint(layout.contentClip, point);
    }

    void beginTextMouse(ui::TextField& field, int focus,
    ...
    void onLeftButtonDown(POINT point) {
        bool dropdownWasOpen = presetDropdown.open();
        auto dropdownResult =
            presetDropdown.mouseDown(point, layout.contentClip);
        notifyDropdownTransition(dropdownWasOpen);
        if (dropdownResult != ui::Dropdown::MouseResult::Ignored) {
            setFocused(FOCUS_PRESET);
            if (dropdownResult ==
                ui::Dropdown::MouseResult::SelectionChanged) {
                notifyAccessibility(EVENT_OBJECT_SELECTION,
                                    FORMULA_ACC_PRESET);
                notifyAccessibility(EVENT_OBJECT_VALUECHANGE,
                                    FORMULA_ACC_PRESET);
        ...
        if (pointInContent(point)) {
            if (containsPoint(layout.formulaField, point)) {
                beginTextMouse(
                    formulaField, FOCUS_FORMULA, point, true);
                return;
            }
            if (containsPoint(layout.bailout, point)) {
                beginTextMouse(
                    bailoutField, FOCUS_BAILOUT, point, false);
                return;
            }
            if (containsPoint(layout.realField, point) &&
                realField.enabled()) {
                beginTextMouse(realField, FOCUS_REAL, point, false);
                return;
            }
            if (containsPoint(layout.imaginaryField, point) &&
                imaginaryField.enabled()) {
                beginTextMouse(
                    imaginaryField, FOCUS_IMAGINARY, point, false);
                return;
            }
    ...
    void onLeftButtonDoubleClick(POINT point) {
        ...
        if (containsPoint(layout.formulaField, point)) {
            ...
            setFocused(FOCUS_BAILOUT);
            bailoutField.selectAll();
        } else if (containsPoint(layout.realField, point) &&
                   realField.enabled()) {
            setFocused(FOCUS_REAL);
            realField.selectAll();
        } else if (containsPoint(layout.imaginaryField, point) &&
                   imaginaryField.enabled()) {
            setFocused(FOCUS_IMAGINARY);
            imaginaryField.selectAll();
        }
    ...
    void onMouseMove(POINT point) {
        if (draggingText) {
            if (draggingText->mouseMove(point))
                InvalidateRect(hwnd, nullptr, FALSE);
        ...
        if (containsPoint(layout.formulaField, point) ||
            containsPoint(layout.bailout, point) ||
            (realField.enabled() &&
            ...
            SetCursor(LoadCursorW(nullptr, IDC_CROSS));
                   ...
                   containsPoint(layout.preset, point) ||
                   scrollbar.contains(point)) {
    ...
    bool keyDown(UINT key) {
        ...
        if (focusedId == FOCUS_PRESET) {
            bool wasOpen = presetDropdown.open();
            if (!presetDropdown.keyDown(
                    key, alt, layout.contentClip)) {
        ...
        if (focusedId == FOCUS_SCROLLBAR && scrollbar.visible()) {
            int line = 48;
            int page = std::max(
                line, clientHeightDip() - HEADER_HEIGHT -
                          FOOTER_HEIGHT - BODY_PADDING * 2);
            if (key == VK_UP) scrollbar.scrollBy(-line);
            else if (key == VK_DOWN) scrollbar.scrollBy(line);
            else if (key == VK_PRIOR) scrollbar.scrollBy(-page);
            else if (key == VK_NEXT) scrollbar.scrollBy(page);
            else if (key == VK_HOME) scrollbar.setPosition(0);
            else if (key == VK_END)
        ...
        if (key == VK_RETURN || key == VK_SPACE) {
            if (focusedId != FOCUS_FORMULA &&
                focusedId != FOCUS_BAILOUT &&
                focusedId != FOCUS_REAL &&
                focusedId != FOCUS_IMAGINARY &&
                focusedId != FOCUS_PRESET) {
                invokeHit(focusedId);
                return true;
            }
        }
    ...
    LRESULT handleMessage(UINT message, WPARAM wp, LPARAM lp) {
        ...
        case WM_GETOBJECT:
            if (static_cast<LONG>(lp) == OBJID_CLIENT)
                return formulaEditorAccessibilityGetObject(
                    accessibility, wp, lp);
            break;
        case WM_FORMULA_ACCESSIBILITY_SNAPSHOT: {
            if (reinterpret_cast<FormulaEditorAccessibilityProvider*>(wp) !=
                    accessibility ||
                !lp) {
                return FALSE;
            }
            auto* request =
                reinterpret_cast<FormulaAccessibilitySnapshotRequest*>(lp);
            if (!request->items) return FALSE;
            std::shared_ptr<AccessibilityGuard> guard =
                accessibilityGuard;
        ...
        case WM_FORMULA_ACCESSIBILITY_ACTION: {
            ...
            auto* request =
                reinterpret_cast<FormulaAccessibilityActionRequest*>(lp);
            std::shared_ptr<AccessibilityGuard> guard =
                accessibilityGuard;
            if (!guard || guard->action) return FALSE;
            guard->action = true;
            request->handled =
                performAccessibilityAction(request->key, request->action);
            guard->action = false;
        ...
        case WM_GETDLGCODE:
            return DLGC_WANTTAB | DLGC_WANTARROWS |
                   DLGC_WANTCHARS;
        case WM_SETFOCUS:
            if (ui::TextField* field =
                    textFieldForFocus(focusedId)) {
        ...
        case WM_NCDESTROY: {
            ...
            LRESULT result =
                DefWindowProcW(destroyed, message, wp, lp);
            hwnd = nullptr;
            return result;
        }
        }
...
bool FormulaEditorPanel::visible() const {
    return _impl->hwnd &&
           IsWindowVisible(_impl->hwnd) != FALSE;
}

void FormulaEditorPanel::move(const RECT& bounds) {
    if (!_impl->hwnd) return;
    SetWindowPos(
        _impl->hwnd, nullptr, bounds.left, bounds.top,
...
void FormulaEditorPanel::setConfig(
    const FormulaDialogConfig& config) {
