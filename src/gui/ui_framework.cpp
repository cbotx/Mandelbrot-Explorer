...
class UniqueGdiObject {
  public:
    ...
    explicit UniqueGdiObject(T object, bool owned = true) : _object(object), _owned(owned) {}
    ...
    void reset(T object = nullptr, bool owned = true) {
        if (_object && _owned) { DeleteObject(_object); }
        _object = object;
        _owned = owned;
    }
...
class ScopedSelection {
  public:
    ...
    ~ScopedSelection() {
        if (_valid) { SelectObject(_dc, _old); }
    }
...
HFONT createUiFont(int dpi, int dipHeight, int weight, const wchar_t* faceName) {
    int height = MulDiv(dipHeight, dpi, 96);
    if (height <= 0) { height = 1; }

    return CreateFontW(-height, 0, 0, 0, weight, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, DEFAULT_PITCH | FF_DONTCARE, faceName);
}

UniqueGdiObject<HFONT> createFontOrFallback(int dpi, int dipHeight, int weight, const wchar_t* faceName) {
    HFONT font = createUiFont(dpi, dipHeight, weight, faceName);
    if (font) { return UniqueGdiObject<HFONT>(font, true); }
...
COLORREF blendColor(COLORREF from, COLORREF to, unsigned amount) {
    ...
    const auto channel = [inverse, amount](BYTE a, BYTE b) { return static_cast<BYTE>((static_cast<unsigned>(a) * inverse + static_cast<unsigned>(b) * amount + 127u) / 255u); };
    return RGB(channel(GetRValue(from), GetRValue(to)), channel(GetGValue(from), GetGValue(to)), channel(GetBValue(from), GetBValue(to)));
}

constexpr wchar_t kTextServiceProperty[] = L"MandelbrotExplorer.Ui.TextService.Context";
...
TextServiceContext* textContext(HWND window) {
    return static_cast<TextServiceContext*>(GetPropW(window, kTextServiceProperty));
}

LRESULT callOriginal(TextServiceContext* context, HWND window, UINT message, WPARAM wp, LPARAM lp) {
    if (context && context->originalProc) { return CallWindowProcW(context->originalProc, window, message, wp, lp); }
...
std::pair<DWORD, DWORD> readSelection(HWND window) {
    DWORD first = 0;
    DWORD last = 0;
    SendMessageW(window, EM_GETSEL, reinterpret_cast<WPARAM>(&first), reinterpret_cast<LPARAM>(&last));
...
SnapshotChanges updateSnapshot(HWND window, TextServiceContext* context) {
    ...
    changes.selection = currentSelection.first != context->selectionFirst || currentSelection.second != context->selectionLast;
    context->selectionFirst = currentSelection.first;
    context->selectionLast = currentSelection.second;
    return changes;
}

bool replacementFits(HWND window, size_t replacementLength, size_t maximumLength) {
    const int nativeLength = GetWindowTextLengthW(window);
    const size_t currentLength = nativeLength > 0 ? static_cast<size_t>(nativeLength) : 0u;
    const auto selected = readSelection(window);
    const size_t first = (std::min)(static_cast<size_t>(selected.first), currentLength);
    const size_t last = (std::min)(static_cast<size_t>(selected.second), currentLength);
    const size_t retainedLength = currentLength - (last - first);

    return retainedLength <= maximumLength && replacementLength <= maximumLength - retainedLength;
}
...
bool clipboardTextLength(HWND owner, size_t& length) {
    ...
    if (data) {
        ...
        const size_t capacity = static_cast<size_t>(bytes / static_cast<SIZE_T>(sizeof(wchar_t)));
        ...
        int wideLength = MultiByteToWideChar(CP_ACP, 0, text, static_cast<int>(byteLength), nullptr, 0);
        GlobalUnlock(data);
        if (wideLength < 0) {
            CloseClipboard();
            return false;
...
bool isMutationMessage(UINT message, LPARAM lp) {
    ...
    case WM_IME_COMPOSITION: return (lp & GCS_RESULTSTR) != 0;
...
bool replacementLengthForMessage(HWND window, UINT message, WPARAM wp, LPARAM lp, size_t& replacementLength, bool& replacesAll) {
    replacementLength = 0;
    replacesAll = false;

    switch (message) {
    case WM_CHAR:
        if (wp < L' ' || wp == 0x7fu) { return false; }
        replacementLength = 1;
        return true;

    case WM_PASTE: return clipboardTextLength(window, replacementLength);

    case EM_REPLACESEL: {
        const wchar_t* replacement = reinterpret_cast<const wchar_t*>(lp);
        replacementLength = replacement ? std::char_traits<wchar_t>::length(replacement) : 0u;
        return true;
    }
    ...
    case WM_IME_COMPOSITION:
        if ((lp & GCS_RESULTSTR) != 0) { return imeResultLength(window, replacementLength); }
        return false;
...
bool messageExceedsLimit(HWND window, UINT message, WPARAM wp, LPARAM lp, size_t maximumLength) {
    size_t replacementLength = 0;
    bool replacesAll = false;
    if (!replacementLengthForMessage(window, message, wp, lp, replacementLength, replacesAll)) { return false; }

    return replacesAll ? replacementLength > maximumLength : !replacementFits(window, replacementLength, maximumLength);
}
...
RECT DpiScale::px(RECT dipRect) const {
    return {px(dipRect.left), px(dipRect.top), px(dipRect.right), px(dipRect.bottom)};
}
...
bool Resources::create(int dpi) {
    ...
    auto regular = createFontOrFallback(effectiveDpi, 14, FW_NORMAL, L"Segoe UI");
    auto semibold = createFontOrFallback(effectiveDpi, 15, FW_SEMIBOLD, L"Segoe UI");
    auto small = createFontOrFallback(effectiveDpi, 11, FW_NORMAL, L"Segoe UI");
    auto mono = createFontOrFallback(effectiveDpi, 14, FW_NORMAL, L"Consolas");
    UniqueGdiObject<HBRUSH> panel(CreateSolidBrush(CLR_PANEL));
    UniqueGdiObject<HBRUSH> card(CreateSolidBrush(CLR_CARD));

    if (!regular.get() || !semibold.get() || !small.get() || !mono.get() || !panel.get() || !card.get()) { return false; }
...
void Resources::reset() {
    ...
    if (_panelBrush) { DeleteObject(_panelBrush); }
    if (_cardBrush) { DeleteObject(_cardBrush); }
...
HDC BackBuffer::begin(HDC target, int width, int height) {
    ...
    if (_dc && _bitmap && width == _width && height == _height) { return _dc; }

    UniqueDc newDc(CreateCompatibleDC(target));
    if (!newDc.get()) { return nullptr; }

    UniqueGdiObject<HBITMAP> newBitmap(CreateCompatibleBitmap(target, width, height));
    if (!newBitmap.get()) { return nullptr; }

    ScopedSelection selection(newDc.get(), newBitmap.get());
    if (!selection.valid()) { return nullptr; }
...
void BackBuffer::present(HDC target, const RECT& area) const {
    if (!target || !_dc) { return; }

    const int width = area.right - area.left;
    const int height = area.bottom - area.top;
    if (width <= 0 || height <= 0) { return; }

    BitBlt(target, area.left, area.top, width, height, _dc, area.left, area.top, SRCCOPY);
}
...
bool ScrollState::configure(int contentExtent, int viewportExtent) {
    ...
    const int newMaximum = newContentExtent > newViewportExtent ? newContentExtent - newViewportExtent : 0;
    const int newPosition = (std::min)(_position, newMaximum);

    const bool changed = _contentExtent != newContentExtent || _viewportExtent != newViewportExtent || _position != newPosition;
...
bool ScrollState::setPosition(int position) {
    ...
    if (clamped == _position) { return false; }
    _position = clamped;
    return true;
}

bool ScrollState::scrollBy(int delta) {
    const std::int64_t candidate = static_cast<std::int64_t>(_position) + static_cast<std::int64_t>(delta);
    const std::int64_t clamped = (std::max)(std::int64_t{0}, (std::min)(candidate, static_cast<std::int64_t>(maximumPosition())));
...
bool ScrollState::handleCommand(int command, int trackPosition) {
    ...
    switch (command) {
    case SB_LINEUP: return scrollBy(-lineStep);
    case SB_LINEDOWN: return scrollBy(lineStep);
    case SB_PAGEUP: return scrollBy(-_viewportExtent);
    case SB_PAGEDOWN: return scrollBy(_viewportExtent);
    case SB_THUMBPOSITION:
    case SB_THUMBTRACK: return setPosition(trackPosition);
    case SB_TOP: return setPosition(0);
    case SB_BOTTOM: return setPosition(maximumPosition());
    default: return false;
    }
}
...
int ScrollState::maximumPosition() const {
    return _contentExtent > _viewportExtent ? _contentExtent - _viewportExtent : 0;
}
...
int HitRouter::hit(int x, int y) const {
    for (auto region = _regions.rbegin(); region != _regions.rend(); ++region) {
        if (region->enabled && x >= region->bounds.left && x < region->bounds.right && y >= region->bounds.top && y < region->bounds.bottom) { return region->id; }
    }
    return 0;
}
...
bool HitRouter::move(int x, int y) {
    ...
    if (newHovered == _hovered) { return false; }
    _hovered = newHovered;
    return true;
}
...
bool HitRouter::press(int x, int y) {
    const int newHovered = hit(x, y);
    const bool changed = newHovered != _hovered || newHovered != _pressed;
    _hovered = newHovered;
    _pressed = newHovered;
    return changed;
}
...
int HitRouter::release(int x, int y) {
    ...
    const int clicked = _pressed != 0 && released == _pressed ? released : 0;
    _hovered = released;
    _pressed = 0;
    return clicked;
}
...
bool HitRouter::cancel() {
    if (_hovered == 0 && _pressed == 0) { return false; }
    _hovered = 0;
    _pressed = 0;
    return true;
}

void drawButton(HDC dc, RECT bounds, const std::wstring& text, HFONT font, ButtonStyle style, bool hovered, bool pressed, bool enabled, int radius) {
    ...
    default:
        ...
        if (pressed) { fill = blendColor(CLR_CARD_HOV, CLR_BG, 48); }
        break;
    }
    ...
    if (pressed && enabled) { OffsetRect(&textBounds, 0, 1); }
    drawText(dc, textBounds, text, textColor, font, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS);
}
...
void drawFocusRing(HDC dc, RECT bounds, int radius) {
    ...
    UniqueGdiObject<HPEN> pen(CreatePen(PS_SOLID, 2, CLR_ACCENT_HI));
    HBRUSH hollow = static_cast<HBRUSH>(GetStockObject(HOLLOW_BRUSH));
    if (!pen.get() || !hollow) { return; }

    ScopedSelection selectedPen(dc, pen.get());
    ScopedSelection selectedBrush(dc, hollow);
    if (!selectedPen.valid() || !selectedBrush.valid()) { return; }

    RoundRect(dc, bounds.left, bounds.top, bounds.right, bounds.bottom, (std::max)(0, radius), (std::max)(0, radius));
}
...
bool TextService::create(HWND parent, int controlId, size_t maximumLength, Callback callback) {
    destroy();
    if (!parent || !IsWindow(parent)) { return false; }

    const size_t effectiveMaximum = (std::min)(maximumLength, kNativeEditLimit);
    HWND edit = CreateWindowExW(0, L"EDIT", L"", WS_CHILD | WS_VISIBLE | WS_TABSTOP | ES_LEFT | ES_AUTOHSCROLL, -32000, -32000, 1, 1, parent, reinterpret_cast<HMENU>(static_cast<INT_PTR>(controlId)), GetModuleHandleW(nullptr), nullptr);
    if (!edit) { return false; }

    TextServiceContext* context = new (std::nothrow) TextServiceContext();
    if (!context) {
        DestroyWindow(edit);
        return false;
    }
    ...
    if (!SetPropW(edit, kTextServiceProperty, reinterpret_cast<HANDLE>(context))) {
        delete context;
        DestroyWindow(edit);
        return false;
    }
    ...
    WNDPROC original = reinterpret_cast<WNDPROC>(SetWindowLongPtrW(edit, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(&TextService::editProc)));
    ...
    const size_t nativeLimit = effectiveMaximum == 0 ? 1u : effectiveMaximum;
    SendMessageW(_edit, EM_SETLIMITTEXT, static_cast<WPARAM>(nativeLimit), 0);
...
void TextService::destroy() {
    ...
    _syncing = true;
    TextServiceContext* context = IsWindow(edit) ? textContext(edit) : nullptr;
    WNDPROC original = context && context->originalProc ? context->originalProc : _originalProc;
    bool restored = false;
    ...
    if (IsWindow(edit) && original) {
        ...
        if (current == &TextService::editProc) {
            SetLastError(ERROR_SUCCESS);
            const LONG_PTR previous = SetWindowLongPtrW(edit, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(original));
            restored = previous != 0 || GetLastError() == ERROR_SUCCESS;
        }
    }
    ...
    if (restored) {
        if (context) {
            if (textContext(edit) == context) { RemovePropW(edit, kTextServiceProperty); }
            delete context;
            context = nullptr;
        }
    }
...
bool TextService::setText(const std::wstring& value) {
    if (!_edit || !IsWindow(_edit) || value.size() > _maximumLength) {
        if (_edit && value.size() > _maximumLength) { MessageBeep(MB_ICONWARNING); }
        return false;
    }

    const bool wasSyncing = _syncing;
    _syncing = true;
    const LRESULT result = SendMessageW(_edit, WM_SETTEXT, 0, reinterpret_cast<LPARAM>(value.c_str()));
    _syncing = wasSyncing;
    return result != FALSE;
}
...
std::wstring TextService::text() const {
    std::wstring value;
    if (_edit && IsWindow(_edit)) { readWindowText(_edit, value); }
    return value;
}
...
void TextService::setInputAnchor(POINT clientPoint, int lineHeight) {
    ...
    SetWindowPos(_edit, nullptr, clientPoint.x, clientPoint.y, 1, height, SWP_NOZORDER | SWP_NOACTIVATE);
...
void TextService::focus() {
    if (_edit && IsWindow(_edit)) { SetFocus(_edit); }
}
...
void TextService::setSelection(size_t first, size_t last) {
    ...
    const size_t length = nativeLength > 0 ? static_cast<size_t>(nativeLength) : 0u;
    first = (std::min)(first, length);
    last = (std::min)(last, length);
    const size_t nativeMaximum = static_cast<size_t>((std::numeric_limits<LONG>::max)());
    first = (std::min)(first, nativeMaximum);
    last = (std::min)(last, nativeMaximum);

    SendMessageW(_edit, EM_SETSEL, static_cast<WPARAM>(first), static_cast<LPARAM>(last));
}

std::pair<size_t, size_t> TextService::selection() const {
    if (!_edit || !IsWindow(_edit)) { return {0u, 0u}; }

    const auto selected = readSelection(_edit);
    return {static_cast<size_t>(selected.first), static_cast<size_t>(selected.second)};
}

bool TextService::replaceSelection(const std::wstring& replacement, bool allowUndo) {
    if (!_edit || !IsWindow(_edit)) { return false; }
    if (!replacementFits(_edit, replacement.size(), _maximumLength)) {
        MessageBeep(MB_ICONWARNING);
        return false;
    }

    SendMessageW(_edit, EM_REPLACESEL, allowUndo ? TRUE : FALSE, reinterpret_cast<LPARAM>(replacement.c_str()));
    return true;
}

bool TextService::undo() {
    if (!_edit || !IsWindow(_edit) || SendMessageW(_edit, EM_CANUNDO, 0, 0) == FALSE) { return false; }
...
LRESULT CALLBACK TextService::editProc(HWND window, UINT message, WPARAM wp, LPARAM lp) {
    TextServiceContext* context = textContext(window);
    if (!context) { return DefWindowProcW(window, message, wp, lp); }

    if (message == WM_NCDESTROY) {
        WNDPROC original = context->originalProc;
        const LRESULT result = original ? CallWindowProcW(original, window, message, wp, lp) : DefWindowProcW(window, message, wp, lp);
    ...
    TextService* owner = context->owner;
    if (owner && mutation && messageExceedsLimit(window, message, wp, lp, owner->_maximumLength)) {
        MessageBeep(MB_ICONWARNING);
        return message == WM_SETTEXT ? FALSE : 0;
    }

    const bool outerMutation = mutation && context->mutationDepth == 0;
    if (mutation) { ++context->mutationDepth; }

    const LRESULT result = callOriginal(context, window, message, wp, lp);

    TextServiceContext* current = textContext(window);
    if (current != context) { return result; }

    if (mutation && current->mutationDepth > 0) { --current->mutationDepth; }
    if (mutation && !outerMutation) { return result; }

    if (message == WM_SETFOCUS || message == WM_KILLFOCUS) {
        owner = current->owner;
        if (owner) { owner->notify(message == WM_SETFOCUS ? Event::FocusGained : Event::FocusLost); }
        return result;
    }
    ...
    if (mutation || navigation) {
        ...
        owner = current->owner;
        if (!owner || owner->_syncing) { return result; }
        ...
        if (changes.text) {
            ...
            if (current != eventContext || current->owner != owner) { return result; }
        }
        if (changes.selection && current->selectionFirst == eventSelectionFirst && current->selectionLast == eventSelectionLast) { owner->notify(Event::CaretMoved); }
    }
...
bool PopupState::move(int delta) {
    if (!open || itemCount <= 0 || delta == 0) { return false; }

    const int oldSelected = selected;
    const int oldHovered = hovered;
    const std::int64_t count = itemCount;
    std::int64_t next = static_cast<std::int64_t>((std::max)(0, selected));
...
bool PopupState::setHovered(int index) {
    int newHovered = -1;
    if (open && itemCount > 0 && index >= 0) { newHovered = (std::min)(index, itemCount - 1); }
    if (newHovered == hovered) { return false; }
    hovered = newHovered;
    return true;
}
...
int PopupState::accept() {
    ...
    const int accepted = (std::max)(0, (std::min)(candidate, itemCount - 1));
    close();
    return accepted;
}
