...
bool classifyOrbitThumbnailPixel(
    ...
    if (expression) {
        ...
        if (!formula::classifyExpressionOrbit(
                *expression, pixel, maxIterations, classification,
                shouldCancel)) {
            return false;
        }
...
OrbitThumbnailWorker::OrbitThumbnailWorker()
    : _thread(&OrbitThumbnailWorker::run, this) {}
...
uint64_t OrbitThumbnailWorker::request(
    ...
    for (;;) {
        ...
        for (int y = 0; y < height && complete; ++y) {
            ...
            for (int x = 0; x < width; ++x) {
                ...
                double real =
                    x0 + (x1 - x0) * x / std::max(1, width - 1);
                OrbitThumbnailPixel classification;
                if (!classifyOrbitThumbnailPixel(
                        expression.get(), { real, imaginary },
                        maxIterations, classification, cancelled) ||
                    classification.cancelled) {
                    complete = false;
                    break;
                }
                uint8_t* pixel =
                    &result.pixels[((size_t)y * width + x) * 3];
                if (!classification.escaped) {
                    ...
                    double t = std::sqrt(
                        (double)classification.iterations /
                        maxIterations);
