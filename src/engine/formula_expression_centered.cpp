...
CenteredValue finish(
        Complex base, Complex delta, Complex endpoint,
        ExpressionCenteredStatus status =
            ExpressionCenteredStatus::Success) {
    if (status == ExpressionCenteredStatus::Success &&
        (!finite(base) || !finite(delta) || !finite(endpoint)))
        status = ExpressionCenteredStatus::NonFinite;
    return { base, delta, endpoint, status };
}

ExpressionCenteredStatus combine(
        ExpressionCenteredStatus left,
        ExpressionCenteredStatus right) {
...
bool segmentMayContainZero(Complex base, Complex endpoint) {
    ...
    const double minImaginary =
        std::min(base.imag(), endpoint.imag());
    const double maxImaginary =
        std::max(base.imag(), endpoint.imag());
    const long double dx =
        (long double)endpoint.real() - (long double)base.real();
    const long double dy =
        (long double)endpoint.imag() - (long double)base.imag();
    if (dx != 0.0L || dy != 0.0L) {
    ...
    return minReal <= 0.0 && maxReal >= 0.0 &&
           minImaginary <= 0.0 && maxImaginary >= 0.0;
}
...
CutCheck checkNegativeRealCut(Complex base, Complex endpoint) {
    ...
    const long double y0 = base.imag();
    const long double dx =
        (long double)endpoint.real() - x0;
    const long double dy =
        (long double)endpoint.imag() - y0;
    if (dx != 0.0L) {
        const long double t = -x0 / dx;
    ...
    if ((base.imag() == 0.0 && base.real() < 0.0) ||
        (endpoint.imag() == 0.0 && endpoint.real() < 0.0)) {
        result.cut = true;
        return result;
    }
    if (dy != 0.0L) {
...
bool signedZeroCutSideChanges(Complex base, Complex endpoint) {
    return base.real() < 0.0 && endpoint.real() < 0.0 &&
           base.imag() == 0.0 && endpoint.imag() == 0.0 &&
           std::signbit(base.imag()) !=
               std::signbit(endpoint.imag());
}
...
bool tangentSegmentMayHitPole(
        Complex base, Complex endpoint) {
    const double y0 = base.imag();
    const double y1 = endpoint.imag();
    if (!(std::min(y0, y1) <= 0.0 &&
          std::max(y0, y1) >= 0.0))
        return false;
    const long double dx =
        (long double)endpoint.real() -
        (long double)base.real();
    const long double dy =
        (long double)endpoint.imag() -
        (long double)base.imag();
    if (dy == 0.0L) {
        return y0 == 0.0 &&
               intervalContainsPeriodicPole(
                   base.real(), endpoint.real());
    }
    long double t =
        -(long double)y0 / dy;
    if (t < 0.0L || t > 1.0L) return false;
    long double crossing =
        (long double)base.real() + t * dx;
    return nearPeriodicPole((double)crossing);
}

bool hyperbolicTangentSegmentMayHitPole(
        Complex base, Complex endpoint) {
    Complex swappedBase{ base.imag(), base.real() };
    Complex swappedEndpoint{
        endpoint.imag(), endpoint.real()
    };
    return tangentSegmentMayHitPole(
        swappedBase, swappedEndpoint);
}

Complex centeredMultiply(
        Complex leftBase, Complex leftDelta,
        Complex rightBase, Complex rightDelta) {
...
Complex complexExpm1(Complex value) {
    ...
    if (x < -0.5) {
        ...
        return {
            exponential * cosine - 1.0,
            exponential * sine
        };
    }
    const double realExpm1 = std::expm1(x);
    const double halfSine = std::sin(y * 0.5);
    const double cosineMinusOne =
        -2.0 * halfSine * halfSine;
...
Complex relativeExponentialDelta(
        Complex baseOutput, Complex perturbedOutput,
...
ExpressionCenteredStatus centeredDivisionDelta(
        Complex numeratorDelta, Complex denominatorBase,
        Complex denominatorDelta, Complex denominatorEndpoint,
        Complex quotientBase,
        Complex& output, bool guardLinearSegment) {
    if (zero(denominatorBase) || zero(denominatorEndpoint) ||
        (guardLinearSegment &&
         segmentMayContainZero(
             denominatorBase, denominatorEndpoint)))
        return ExpressionCenteredStatus::Singular;
    Complex numerator = numeratorDelta -
                        quotientBase * denominatorDelta;
...
double centeredRealProduct(
        double leftBase, double leftDelta,
        double rightBase, double rightDelta) {
...
ExpressionCenteredStatus centeredTangent(
        Complex base, Complex delta, Complex endpoint,
        Complex baseOutput, Complex endpointOutput,
        Complex& output) {
    if (zero(delta)) {
        output = {};
        return ExpressionCenteredStatus::Success;
    }
    if (tangentSegmentMayHitPole(base, endpoint))
        return ExpressionCenteredStatus::Singular;
    if (!finite(baseOutput) || !finite(endpointOutput))
        return ExpressionCenteredStatus::NonFinite;

    const bool sameHighHalfPlane =
        std::abs(base.imag()) > 16.0 &&
        std::abs(endpoint.imag()) > 16.0 &&
        std::signbit(base.imag()) ==
            std::signbit(endpoint.imag());
    if (!sameHighHalfPlane) {
        ...
        if (!finite(sineBase) || !finite(cosineBase) ||
            !finite(cosineEndpoint) ||
            !finite(sineDelta) || !finite(cosineDelta))
            return ExpressionCenteredStatus::NonFinite;
        return centeredDivisionDelta(
            sineDelta, cosineBase, cosineDelta,
            cosineEndpoint, baseOutput, output, false);
    }

    const double sign = std::signbit(base.imag()) ? -1.0 : 1.0;
    const double q = std::exp(-2.0 * std::abs(base.imag()));
    const double q1 =
        std::exp(-2.0 * std::abs(endpoint.imag()));
    const double qDelta = relativeExponentialDelta(
        { q, 0.0 }, { q1, 0.0 },
        { -2.0 * sign * delta.imag(), 0.0 }).real();
    const double angle = 2.0 * base.real();
    const double angleDelta = 2.0 * delta.real();
    const double sine = std::sin(angle);
    const double cosine = std::cos(angle);
    const double sineDelta =
        centeredSine({ angle, 0.0 }, { angleDelta, 0.0 }).real();
    const double cosineDelta =
        centeredCosine({ angle, 0.0 }, { angleDelta, 0.0 }).real();
    double qSquaredDelta = q * qDelta;
    qSquaredDelta += q * qDelta;
    qSquaredDelta += qDelta * qDelta;
    const double numeratorRealDelta =
        2.0 * centeredRealProduct(
            q, qDelta, sine, sineDelta);
    const double numeratorImaginaryDelta =
        -sign * qSquaredDelta;
    double denominatorDelta = qSquaredDelta;
    denominatorDelta +=
        2.0 * centeredRealProduct(
            q, qDelta, cosine, cosineDelta);
    double denominator = 1.0 + q * q;
    denominator += 2.0 * q * cosine;
    double endpointDenominator = 1.0 + q1 * q1;
    endpointDenominator +=
        2.0 * q1 * std::cos(2.0 * endpoint.real());
    return centeredDivisionDelta(
        { numeratorRealDelta, numeratorImaginaryDelta },
        { denominator, 0.0 }, { denominatorDelta, 0.0 },
        { endpointDenominator, 0.0 },
        baseOutput, output, false);
}

ExpressionCenteredStatus centeredHyperbolicTangent(
        Complex base, Complex delta, Complex endpoint,
        Complex baseOutput, Complex endpointOutput,
        Complex& output) {
    if (zero(delta)) {
    ...
    if (hyperbolicTangentSegmentMayHitPole(
            base, endpoint))
        return ExpressionCenteredStatus::Singular;
    if (!finite(baseOutput) || !finite(endpointOutput))
        return ExpressionCenteredStatus::NonFinite;
    ...
    const bool sameHighHalfPlane =
        std::abs(base.real()) > 16.0 &&
        std::abs(endpoint.real()) > 16.0 &&
        std::signbit(base.real()) ==
            std::signbit(endpoint.real());
    if (!sameHighHalfPlane) {
        Complex sineBase = std::sinh(base);
        Complex cosineBase = std::cosh(base);
        Complex cosineEndpoint = std::cosh(endpoint);
        Complex sineDelta =
            centeredHyperbolicSine(base, delta);
        Complex cosineDelta =
            centeredHyperbolicCosine(base, delta);
        if (!finite(sineBase) || !finite(cosineBase) ||
            !finite(cosineEndpoint) ||
            !finite(sineDelta) || !finite(cosineDelta))
            return ExpressionCenteredStatus::NonFinite;
    ...
    const double q1 =
        std::exp(-2.0 * std::abs(endpoint.real()));
    const double qDelta = relativeExponentialDelta(
        { q, 0.0 }, { q1, 0.0 },
        { -2.0 * sign * delta.real(), 0.0 }).real();
    ...
    double endpointDenominator = 1.0 + q1 * q1;
    endpointDenominator +=
        2.0 * q1 * std::cos(2.0 * endpoint.imag());
    return centeredDivisionDelta(
        { numeratorRealDelta, numeratorImaginaryDelta },
        { denominator, 0.0 }, { denominatorDelta, 0.0 },
        { endpointDenominator, 0.0 },
...
ExpressionCenteredStatus centeredLogarithm(
        Complex base, Complex delta, Complex endpoint,
        Complex& output) {
    if (zero(base) || zero(endpoint))
        return ExpressionCenteredStatus::Singular;
    if (signedZeroCutSideChanges(base, endpoint))
        return ExpressionCenteredStatus::BranchUncertain;
    if (zero(delta)) {
        output = {};
        return ExpressionCenteredStatus::Success;
    }
...
ExpressionCenteredStatus centeredAbsolute(
        Complex base, Complex delta, Complex endpoint,
        Complex& output) {
    ...
    double numerator = centeredNormDelta(
        scaledBase, scaledDelta).real();
    double denominator =
        baseRadius / scale + perturbedRadius / scale;
    if (denominator == 0.0)
        return ExpressionCenteredStatus::Singular;
...
ExpressionCenteredStatus centeredSquareRoot(
        Complex base, Complex delta, Complex endpoint,
        Complex baseRoot, Complex endpointRoot,
        Complex& output) {
    if (zero(base) && zero(endpoint))
        return ExpressionCenteredStatus::Singular;
    if (signedZeroCutSideChanges(base, endpoint))
    ...
    if (zero(denominator) || cut.singular)
        return ExpressionCenteredStatus::Singular;
...
ExpressionCenteredStatus centeredPower(
        Complex base, Complex baseDelta, Complex baseEndpoint,
        Complex exponent, Complex exponentDelta,
        Complex exponentEndpoint, Complex baseOutput,
        Complex endpointOutput, Complex& output) {
    if (zero(base) || zero(baseEndpoint))
        return ExpressionCenteredStatus::Singular;
    if (signedZeroCutSideChanges(base, baseEndpoint))
        return ExpressionCenteredStatus::BranchUncertain;
    if (zero(baseDelta) && zero(exponentDelta)) {
        output = {};
        return ExpressionCenteredStatus::Success;
    }
    CutCheck cut = checkNegativeRealCut(
        base, baseEndpoint);
    if (cut.singular)
        return ExpressionCenteredStatus::Singular;
    if (cut.cut) return ExpressionCenteredStatus::BranchUncertain;
    ...
    ExpressionCenteredStatus status =
        centeredLogarithm(
            base, baseDelta, baseEndpoint, logDelta);
    if (status != ExpressionCenteredStatus::Success)
        return status;
    Complex productDelta =
        centeredMultiply(
            exponent, exponentDelta,
            logBase, logDelta);
    if (!finite(exponentEndpoint) ||
        !finite(baseOutput) || !finite(endpointOutput))
        return ExpressionCenteredStatus::NonFinite;
    output = relativeExponentialDelta(
        baseOutput, endpointOutput, productDelta);
    return finite(output)
        ? ExpressionCenteredStatus::Success
        : ExpressionCenteredStatus::NonFinite;
}

ExpressionCenteredStatus centeredPolar(
        const CenteredValue& radius,
        const CenteredValue& angle,
        Complex baseOutput, Complex endpointOutput,
        Complex& output) {
    if (radius.base.imag() != 0.0 ||
        radius.delta.imag() != 0.0 ||
        radius.endpoint.imag() != 0.0 ||
        angle.base.imag() != 0.0 ||
        angle.delta.imag() != 0.0 ||
        angle.endpoint.imag() != 0.0)
        return ExpressionCenteredStatus::Undefined;
    ...
    if (!std::isfinite(baseRadius) ||
        !std::isfinite(radiusDelta) ||
        !std::isfinite(baseAngle) ||
        !std::isfinite(angleDelta) ||
        !std::isfinite(perturbedRadius) ||
        !std::isfinite(perturbedAngle))
        return ExpressionCenteredStatus::NonFinite;
    if (baseRadius < 0.0 || perturbedRadius < 0.0)
        return ExpressionCenteredStatus::Undefined;
    if (!finite(baseOutput) || !finite(endpointOutput))
        return ExpressionCenteredStatus::NonFinite;
    Complex angleResidual =
        complexExpm1({ 0.0, angleDelta });
    Complex result = baseOutput * angleResidual;
    result += radiusDelta *
              std::polar(1.0, perturbedAngle);
    output = result;
    return finite(output)
        ? ExpressionCenteredStatus::Success
        : ExpressionCenteredStatus::NonFinite;
...
const char* expressionCenteredStatusName(
        ExpressionCenteredStatus status) {
    switch (status) {
    case ExpressionCenteredStatus::Success:
        return "success";
    case ExpressionCenteredStatus::BranchUncertain:
        return "branch-uncertain";
    case ExpressionCenteredStatus::Singular:
        return "singular";
    case ExpressionCenteredStatus::Undefined:
        return "undefined";
    case ExpressionCenteredStatus::Unsupported:
        return "unsupported";
    case ExpressionCenteredStatus::NonFinite:
        return "nonfinite";
    case ExpressionCenteredStatus::InvalidProgram:
        return "invalid-program";
    }
    return "invalid-program";
}

ExpressionCenteredResult ExpressionCenteredEvaluator::evaluate(
        const ExpressionProgram& program,
        const ExpressionContext& reference,
        const ExpressionDeltaContext& inputDelta) {
    ...
    auto unary = [&](const ExpressionProgram::Instruction& instruction) {
        ...
        Complex base =
            ExpressionProgram::evaluateUnary(
                instruction.op, input.base);
        Complex endpoint =
            ExpressionProgram::evaluateUnary(
                instruction.op, input.endpoint);
        if (input.status != ExpressionCenteredStatus::Success) {
            stack[top - 1] =
                finish(
                    base, nanComplex(), endpoint,
                    input.status);
            return;
        }
        ...
        case ExpressionProgram::Op::Square:
            delta = centeredSquare(input.base, input.delta);
            break;
        case ExpressionProgram::Op::Sin:
            delta = centeredSine(input.base, input.delta);
            break;
        case ExpressionProgram::Op::Cos:
            delta = centeredCosine(input.base, input.delta);
            break;
        case ExpressionProgram::Op::Tan:
            status = centeredTangent(
                input.base, input.delta, input.endpoint,
                base, endpoint, delta);
            break;
        case ExpressionProgram::Op::Sinh:
            delta =
                centeredHyperbolicSine(
                    input.base, input.delta);
            break;
        case ExpressionProgram::Op::Cosh:
            delta =
                centeredHyperbolicCosine(
                    input.base, input.delta);
            break;
        case ExpressionProgram::Op::Tanh:
            status = centeredHyperbolicTangent(
                input.base, input.delta, input.endpoint,
                base, endpoint, delta);
            break;
        case ExpressionProgram::Op::Exp: {
            if (!finite(base) || !finite(endpoint)) {
                ...
                delta = relativeExponentialDelta(
                    base, endpoint, input.delta);
            }
            break;
        }
        case ExpressionProgram::Op::Log:
            status = centeredLogarithm(
                input.base, input.delta,
                input.endpoint, delta);
            break;
        ...
        case ExpressionProgram::Op::Sqrt:
            status = centeredSquareRoot(
                input.base, input.delta, input.endpoint,
                base, endpoint, delta);
            break;
        case ExpressionProgram::Op::Abs:
            status = centeredAbsolute(
                input.base, input.delta,
                input.endpoint, delta);
            break;
        ...
        stack[top - 1] =
            finish(base, delta, endpoint, status);
    };
    auto binary = [&](const ExpressionProgram::Instruction& instruction) {
        CenteredValue right = stack[--top];
        CenteredValue left = stack[top - 1];
        Complex base =
            ExpressionProgram::evaluateBinary(
                instruction.op, left.base, right.base);
        Complex endpoint =
            ExpressionProgram::evaluateBinary(
                instruction.op,
                left.endpoint, right.endpoint);
        ExpressionCenteredStatus status =
            combine(left.status, right.status);
        if (status != ExpressionCenteredStatus::Success) {
            stack[top - 1] =
                finish(
                    base, nanComplex(), endpoint, status);
            return;
        }
        ...
        case ExpressionProgram::Op::Multiply:
            delta = centeredMultiply(
                left.base, left.delta,
                right.base, right.delta);
            break;
        case ExpressionProgram::Op::Divide: {
            Complex quotientDelta;
            status = centeredDivisionDelta(
                left.delta, right.base, right.delta,
                right.endpoint, base,
                quotientDelta, true);
            delta = quotientDelta;
            break;
        ...
        case ExpressionProgram::Op::Power:
            status = centeredPower(
                left.base, left.delta, left.endpoint,
                right.base, right.delta, right.endpoint,
                base, endpoint, delta);
            break;
        ...
        case ExpressionProgram::Op::Polar:
            status = centeredPolar(
                left, right, base, endpoint, delta);
            break;
        default:
            status = ExpressionCenteredStatus::Unsupported;
            delta = nanComplex();
    ...
    for (const ExpressionProgram::Instruction& instruction :
        ...
        case ExpressionProgram::Op::Constant:
            pushUnchanged(instruction.value);
            break;
        case ExpressionProgram::Op::Z:
            pushPerturbed(reference.z, inputDelta.z);
            break;
        case ExpressionProgram::Op::C:
            pushPerturbed(reference.c, inputDelta.c);
            break;
        case ExpressionProgram::Op::Z0:
            pushPerturbed(reference.z0, inputDelta.z0);
            break;
        case ExpressionProgram::Op::Iteration:
            pushUnchanged({
                (double)reference.iteration, 0.0
            });
            break;
        case ExpressionProgram::Op::Parameter:
            pushPerturbed(
                reference.parameters[instruction.argument],
                inputDelta.parameters[instruction.argument]);
            break;
        case ExpressionProgram::Op::OrbitInvariant:
            stack[top++] = finish(
                nanComplex(), nanComplex(), nanComplex(),
                ExpressionCenteredStatus::Unsupported);
            break;
        case ExpressionProgram::Op::Negate:
        case ExpressionProgram::Op::Square:
        case ExpressionProgram::Op::Sin:
