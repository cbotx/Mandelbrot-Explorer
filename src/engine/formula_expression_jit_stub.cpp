...
void ExpressionJitInput4::setContextLane(int lane, const ExpressionContext& context) {
    ...
    vectors[N_RE][lane] = static_cast<double>(context.iteration);
    vectors[N_IM][lane] = 0.0;
    for (int parameter = 0; parameter < 8; ++parameter) {
        vectors[PARAM_RE + 2 * parameter][lane] = context.parameters[parameter].real();
        vectors[PARAM_RE + 2 * parameter + 1][lane] = context.parameters[parameter].imag();
    }
}
...
bool ExpressionJit4::compile(const ExpressionProgram&, std::string* error) {
    if (error) *error = "JIT is disabled in this build";
    return false;
}

bool ExpressionJit4::compile(const ExpressionOrbitPlan&, std::string* error) {
    if (error) *error = "JIT is disabled in this build";
    return false;
}

bool ExpressionJit4::compileProgram(const ExpressionProgram&, size_t, const std::string*, std::string* error) {
    if (error) *error = "JIT is disabled in this build";
    return false;
}
...
bool ExpressionJit4::supports(const ExpressionOrbitPlan&) const {
    return false;
}
...
void ExpressionJit4::evaluate(const ExpressionJitInput4&, ExpressionJitOutput4&) const {
}
void ExpressionJit4::evaluate(const ExpressionJitInput4&, const ExpressionJitInvariantInput4*, ExpressionJitOutput4&) const {
}
bool ExpressionJit4::evaluate(const ExpressionContext*, Complex*) const {
    return false;
}
bool ExpressionJit4::evaluateOrbit(const ExpressionContext*, int, int, double, float*, const std::atomic_bool*, const ExpressionOrbitPlan*) const {
    return false;
}
