...
static inline FloatExp fe_from(double x) {
    ...
    int ex; double mm = std::frexp(x, &ex);
...
static inline FloatExp fe_renorm(double m, int64_t e) {
    ...
    int ex; double mm = std::frexp(m, &ex);
...
static inline FloatExp fe_add(FloatExp a, FloatExp b) {
    ...
    if (d >= 0) return fe_renorm(a.m + std::ldexp(b.m, -(int)d), a.e);
    else        return fe_renorm(b.m + std::ldexp(a.m,  (int)d), b.e);
}

static inline FloatExp fe_neg(FloatExp a) { return FloatExp{ -a.m, a.e }; }
static inline FloatExp fe_sub(FloatExp a, FloatExp b) { return fe_add(a, fe_neg(b)); }
...
static inline FloatExp fe_sqrt(FloatExp a) {
    ...
    int64_t e = a.e; double m = a.m;
    if (e & 1) { m *= 2.0; --e; }       // now e even, m in [1,2)
