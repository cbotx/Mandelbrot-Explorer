...
private:
    ...
    Complex evaluatePrepared(const ExpressionContext& context,
                             const Complex* invariants,
                             size_t invariantCount,
                             Complex* stack, size_t capacity) const;
    bool evaluate4Prepared(const ExpressionContext* contexts,
                           const Complex* const* invariants,
                           size_t invariantCount,
                           Complex* outputs) const;
    bool evaluate4HybridPrepared(const ExpressionContext* contexts,
                                 const Complex* const* invariants,
                                 size_t invariantCount,
                                 Complex* outputs) const;
...
public:
    ...
    bool build(const ExpressionProgram& program,
               ExpressionError* error = nullptr);
    void reset();

    bool valid() const { return _valid; }
    bool profitable() const {
        return _valid && !_invariantPrograms.empty();
    }
    ...
    uint8_t invariantDependencyMask(size_t index) const {
        return index < _invariantDependencies.size()
            ? _invariantDependencies[index] : DependencyNone;
    }
    size_t invariantInstructionCount(size_t index) const {
        return index < _invariantPrograms.size()
            ? _invariantPrograms[index].instructionCount() : 0;
    }
    size_t invariantOperationCount() const {
        return _invariantOperationCount;
    }
    ...
    bool avx2Compatible() const {
        return _valid && _body.avx2Compatible();
    }
    bool batchCompatible() const {
        return _valid && _body.batchCompatible();
    }

    bool prepare(const ExpressionContext& context,
                 Prepared& prepared) const;
    bool prepare4(const ExpressionContext* contexts, uint8_t activeMask,
                  Prepared* prepared) const;
    Complex evaluate(const ExpressionContext& context,
                     const Prepared& prepared) const;
    Complex evaluate(const ExpressionContext& context,
                     const Prepared& prepared,
                     Complex* stack, size_t capacity) const;
    bool evaluate4(const ExpressionContext* contexts,
                   const Prepared* prepared, Complex* outputs) const;
    bool evaluate4Hybrid(const ExpressionContext* contexts,
                         const Prepared* prepared, Complex* outputs) const;
