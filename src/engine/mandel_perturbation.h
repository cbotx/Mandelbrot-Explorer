...
inline bool expressionColoringIndexSupported(int index, bool supportsDistance) {
    return index == 0 || index == 2 || index == 5 || (index == 1 && supportsDistance);
}

inline int expressionColoringMethodMask(bool supportsDistance) {
    return ColoringMethod::STRIPE_AVERAGE | ColoringMethod::ORBIT_TRAP | (supportsDistance ? ColoringMethod::EXTERIOR_DIST_EST : 0);
}

inline formula::ExpressionColoring expressionColoringFromMethod(int method, bool supportsDistance) {
    if (method & ColoringMethod::STRIPE_AVERAGE) return formula::ExpressionColoring::Feather;
    if (method & ColoringMethod::ORBIT_TRAP) return formula::ExpressionColoring::OrbitTrap;
    if (supportsDistance) { return method & ColoringMethod::EXTERIOR_DIST_EST ? formula::ExpressionColoring::Distance : formula::ExpressionColoring::Smooth; }
...
class Mandel {
  ...
  public:
    ...
    void Compute(mpf_t c_re, mpf_t c_im, mpf_t scale, int mxit, int c_method = 0, int full_h = 0, int row_base = 0);
    ...
    bool ComputeExpressionResidual(mpf_t center_re, mpf_t center_im, mpf_t scale, const formula::ExpressionProgram& program, const formula::ExpressionContext& fixed, FormulaParameter pixelParameter, int mxit, double bailout, bool* usedPerturbation = nullptr, formula::ExpressionColoring coloring = formula::ExpressionColoring::Raw, int* seriesIterations = nullptr, int minimumSeriesIterations = 0);
    ...
    void ComputeShallowPoints(const double* cRe, const double* cIm, int count, float* output, int mxit) const;
    ...
    void ComputeJuliaDirect(mpf_t fixed_c_re, mpf_t fixed_c_im, int mxit, float* out, int step = 1, int c_method = 0);
    ...
    bool HaltRequested() const { return _haltRequested.load(std::memory_order_acquire); }
    void SetProgress(std::atomic<float>* progress, float offset = 0.0f, float scale = 1.0f);
    double escapeRadius() const { return _customEscapeRadiusActive ? _customEscapeRadius : static_cast<double>(_ESCAPE_RADIUS); }
    double escapeRadiusSquared() const {
        if (_customEscapeRadiusActive) return _customEscapeRadius * _customEscapeRadius;
        return static_cast<double>(_ESCAPE_RADIUS * _ESCAPE_RADIUS);
    }
    class ScopedCustomCompute {
      public:
        ScopedCustomCompute(Mandel& engine, double escapeRadius, formula::CustomDeepZoomOutputAdapter outputAdapter) : _engine(&engine), _previousRadiusActive(engine._customEscapeRadiusActive), _previousRadius(engine._customEscapeRadius), _previousAdapter(engine._customOutputAdapter) {
            if (!formula::isCustomDeepEscapeRadiusSupported(escapeRadius)) {
                _engine = nullptr;
                return;
            }
    ...
    bool lastCustomHistorySeriesWasBuilt() const { return _lastCustomHistorySeriesBuilt; }
    bool lastCustomHistoryBlaWasRequested() const { return _lastCustomHistoryBlaRequested; }
    bool lastCustomHistorySkipsDisabled() const { return _lastCustomHistoryMode && !_use_bla && !_SA_flag && _SA_it == 0 && _SA_order == 0; }
  ...
  private:
    ...
    void stepParallel(std::set<std::array<int, 4>>& s, int mx_ref_it, int mxit, int c_method = 0, const std::vector<std::array<int, 4>>* contiguous = nullptr);
    ...
    void solveSimd4(const std::array<int, 4>* v, int g, int lanes, const double* dcr, const double* dci, const double* dzr, const double* dzi, int mx_ref_it, int mxit, Float* glitch_p);
    ...
    void solveShallowSimdList(const double* cre, const double* cim, int count, float* out, int mxit, int c_method) const;
    void solveJuliaShallowSimdList(const double* z0re, const double* z0im, int count, double cre, double cim, float* out, int mxit, bool ede) const;
    float accurateJuliaPoint(mpf_t z0_re, mpf_t z0_im, mpf_t c_re, mpf_t c_im, int mxit, bool ede) const;
    ...
    int tryBLA(int s, double& dzr, double& dzi, double& ddr, double& ddi, double dcr, double dci, bool ede, double ESC2, int mx_ref_it) const;
    ...
    int tryBLAfe(int s, FloatExp& S, FloatExp S2, double& wr, double& wi, double dr, double di, double ESC2, int mx_ref_it, double* outAB = nullptr) const;
    int createRef(std::set<std::array<int, 4>>& s, int pr_it, int mxit, bool random, int c_method = 0, bool view_center = false);
    ...
    bool calCoefficient(int i, int pr_it, int c_method = 0, bool continuePastEscape = false);
    ...
    float pixelRescaled(FloatExp dcr, FloatExp dci, double customSeedRe, double customSeedIm, int mx_ref_it, int mxit, int c_method, float* normalOut = nullptr) const;
    ...
    int buildReferenceOrbit(std::set<std::array<int, 4>>& s, mpf_t scale, int mxit, int c_method, bool profile, double& pf_ref);
  ...
  private:
    ...
    formula::CustomDeepZoomOutputAdapter _customOutputAdapter = formula::CustomDeepZoomOutputAdapter::None;
