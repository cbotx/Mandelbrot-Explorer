...
uint64_t allocateProgramIdentity() {
    uint64_t identity = nextProgramIdentity.fetch_add(1, std::memory_order_relaxed);
    if (identity == 0) identity = nextProgramIdentity.fetch_add(1, std::memory_order_relaxed);
    return identity;
}
...
class ExpressionParser {
  public:
    ExpressionParser(ExpressionProgram& program, const std::string& source, ExpressionError* error) : _program(program), _source(source), _error(error) {}

    bool parse() {
        if (_source.empty()) return fail(0, "expression is empty");
        if (_source.size() > ExpressionProgram::MAX_SOURCE) return fail(ExpressionProgram::MAX_SOURCE, "expression is too long");
  ...
  private:
    ...
    struct DepthGuard {
        ...
        bool ok;
        explicit DepthGuard(ExpressionParser& p) : parser(p) { ok = ++parser._depth <= ExpressionProgram::MAX_PARSE_DEPTH; }
    ...
    void skipSpace() {
        while (_pos < _source.size() && (_source[_pos] == ' ' || _source[_pos] == '\t' || _source[_pos] == '\r' || _source[_pos] == '\n')) ++_pos;
    }
    ...
    bool emit(ExpressionProgram::Op op, int pops, int pushes, uint8_t argument = 0, Complex value = {}) {
        if (_program._code.size() >= ExpressionProgram::MAX_INSTRUCTIONS) return fail(_pos, "expression has too many operations");
        if (_stack < (size_t)pops) return fail(_pos, "invalid operand count");
        _stack = _stack - pops + pushes;
        _maxStack = std::max(_maxStack, _stack);
        if (_maxStack > ExpressionProgram::MAX_STACK) return fail(_pos, "expression stack is too deep");
    ...
    bool parseAddSubtract() {
        ...
        for (;;) {
            ...
            ++_pos;
            if (!parseMultiplyDivide()) return false;
            if (!emit(op == '+' ? ExpressionProgram::Op::Add : ExpressionProgram::Op::Subtract, 2, 1)) return false;
        }
    }
    ...
    bool parseMultiplyDivide() {
        ...
        for (;;) {
            ...
            ++_pos;
            if (!parseUnary()) return false;
            if (!emit(op == '*' ? ExpressionProgram::Op::Multiply : ExpressionProgram::Op::Divide, 2, 1)) return false;
        }
    }
    ...
    bool parsePrimary() {
        ...
        if ((first >= '0' && first <= '9') || first == '.') return parseNumber();
        if ((first >= 'A' && first <= 'Z') || (first >= 'a' && first <= 'z') || first == '_') return parseIdentifier();
    ...
    bool parseNumber() {
        ...
        if (end == begin || errno == ERANGE) return fail(_pos, "invalid numeric literal");
        _pos += (size_t)(end - begin);
        skipSpace();
        bool imaginary = _pos < _source.size() && _source[_pos] == 'i';
        if (imaginary) ++_pos;
        return emit(ExpressionProgram::Op::Constant, 0, 1, 0, imaginary ? Complex{0.0, value} : Complex{value, 0.0});
    }
    ...
    bool parseIdentifier() {
        ...
        if (name == "i") return emit(ExpressionProgram::Op::Constant, 0, 1, 0, Complex{0.0, 1.0});
        if (name == "pi") return emit(ExpressionProgram::Op::Constant, 0, 1, 0, Complex{3.14159265358979323846, 0.0});
        if (name == "e") return emit(ExpressionProgram::Op::Constant, 0, 1, 0, Complex{2.71828182845904523536, 0.0});
        if (name.size() == 2 && name[0] == 'p' && name[1] >= '0' && name[1] <= '7') return emit(ExpressionProgram::Op::Parameter, 0, 1, (uint8_t)(name[1] - '0'));
    ...
    bool parseFunction(const std::string& name, size_t namePosition) {
        ...
        static constexpr Unary unary[] = {{"sqr", ExpressionProgram::Op::Square}, {"sin", ExpressionProgram::Op::Sin}, {"cos", ExpressionProgram::Op::Cos}, {"tan", ExpressionProgram::Op::Tan}, {"sinh", ExpressionProgram::Op::Sinh}, {"cosh", ExpressionProgram::Op::Cosh}, {"tanh", ExpressionProgram::Op::Tanh}, {"exp", ExpressionProgram::Op::Exp}, {"log", ExpressionProgram::Op::Log}, {"log10", ExpressionProgram::Op::Log10}, {"sqrt", ExpressionProgram::Op::Sqrt}, {"abs", ExpressionProgram::Op::Abs}, {"norm", ExpressionProgram::Op::Norm}, {"arg", ExpressionProgram::Op::Arg}, {"conj", ExpressionProgram::Op::Conjugate}, {"real", ExpressionProgram::Op::Real}, {"re", ExpressionProgram::Op::Real}, {"imag", ExpressionProgram::Op::Imaginary}, {"im", ExpressionProgram::Op::Imaginary}};
        ...
        ExpressionProgram::Op op;
        if (name == "pow")
            op = ExpressionProgram::Op::Power;
        else if (name == "complex")
            op = ExpressionProgram::Op::MakeComplex;
        else if (name == "polar")
            op = ExpressionProgram::Op::Polar;
        else
            return fail(namePosition, "unknown function");
...
int ExpressionProgram::operandCount(Op op) {
    ...
    case Op::Iteration:
    case Op::Parameter:
    case Op::OrbitInvariant: return 0;
    case Op::Negate:
    case Op::Square:
    case Op::Sin:
    case Op::Cos:
...
Complex ExpressionProgram::evaluateBinary(Op op, Complex left, Complex right) {
    ...
    case Op::Polar: {
        const double nan = std::numeric_limits<double>::quiet_NaN();
        if (left.imag() != 0.0 || right.imag() != 0.0 || !std::isfinite(left.real()) || left.real() < 0.0 || !std::isfinite(right.real())) return {nan, nan};
...
bool ExpressionProgram::analyze(ExpressionError* error) {
    ...
    if (_code.size() > MAX_INSTRUCTIONS) return fail("expression has too many operations");
    ...
    for (const Instruction& instruction : _code) {
        ...
        if (instruction.op == Op::Parameter && instruction.argument >= 8) return fail("expression contains an invalid parameter");
        if (stack < (size_t)operands) return fail("invalid expression stack");
        stack = stack - (size_t)operands + 1;
        _stackDepth = std::max(_stackDepth, stack);
        if (_stackDepth > MAX_STACK) return fail("expression stack is too deep");
    }
    if (stack != 1) return fail("invalid expression stack");

    const auto is = [&](size_t index, Op op) { return index < _code.size() && _code[index].op == op; };
    const auto isRecurrenceC = [&](size_t index) { return is(index, Op::C) || (is(index, Op::Constant) && _code[index].argument == CONSTANT_FIXED_C); };
    int degree = 0;
    if (_code.size() >= 5 && is(0, Op::Z) && is(1, Op::Z) && is(2, Op::Multiply)) {
        degree = 2;
        size_t cursor = 3;
        while (degree < 16 && cursor + 1 < _code.size() && is(cursor, Op::Z) && is(cursor + 1, Op::Multiply)) {
            ++degree;
            cursor += 2;
        }
        if (!(cursor + 2 == _code.size() && isRecurrenceC(cursor) && is(cursor + 1, Op::Add))) degree = 0;
    } else if (_code.size() == 4 && is(0, Op::Z) && is(1, Op::Square) && isRecurrenceC(2) && is(3, Op::Add)) {
        degree = 2;
    }
    if (degree >= 2) {
    ...
    _avx2Compatible = std::all_of(_code.begin(), _code.end(), [](const Instruction& instruction) {
        ...
        case Op::Real:
        case Op::Imaginary:
        case Op::MakeComplex: return true;
        default: return false;
        }
    });
    _batchCompatible = std::all_of(_code.begin(), _code.end(), [](const Instruction& instruction) {
        switch (instruction.op) {
        case Op::Constant:
        case Op::Z:
        case Op::C:
        ...
        case Op::Imaginary:
        case Op::MakeComplex:
        case Op::Polar: return true;
        }
        return false;
    });
    _derivativeCompatible = std::all_of(_code.begin(), _code.end(), [](const Instruction& instruction) {
        switch (instruction.op) {
        case Op::Constant:
        case Op::Z:
        case Op::C:
...
bool ExpressionProgram::isCanonicalQuadraticPlusC() const {
    ...
    const auto is = [&](size_t index, Op op) { return index < _code.size() && _code[index].op == op; };
    return (_code.size() == 5 && is(0, Op::Z) && is(1, Op::Z) && is(2, Op::Multiply) && is(3, Op::C) && is(4, Op::Add)) || (_code.size() == 4 && is(0, Op::Z) && is(1, Op::Square) && is(2, Op::C) && is(3, Op::Add));
}

ExpressionScaledResidualCapability ExpressionProgram::scaledResidualCapability() const {
    if (!_valid) return ExpressionScaledResidualCapability::Unsupported;
    ...
    for (size_t index = 0; index < _code.size(); ++index) {
        ...
        case Op::Constant:
            ...
            kind.zero = instruction.value.real() == 0.0 && instruction.value.imag() == 0.0;
            break;
        case Op::Z:
        case Op::C:
        case Op::Z0:
        ...
        case Op::OrbitInvariant:
            ...
            break;
        case Op::Iteration: kind.real = true; break;
        case Op::Negate: kind = kinds[left]; break;
        case Op::Add:
        case Op::Subtract:
            kind.real = kinds[left].real && kinds[right].real;
            kind.zero = kinds[left].zero && kinds[right].zero;
            break;
        case Op::Multiply:
            kind.real = kinds[left].real && kinds[right].real;
            kind.zero = kinds[left].zero || kinds[right].zero;
        ...
        case Op::Sinh:
        case Op::Cosh:
        case Op::Exp: hasSeries = true; break;
        case Op::Divide:
            kind.real = kinds[left].real && kinds[right].real;
            kind.zero = kinds[left].zero;
            hasMeromorphic = true;
            break;
        case Op::Tan:
        case Op::Tanh: hasMeromorphic = true; break;
        case Op::Power:
        case Op::Log:
        case Op::Log10:
        case Op::Sqrt: hasCertifiedBranch = true; break;
        case Op::Arg:
            kind.real = true;
    ...
    if (hasSeries) return ExpressionScaledResidualCapability::CertifiedEntireCandidate;
...
bool ExpressionProgram::scaledResidualRequiresTaylor() const {
    ...
    return std::any_of(_code.begin(), _code.end(), [](const Instruction& instruction) { return instruction.op == Op::Arg || instruction.op == Op::Polar; });
}

ExpressionPiecewiseQuadraticKind ExpressionProgram::piecewiseQuadraticKind() const {
    if (!_valid) return ExpressionPiecewiseQuadraticKind::None;
    auto is = [&](size_t index, Op op) { return index < _code.size() && _code[index].op == op; };
    if (_code.size() == 10 && is(0, Op::Z) && is(1, Op::Real) && is(2, Op::Abs) && is(3, Op::Z) && is(4, Op::Imaginary) && is(5, Op::Abs) && is(6, Op::MakeComplex) && is(7, Op::Square) && is(8, Op::C) && is(9, Op::Add)) return ExpressionPiecewiseQuadraticKind::BurningShip;
...
uint64_t ExpressionProgram::semanticHash() const {
    ...
    for (const Instruction& instruction : _code) {
        ...
        append(&op, sizeof(op));
        const uint8_t argument = instruction.op == Op::Constant ? 0 : instruction.argument;
...
bool ExpressionProgram::semanticallyEquivalent(const ExpressionProgram& other) const {
    if (!_valid || !other._valid || _code.size() != other._code.size()) return false;
    for (size_t i = 0; i < _code.size(); ++i) {
        ...
        if (left.op != Op::Constant && left.argument != right.argument) return false;
        ...
        if (leftReal != rightReal || leftImaginary != rightImaginary) return false;
    }
    return true;
}
...
ExpressionProgram::ExpressionProgram(const ExpressionProgram& other) : ExpressionProgram() {
    *this = other;
}

ExpressionProgram::ExpressionProgram(ExpressionProgram&& other) noexcept : ExpressionProgram() {
    *this = std::move(other);
}

ExpressionProgram& ExpressionProgram::operator=(const ExpressionProgram& other) {
...
ExpressionProgram& ExpressionProgram::operator=(ExpressionProgram&& other) noexcept {
    if (this == &other) return *this;
    const uint64_t nextRevision = _revision + 1;
...
bool ExpressionProgram::specialize(const ExpressionContext& fixed, FormulaParameter pixelParameter, ExpressionProgram& output, ExpressionError* error) const {
    if (error) *error = {};
    auto fail = [&](const char* message) {
        if (error) {
            error->position = 0;
    ...
    if (pixelParameter != FormulaParameter::C && pixelParameter != FormulaParameter::InitialZ) return fail("unsupported pixel parameter");
    ...
    auto materialize = [](Node& node) {
        if (node.constant) {
            node.code.push_back({Op::Constant, node.constantArgument, node.value});
            node.constant = false;
        }
    };
    ...
    for (const Instruction& instruction : _code) {
        ...
        if (operands == 0) {
            ...
            case Op::Z0:
                ...
                break;
            case Op::Parameter:
                if (instruction.argument >= fixed.parameters.size()) return fail("expression contains an invalid parameter");
                node.constant = true;
                node.value = fixed.parameters[instruction.argument];
                break;
            case Op::Z:
            case Op::Iteration: node.code.push_back(instruction); break;
            default: return fail("expression contains an invalid operation");
            }
            stack.push_back(std::move(node));
            continue;
        }
        if (stack.size() < (size_t)operands) return fail("invalid expression stack");
        if (operands == 1) {
        ...
        if (operands != 2) return fail("expression contains an invalid operation");
        ...
        if (left.constant && right.constant) {
            node.constant = true;
            node.value = evaluateBinary(instruction.op, left.value, right.value);
        } else {
            materialize(left);
            materialize(right);
            node.code.reserve(left.code.size() + right.code.size() + 1);
            node.code.insert(node.code.end(), std::make_move_iterator(left.code.begin()), std::make_move_iterator(left.code.end()));
            node.code.insert(node.code.end(), std::make_move_iterator(right.code.begin()), std::make_move_iterator(right.code.end()));
            node.code.push_back(instruction);
        }
        stack.push_back(std::move(node));
    }
...
bool ExpressionOrbitPlan::build(const ExpressionProgram& program, ExpressionError* error) {
    if (error) *error = {};
    auto fail = [&](const char* message) {
        if (error) {
    ...
    auto appendBytes = [](std::string& key, const void* data, size_t size) { key.append(static_cast<const char*>(data), size); };
    for (const ExpressionProgram::Instruction& instruction : program._code) {
        ...
        if (node.operands < 0 || stack.size() < (size_t)node.operands) return fail("invalid expression stack");
        if (node.operands == 1) {
        ...
        switch (instruction.op) {
        case ExpressionProgram::Op::Z: node.dependencies = DependencyZ; break;
        case ExpressionProgram::Op::Iteration: node.dependencies = DependencyIteration; break;
        case ExpressionProgram::Op::C: node.dependencies = DependencyC; break;
        case ExpressionProgram::Op::Z0: node.dependencies = DependencyZ0; break;
        case ExpressionProgram::Op::Parameter: node.dependencies = DependencyParameter; break;
        case ExpressionProgram::Op::OrbitInvariant: return fail("nested orbit plan is not supported");
        default: break;
        }
        if (node.left >= 0) {
            node.dependencies |= nodes[(size_t)node.left].dependencies;
            node.instructionCount += nodes[(size_t)node.left].instructionCount;
        }
        if (node.right >= 0) {
            node.dependencies |= nodes[(size_t)node.right].dependencies;
            node.instructionCount += nodes[(size_t)node.right].instructionCount;
        }
    ...
    const uint8_t iterationDependencies = DependencyZ | DependencyIteration;
    auto isInvariant = [&](const Node& node) { return (node.dependencies & iterationDependencies) == 0 && node.instructionCount > 1; };
    ...
    for (int index : maximalInvariantRoots) countInvariantNodes(index);

    std::unordered_map<std::string, uint8_t> invariantIndices;
    bool okay = true;
    std::function<uint8_t(int)> internInvariant;
    std::function<void(int, std::vector<ExpressionProgram::Instruction>&)> emitInvariant;
    emitInvariant = [&](int index, std::vector<ExpressionProgram::Instruction>& code) {
        if (!okay) return;
        const Node& node = nodes[(size_t)index];
        auto emitChild = [&](int child) {
            ...
            if (isInvariant(childNode) && occurrence != invariantOccurrences.end() && occurrence->second > 1) {
                uint8_t childIndex = internInvariant(child);
                code.push_back({ExpressionProgram::Op::OrbitInvariant, childIndex, {}});
            } else {
    ...
    internInvariant = [&](int index) -> uint8_t {
        ...
        uint8_t invariantIndex = static_cast<uint8_t>(candidate._invariantPrograms.size());
        invariantIndices.emplace(node.key, invariantIndex);
        candidate._invariantDependencies.push_back(node.dependencies);
        candidate._invariantOperationCount += std::count_if(invariantProgram._code.begin(), invariantProgram._code.end(), [](const ExpressionProgram::Instruction& item) { return ExpressionProgram::operandCount(item.op) > 0; });
        candidate._invariantPrograms.push_back(std::move(invariantProgram));
    ...
    emitBody = [&](int index) {
        ...
        if (isInvariant(node)) {
            ...
            candidate._body._code.push_back({ExpressionProgram::Op::OrbitInvariant, invariantIndex, {}});
            return;
        }
    ...
    emitBody(root);
    if (!okay) {
        if (!error || error->message.empty()) return fail("expression has too many orbit invariants");
        return false;
    }
    if (!candidate._body.analyze(error)) return false;
    candidate._bodyOperationCount = std::count_if(candidate._body._code.begin(), candidate._body._code.end(), [](const ExpressionProgram::Instruction& instruction) { return ExpressionProgram::operandCount(instruction.op) > 0; });
...
bool ExpressionOrbitPlan::matches(const ExpressionProgram& program) const {
    if (!_valid || !program.valid() || _source != program._source || _originalCode.size() != program._code.size()) return false;
    for (size_t i = 0; i < _originalCode.size(); ++i) {
        ...
        if (left.op != right.op || left.argument != right.argument) return false;
        uint64_t leftReal = 0, leftImaginary = 0;
        uint64_t rightReal = 0, rightImaginary = 0;
        double value = left.value.real();
        std::memcpy(&leftReal, &value, sizeof(leftReal));
...
bool ExpressionOrbitPlan::prepare(const ExpressionContext& context, Prepared& prepared) const {
    if (!_valid) return false;
    std::array<Complex, ExpressionProgram::MAX_STACK> stack;
    for (size_t i = 0; i < _invariantPrograms.size(); ++i) { prepared.values[i] = _invariantPrograms[i].evaluatePrepared(context, prepared.values.data(), i, stack.data(), stack.size()); }
    return true;
}

bool ExpressionOrbitPlan::prepare4(const ExpressionContext* contexts, uint8_t activeMask, Prepared* prepared) const {
    if (!_valid || !contexts || !prepared) return false;
    activeMask &= 0x0f;
    if (activeMask == 0) return true;
    if (activeMask != 0x0f) {
        for (int lane = 0; lane < 4; ++lane) {
            if ((activeMask & (1u << lane)) && !prepare(contexts[lane], prepared[lane])) return false;
        }
        return true;
    }
    for (size_t index = 0; index < _invariantPrograms.size(); ++index) {
        Complex values[4];
        const ExpressionProgram& invariant = _invariantPrograms[index];
        const Complex* invariants[4] = {prepared[0].values.data(), prepared[1].values.data(), prepared[2].values.data(), prepared[3].values.data()};
        bool evaluated = _programAvx2Compatible ? invariant.evaluate4Prepared(contexts, invariants, index, values) : invariant.evaluate4HybridPrepared(contexts, invariants, index, values);
        if (!evaluated) return false;
        for (int lane = 0; lane < 4; ++lane) prepared[lane].values[index] = values[lane];
    }
    return true;
}

Complex ExpressionOrbitPlan::evaluate(const ExpressionContext& context, const Prepared& prepared) const {
    std::array<Complex, ExpressionProgram::MAX_STACK> stack;
    return evaluate(context, prepared, stack.data(), stack.size());
}

Complex ExpressionOrbitPlan::evaluate(const ExpressionContext& context, const Prepared& prepared, Complex* stack, size_t capacity) const {
    if (!_valid) {
        const double nan = std::numeric_limits<double>::quiet_NaN();
        return {nan, nan};
    }
    return _body.evaluatePrepared(context, prepared.values.data(), _invariantPrograms.size(), stack, capacity);
}

bool ExpressionOrbitPlan::evaluate4(const ExpressionContext* contexts, const Prepared* prepared, Complex* outputs) const {
...
Complex ExpressionProgram::evaluate(const ExpressionContext& context, Complex* stack, size_t capacity) const {
    return evaluatePrepared(context, nullptr, 0, stack, capacity);
}

Complex ExpressionProgram::evaluatePrepared(const ExpressionContext& context, const Complex* invariants, size_t invariantCount, Complex* stack, size_t capacity) const {
    if (!_valid) return {std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::quiet_NaN()};
    if (!stack || capacity < _stackDepth) return {std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::quiet_NaN()};
    size_t top = 0;
    ...
    for (const Instruction& instruction : _code) {
        ...
        case Op::OrbitInvariant:
            if (!invariants || instruction.argument >= invariantCount) return {std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::quiet_NaN()};
        ...
        case Op::Conjugate:
        case Op::Real:
        case Op::Imaginary: unary([&](Complex value) { return evaluateUnary(instruction.op, value); }); break;
        case Op::Add:
        case Op::Subtract:
        case Op::Multiply:
        case Op::Divide:
        case Op::Power:
        case Op::MakeComplex:
        case Op::Polar: binary([&](Complex left, Complex right) { return evaluateBinary(instruction.op, left, right); }); break;
        }
    }
    return stack[0];
}

bool ExpressionProgram::evaluate4(const ExpressionContext* contexts, Complex* outputs) const {
    return evaluate4Prepared(contexts, nullptr, 0, outputs);
}

bool ExpressionProgram::evaluate4Prepared(const ExpressionContext* contexts, const Complex* const* invariants, size_t invariantCount, Complex* outputs) const {
    ...
    size_t top = 0;
    auto real = [&](auto getter) { return _mm256_set_pd(getter(contexts[3]).real(), getter(contexts[2]).real(), getter(contexts[1]).real(), getter(contexts[0]).real()); };
    auto imag = [&](auto getter) { return _mm256_set_pd(getter(contexts[3]).imag(), getter(contexts[2]).imag(), getter(contexts[1]).imag(), getter(contexts[0]).imag()); };
    for (const Instruction& instruction : _code) {
        switch (instruction.op) {
        case Op::Constant: stack[top++] = {_mm256_set1_pd(instruction.value.real()), _mm256_set1_pd(instruction.value.imag())}; break;
        case Op::Z: stack[top++] = {real([](const ExpressionContext& c) { return c.z; }), imag([](const ExpressionContext& c) { return c.z; })}; break;
        case Op::C: stack[top++] = {real([](const ExpressionContext& c) { return c.c; }), imag([](const ExpressionContext& c) { return c.c; })}; break;
        case Op::Z0: stack[top++] = {real([](const ExpressionContext& c) { return c.z0; }), imag([](const ExpressionContext& c) { return c.z0; })}; break;
        case Op::Iteration: stack[top++] = {_mm256_set_pd((double)contexts[3].iteration, (double)contexts[2].iteration, (double)contexts[1].iteration, (double)contexts[0].iteration), _mm256_setzero_pd()}; break;
        case Op::Parameter: {
            int p = instruction.argument;
            stack[top++] = {_mm256_set_pd(contexts[3].parameters[p].real(), contexts[2].parameters[p].real(), contexts[1].parameters[p].real(), contexts[0].parameters[p].real()), _mm256_set_pd(contexts[3].parameters[p].imag(), contexts[2].parameters[p].imag(), contexts[1].parameters[p].imag(), contexts[0].parameters[p].imag())};
            break;
        }
        case Op::OrbitInvariant: {
            if (!invariants || instruction.argument >= invariantCount) return false;
            const size_t index = instruction.argument;
            stack[top++] = {_mm256_set_pd(invariants[3][index].real(), invariants[2][index].real(), invariants[1][index].real(), invariants[0][index].real()), _mm256_set_pd(invariants[3][index].imag(), invariants[2][index].imag(), invariants[1][index].imag(), invariants[0][index].imag())};
            break;
        }
        case Op::Negate:
        ...
        case Op::Multiply: {
            ...
            if (instruction.op == Op::Add) {
                ...
                __m256d re = _mm256_sub_pd(_mm256_mul_pd(left.re, right.re), _mm256_mul_pd(left.im, right.im));
                __m256d im = _mm256_add_pd(_mm256_mul_pd(left.re, right.im), _mm256_mul_pd(left.im, right.re));
                left.re = re;
                left.im = im;
            }
            break;
        ...
        case Op::Square: {
            ...
            __m256d re = _mm256_sub_pd(_mm256_mul_pd(value.re, value.re), _mm256_mul_pd(value.im, value.im));
            __m256d im = _mm256_add_pd(_mm256_mul_pd(value.re, value.im), _mm256_mul_pd(value.im, value.re));
            value.re = re;
            value.im = im;
            break;
        }
        case Op::Conjugate: stack[top - 1].im = _mm256_xor_pd(stack[top - 1].im, _mm256_set1_pd(-0.0)); break;
        case Op::Real: stack[top - 1].im = _mm256_setzero_pd(); break;
        case Op::Imaginary:
...
bool ExpressionProgram::evaluate4HybridPrepared(const ExpressionContext* contexts, const Complex* const* invariants, size_t invariantCount, Complex* outputs, int vectorTranscendentalMode) const {
    ...
    auto binary = [&](auto function) {
        ...
        for (int lane = 0; lane < 4; ++lane) {
            Complex result = function(Complex{leftRe[lane], leftIm[lane]}, Complex{rightRe[lane], rightIm[lane]});
    ...
    for (const Instruction& instruction : _code) {
        ...
        case Op::Polar:
            binary([](Complex a, Complex b) {
                const double nan = std::numeric_limits<double>::quiet_NaN();
                if (a.imag() != 0.0 || b.imag() != 0.0 || !std::isfinite(a.real()) || a.real() < 0.0 || !std::isfinite(b.real())) return Complex{nan, nan};
...
bool ExpressionProgram::evaluateWithDerivative(const ExpressionContext& context, const ExpressionDerivativeSeed& seed, Complex& value, Complex& derivative) const {
    value = derivative = {std::numeric_limits<double>::quiet_NaN(), std::numeric_limits<double>::quiet_NaN()};
    ...
    for (const Instruction& instruction : _code) {
        ...
        case Op::Parameter: push(context.parameters[instruction.argument], seed.parameters[instruction.argument]); break;
        case Op::Negate: unary([](Dual a) { return Dual{-a.value, -a.derivative}; }); break;
        case Op::Add: binary([](Dual a, Dual b) { return Dual{a.value + b.value, a.derivative + b.derivative}; }); break;
        case Op::Subtract: binary([](Dual a, Dual b) { return Dual{a.value - b.value, a.derivative - b.derivative}; }); break;
        case Op::Multiply: binary([](Dual a, Dual b) { return Dual{a.value * b.value, a.derivative * b.value + a.value * b.derivative}; }); break;
        case Op::Divide:
            binary([&](Dual a, Dual b) {
                ...
                return Dual{result, (a.derivative - result * b.derivative) / b.value};
            });
            break;
        case Op::Power:
            binary([&](Dual a, Dual b) {
                bool constantInteger = b.derivative == Complex{} && b.value.imag() == 0.0 && std::isfinite(b.value.real()) && std::trunc(b.value.real()) == b.value.real();
                if (constantInteger && a.value == Complex{} && b.value.real() >= 0.0) {
                    if (b.value.real() == 0.0) return Dual{Complex{1.0, 0.0}, {}};
                    int exponent = b.value.real() <= 1.0 ? 1 : 2;
                    return Dual{{}, exponent == 1 ? a.derivative : Complex{}};
                }
                if (a.value == Complex{}) {
                    domain = false;
                    return Dual{};
                ...
                Complex d;
                if (constantInteger) {
                    d = b.value * std::pow(a.value, b.value - Complex{1.0, 0.0}) * a.derivative;
                } else {
                    if (a.value.imag() == 0.0 && a.value.real() < 0.0) {
                        domain = false;
                        return Dual{};
                    }
                    d = result * (b.derivative * std::log(a.value) + b.value * a.derivative / a.value);
                }
                return Dual{result, d};
            });
            break;
        case Op::Square: unary([](Dual a) { return Dual{a.value * a.value, a.derivative * a.value + a.value * a.derivative}; }); break;
        case Op::Sin: unary([](Dual a) { return Dual{std::sin(a.value), std::cos(a.value) * a.derivative}; }); break;
        case Op::Cos: unary([](Dual a) { return Dual{std::cos(a.value), -std::sin(a.value) * a.derivative}; }); break;
        case Op::Tan:
            unary([](Dual a) {
                ...
                if (std::abs(a.value.imag()) < 300.0) {
                    ...
                    Complex scaled{std::cos(a.value.real()) * (1.0 + q), -sign * std::sin(a.value.real()) * (1.0 - q)};
                    factor = q == 0.0 ? Complex{} : 4.0 * q / (scaled * scaled);
                }
                return Dual{result, factor * a.derivative};
            });
            break;
        case Op::Sinh: unary([](Dual a) { return Dual{std::sinh(a.value), std::cosh(a.value) * a.derivative}; }); break;
        case Op::Cosh: unary([](Dual a) { return Dual{std::cosh(a.value), std::sinh(a.value) * a.derivative}; }); break;
        case Op::Tanh:
            unary([](Dual a) {
                ...
                if (std::abs(a.value.real()) < 300.0) {
                    ...
                    Complex scaled{std::cos(a.value.imag()) * (1.0 + q), sign * std::sin(a.value.imag()) * (1.0 - q)};
                    factor = q == 0.0 ? Complex{} : 4.0 * q / (scaled * scaled);
                }
                return Dual{result, factor * a.derivative};
            });
        ...
        case Op::Log:
            unary([&](Dual a) {
                if (a.value == Complex{} || (a.value.imag() == 0.0 && a.value.real() < 0.0)) {
                    domain = false;
                    return Dual{};
                }
        ...
        case Op::Log10:
            unary([&](Dual a) {
                ...
                return Dual{std::log10(a.value), a.derivative / (a.value * ln10)};
            });
            break;
        case Op::Sqrt:
            unary([&](Dual a) {
    ...
    return std::isfinite(value.real()) && std::isfinite(value.imag()) && std::isfinite(derivative.real()) && std::isfinite(derivative.imag());
}
