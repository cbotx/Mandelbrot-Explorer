...
struct alignas(32) ExpressionJitInput4 {
    enum VectorIndex : size_t { Z_RE = 0,
                                Z_IM,
                                C_RE,
                                C_IM,
                                Z0_RE,
...
struct alignas(32) ExpressionJitInvariantInput4 {
    static constexpr size_t VECTOR_COUNT = 2 * ExpressionOrbitPlan::MAX_INVARIANTS;
    double vectors[VECTOR_COUNT][4];

    void setPreparedLane(int lane, const ExpressionOrbitPlan& plan, const ExpressionOrbitPlan::Prepared& prepared);
};
...
class ExpressionJit4 {
  public:
    ...
    bool compile(const ExpressionOrbitPlan& plan, std::string* error = nullptr);
    void reset();
    bool valid() const;
    ...
    void evaluate(const ExpressionJitInput4& input, const ExpressionJitInvariantInput4* invariants, ExpressionJitOutput4& output) const;
    bool evaluate(const ExpressionContext* contexts, Complex* outputs) const;
    bool evaluateOrbit(const ExpressionContext* contexts, int lanes, int mxit, double bailout, float* results, const std::atomic_bool* halt = nullptr, const ExpressionOrbitPlan* plan = nullptr) const;

  private:
    struct Impl;
    std::unique_ptr<Impl> _impl;
    bool compileProgram(const ExpressionProgram& program, size_t invariantCount, const std::string* planKey, std::string* error);
};
