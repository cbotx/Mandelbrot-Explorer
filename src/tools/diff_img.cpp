...
static inline float color_func(float it) {
    if (g_method & ColoringMethod::EXTERIOR_DIST_EST) return tanhf(it / color_density * 5.0f);
...
static void colorImage(const float* buf, int n, std::vector<uint8_t>& img) {
    img.resize(n * 3);
    for (int i = 0; i < n; ++i) getColor(buf[i], img[i * 3], img[i * 3 + 1], img[i * 3 + 2]);
}
...
static DiffStats diffImage(const float* pert, const float* gt, const std::vector<uint8_t>& pc, const std::vector<uint8_t>& gc, int n, int amp, std::vector<uint8_t>& out) {
    out.assign(n * 3, 0);
    DiffStats s{0, 0, 0, 0};
    double sum = 0;
    for (int i = 0; i < n; ++i) {
...
int main(int argc, char** argv) {
    ...
    if (const char* cxs = getenv("MANDEL_CX")) {
        ...
        if (sa.find_first_of("eE.") != std::string::npos) {
            ...
            int zeros = e10 - frac;
            scl = sign + mant;
            if (zeros > 0) scl.append(zeros, '0');
        } else {
            int se = atoi(sa.c_str());
            scl = "1";
            for (int i = 0; i < se; ++i) scl += "0";
        }
        cases.clear();
        cases.push_back({"custom", cxs, getenv("MANDEL_CY"), scl, mxarg ? mxarg : 100000, Warg ? Warg : 160, Harg ? Harg : 120});
        which = "custom";
    }

    printf("mode: %s\n", c_method ? "EDE" : "raw-iter");
    printf("%-10s %6s %8s  %-18s %-18s\n", "case", "WxH", "mxit", "BLA-off vs GT", "BLA-on vs GT");
    ...
    for (auto& tc : cases) {
        ...
        if (skipOracle) {
            std::vector<uint8_t> onc2;
            colorImage(on.data(), n, onc2);
            std::string d2 = "diffs\\";
        ...
        printf("%-10s %3dx%-3d %8d  %3ld /%5.1f /%5.2f   %3ld /%5.1f /%5.2f\n", tc.name, tc.W, tc.H, tc.mxit, soff.class_mismatch, soff.max_cdiff, soff.mean_cdiff, son.class_mismatch, son.max_cdiff, son.mean_cdiff);
        fflush(stdout);
