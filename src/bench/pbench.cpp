...
static BLATable buildBLA(const Ref& ref, double eps, double dcmax, int minskip) {
    BLATable T;
    T.minlevel = 0;
    while ((1 << (T.minlevel + 1)) <= minskip) ++T.minlevel;
    int reflen = ref.len;
...
static int tryBLA(const BLATable& T, const Ref& ref, int s, double& dzr, double& dzi, double dcr, double dci, int reflen) {
...
static double pixelPert(const Ref& ref, const BLATable* T, double dcr, double dci, int maxit, long long& skips, long long& applies, long long& normals) {
...
static double pixelBrute(mpf_t cr, mpf_t ci, int maxit) {
    ...
    for (int n = 0; n < maxit; ++n) {
        ...
        double zrad = zr * zr + zi * zi;
        if (zrad > ESC2) {
            res = (double)(n + 1) + 1.0 - log(log(zrad) / 2.0 / LOG2) / LOG2;
            break;
        }
    }
    mpf_clears(xr, xi, xr2, xi2, t, (mpf_ptr)0);
...
static FloatExp mpf_to_fe(mpf_t x) {
    ...
    long ex;
    double d = mpf_get_d_2exp(&ex, x); // x = d * 2^ex, 0.5<=|d|<1
...
static RefFE computeRefFE(mpf_t cr, mpf_t ci, int maxit) {
    ...
    R.zr.assign(maxit + 2, 0.0);
    R.zi.assign(maxit + 2, 0.0);
    R.fr.assign(maxit + 2, FloatExp{0.0, 0});
    R.fi.assign(maxit + 2, FloatExp{0.0, 0});
    mpf_t xr, xi, xr2, xi2, t;
    mpf_inits(xr, xi, xr2, xi2, t, (mpf_ptr)0);
    mpf_set_ui(xr, 0);
    mpf_set_ui(xi, 0);
    ...
    for (; n < maxit; ++n) {
        ...
        mpf_mul_2exp(xi, t, 1);
        mpf_add(xi, xi, ci);
        R.fr[n + 1] = mpf_to_fe(xr);
        R.fi[n + 1] = mpf_to_fe(xi);
        R.zr[n + 1] = fe_to_double(R.fr[n + 1]);
        R.zi[n + 1] = fe_to_double(R.fi[n + 1]);
        double zr = R.zr[n + 1], zi = R.zi[n + 1];
        if (zr * zr + zi * zi > ESC2) {
            R.len = n + 1;
            mpf_clears(xr, xi, xr2, xi2, t, (mpf_ptr)0);
...
int main(int argc, char** argv) {
    if (argc < 3) {
        fprintf(stderr,
                "usage: %s <location> <scaleExp> [W H maxit oracleStep]\n"
    ...
    // scale = 10^scaleExp ; half-width dw = 2/scale ; dx = 2*dw/(W-1)
    {
        std::string s = "1";
        s.append(scaleExp, '0');
        mpf_set_str(scale, s.c_str(), 10);
    }
    mpf_set_ui(dw, 2);
    mpf_div(dw, dw, scale);
    mpf_mul_ui(dx, dw, 2);
    mpf_div_ui(dx, dx, W - 1);
    ...
    if (getenv("PB_FE")) {
        ...
        auto dcFE = [&](int i, int j, FloatExp& fr, FloatExp& fi) {
            mpf_t off;
            mpf_init(off);
            mpf_set_si(off, 2 * j - (W - 1));
            mpf_mul(off, off, dx);
            mpf_div_ui(off, off, 2);
            fr = mpf_to_fe(off);
            mpf_set_si(off, 2 * i - (H - 1));
            mpf_mul(off, off, dx);
            mpf_div_ui(off, off, 2);
            fi = mpf_to_fe(off);
            mpf_clear(off);
        ...
        for (int i = 0; i < H; i += ostep)
            for (int j = 0; j < W; j += ostep) {
                ...
                ++samp;
                bool ti = truth < 0, fi2 = f < 0;
                if (ti != fi2)
                    ++cls;
                else if (!ti) {
                    double d = fabs(truth - f);
                    sumd += d;
                    if (d > maxd) {
                        maxd = d;
                        wi = i;
    ...
    printf("\n  -- BLA skip stats --\n");
    printf("    applies=%lld  skipped-iters=%lld  normal-steps=%lld  avg-skip=%.1f  skip-frac=%.1f%%\n", applies, skips, normals, applies ? (double)skips / applies : 0.0, totalsteps ? 100.0 * skips / totalsteps : 0.0);
    ...
    if (getenv("PB_BMP")) {
        ...
        if (f) {
            ...
            for (int i = 0; i < H; ++i)
                for (int j = 0; j < W; ++j) {
                    unsigned char px = (unsigned char)shade(itC[i * W + j]);
                    fwrite(&px, 1, 1, f);
                }
            fclose(f);
            printf("\n  wrote %s\n", fn.c_str());
