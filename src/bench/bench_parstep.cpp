...
static uint64_t rnd64() { uint64_t x=0; for(int i=0;i<4;++i) x=(x<<16)^(uint64_t)(rand()&0xffff); return x; }
...
int main() {
    ...
    // 1) one big multiply
    auto t0=Clock::now();
    for(long k=0;k<N;++k){ mpn_mul_n((mp_limb_t*)full.data(),(mp_limb_t*)a.data(),(mp_limb_t*)b.data(),L); a[0]^=full[L]; }
    auto t1=Clock::now();
    double mul_ns = ns_per(t0,t1,N);

    // 2) one full complex-square orbit step (2 muls Karatsuba-style + adds) via mpf
    mpf_t zr,zi,cr,ci,t,u,v; mp_bitcnt_t p=(mp_bitcnt_t)64*L;
    mpf_init2(zr,p);mpf_init2(zi,p);mpf_init2(cr,p);mpf_init2(ci,p);
    mpf_init2(t,p);mpf_init2(u,p);mpf_init2(v,p);
    mpf_set_d(zr,0.1);mpf_set_d(zi,0.2);mpf_set_d(cr,-0.5);mpf_set_d(ci,0.0);
    t0=Clock::now();
    for(long k=0;k<N;++k){
        mpf_add(t,zr,zi); mpf_sub(u,zr,zi); mpf_mul(v,t,u);      // zr^2 - zi^2
        mpf_mul(t,zr,zi);                                        // zr*zi
        mpf_add(zr,v,cr); mpf_add(t,t,t); mpf_add(zi,t,ci);
    }
    t1=Clock::now();
    double step_ns = ns_per(t0,t1,N);
    mpf_clear(zr);mpf_clear(zi);mpf_clear(cr);mpf_clear(ci);mpf_clear(t);mpf_clear(u);mpf_clear(v);
