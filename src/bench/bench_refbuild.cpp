...
int main(int argc, char** argv) {
    ...
    a[L - 1] &= 0x3fffffffffffffffULL;
    b[L - 1] &= 0x3fffffffffffffffULL; // |.|<2
    memcpy(Cr, a, L * sizeof(mp_limb_t));
    memcpy(Ci, b, L * sizeof(mp_limb_t));
    ...
    for (int r = 0; r < reps; ++r) {
        mp_limb_t x[64], y[64];
        memcpy(x, a, L * sizeof(mp_limb_t));
        memcpy(y, b, L * sizeof(mp_limb_t));
        double t0 = nows();
        for (int i = 0; i < N; ++i) {
    ...
    // Correctness spot-check: mul_hi vs full mpn_mul_n high half.
    {
        ...
        auto mh = [&](mp_limb_t* o, const mp_limb_t* X, const mp_limb_t* Y, int g) {
            ...
            for (int i = 0; i < L; ++i) {
                ...
                int len = L - jlo;
                int ai = (i + jlo) - base;
                mp_limb_t cy = mpn_addmul_1(acc + ai, Y + jlo, len, X[i]);
                if (ai + len < accsz) mpn_add_1(acc + ai + len, acc + ai + len, accsz - (ai + len), cy);
            }
        ...
        long long maxerr = 0;
        for (int k = 0; k < L; ++k) {
            long long e = (long long)(hi[k] - full[L + k]);
            if (e < 0) e = -e;
