...
static void getColor(float it, float& r, float& g, float& b) {
    ...
    int x = (int)(colorFunction(it) * CP) % CP;
    if (x < 0) x += CP;
    r = color_map[0][x];
    g = color_map[1][x];
    b = color_map[2][x];
...
int main(int argc, char** argv) {
    if (argc < 9) {
        fprintf(stderr, "usage: palette_preview out W H cx cy scaleExp mxit palIdx [density] [SS]\n");
        return 1;
    }
    const char* out = argv[1];
    int W = atoi(argv[2]), H = atoi(argv[3]);
    const char* cx = argv[4];
    const char* cy = argv[5];
    const char* scaleArg = argv[6];
    int mxit = atoi(argv[7]);
    int palIdx = atoi(argv[8]);
    ...
    if (sa.find_first_of("eE.") != std::string::npos) {
        ...
        if (zeros >= 0) {
            scale = sign + mant;
            scale.append(zeros, '0');
        } else {
            int point = (int)mant.size() + zeros; // decimal point position from the left
            if (point <= 0)
                scale = sign + "0." + std::string(-point, '0') + mant;
            else
                scale = sign + mant.substr(0, point) + "." + mant.substr(point);
        }
    } else {
        int se = atoi(sa.c_str());
        scale = "1";
        for (int i = 0; i < se; ++i) scale += "0";
    }
    ...
    for (int i = 0; i < H; ++i)
        for (int j = 0; j < W; ++j) {
            ...
            for (int a = 0; a < SS; ++a)
                for (int b = 0; b < SS; ++b) {
                    float r, g, bb;
                    getColor(iter[(i * SS + a) * Ws + (j * SS + b)], r, g, bb);
                    lr += toLin(r);
                    lg += toLin(g);
                    lb += toLin(bb);
                }
