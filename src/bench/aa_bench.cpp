...
static float* renderSS(int W, int H, int mxit, int sub, int precision, mpf_t mcx, mpf_t mcy, mpf_t msc, const char* kval) {
...
static bool colorBlock(const float* it, int W, int sub, int i, int j, float& r, float& g, float& b) {
    int c = sub / 2, stride = W * sub;
    int idx = (i * sub + c) * stride + (j * sub + c);
    if (it[idx] == EMPTYPIXEL) {
        r = g = b = 0;
        return false;
    }
    if (it[idx - (stride + 1) * c] != EMPTYPIXEL) { // flagged
        ...
        for (int a = -c; a <= c; ++a)
            for (int bb = -c; bb <= c; ++bb) {
                float rr, gg, bbb;
                getColor(it[idx + a * stride + bb], rr, gg, bbb);
                rl += srgb2linF(rr);
                gl += srgb2linF(gg);
                bl += srgb2linF(bbb);
                n++;
...
static void colorBlockRMS(const float* it, int W, int sub, int i, int j, float& r, float& g, float& b) {
    int c = sub / 2, stride = W * sub;
    int idx = (i * sub + c) * stride + (j * sub + c);
    if (it[idx] == EMPTYPIXEL) {
        r = g = b = 0;
    ...
    if (it[idx - (stride + 1) * c] != EMPTYPIXEL) {
        ...
        r = (float)std::sqrt(r2 / n);
        g = (float)std::sqrt(g2 / n);
        b = (float)std::sqrt(b2 / n);
        return;
    }
...
static bool colorBlockAA(const float* it, int W, int H, int sub, int i, int j, float& r, float& g, float& b) {
    ...
    for (int a = -c; a <= c; ++a)
        for (int bb = -c; bb <= c; ++bb) {
            ...
            float rr, gg, bbb;
            getColorAA(v, vL, vR, vU, vD, rr, gg, bbb);
            r2 += srgb2linF(rr);
            g2 += srgb2linF(gg);
            b2 += srgb2linF(bbb);
            n++;
...
static void sampleGrid(const float* it, int W, int GTsub, int s, int i, int j, float& r, float& g, float& b, double& sd) {
    int stride = W * GTsub;
    double rl = 0, gl = 0, bl = 0, s2 = 0, sm = 0;
    int n = 0;
    for (int k = 0; k < s; ++k)
        for (int l = 0; l < s; ++l) {
    ...
    g = (float)enc255(gl / n);
    b = (float)enc255(bl / n);
    double vv = s2 / n - (sm / n) * (sm / n);
    sd = std::sqrt(vv > 0 ? vv : 0);
}

int main(int argc, char** argv) {
    if (argc < 5) {
        fprintf(stderr, "usage: aa_bench cx cy scale mxit [W H GTsub]\n");
        return 1;
    }
    const char* cx = argv[1];
    const char* cy = argv[2];
    const char* scaleStr = argv[3];
    ...
    int subA = 3;
    {
        const char* e = getenv("MANDEL_ADAPT_SUB");
        if (e) subA = atoi(e);
    }
    if (subA % 2 == 0) subA++;
    const char* Kstr = getenv("MANDEL_SS_K");
    if (!Kstr) Kstr = "8";
    ...
    mpf_init_set_str(msc, scaleStr, 10);
    ...
    double sP = 0, sA = 0, sF = 0, sD = 0, sE = 0, mP = 0, mA = 0, mF = 0, mD = 0, mE = 0;
    long cnt = 0, ssPix = 0, gtSS = 0;
    ...
    for (int i = 0; i < H; ++i)
        for (int j = 0; j < W; ++j) {
            // --- ground truth
            float Gr, Gg, Gb;
            bool gflag = colorBlock(gtIt, W, GTsub, i, j, Gr, Gg, Gb);
            if (gflag) gtSS++;
            ...
            float pr, pg, pb;
            getColor(vC, pr, pg, pb);
            float ar, ag, ab;
            getColorAA(vC, vL, vR, vU, vD, ar, ag, ab);
            uint8_t* pP = &imgP[(i * W + j) * 3];
            pP[0] = (uint8_t)(pr + .5f);
            pP[1] = (uint8_t)(pg + .5f);
            pP[2] = (uint8_t)(pb + .5f);
            uint8_t* pA = &imgA[(i * W + j) * 3];
            pA[0] = (uint8_t)(ar + .5f);
            pA[1] = (uint8_t)(ag + .5f);
            pA[2] = (uint8_t)(ab + .5f);
            ...
            // --- full 3x SS
            float Fr, Fg, Fb;
            colorBlock(f3It, W, 3, i, j, Fr, Fg, Fb);
            uint8_t* pF = &imgF[(i * W + j) * 3];
            pF[0] = (uint8_t)(Fr + .5f);
            pF[1] = (uint8_t)(Fg + .5f);
            pF[2] = (uint8_t)(Fb + .5f);
            ...
            float Dr, Dg, Db;
            {
                ...
                if (adIt[ia] == EMPTYPIXEL) {
                    Dr = Dg = Db = 0;
                } else if (adIt[ia - (sA_stride + 1) * cA] != EMPTYPIXEL) {
                    colorBlock(adIt, W, subA, i, j, Dr, Dg, Db);
                    ssPix++;
                } else {
    ...
    {
        const int NT = 5;
        const double Th[NT] = {6, 10, 16, 24, 1e9};
        ...
        for (int i = 0; i < H; ++i)
            for (int j = 0; j < W; ++j) {
                ...
                if (!gtFlag[i * W + j]) { // uniform: 1 eval everywhere
                    float r, g, b;
                    double sd0;
                    sampleGrid(gtIt, W, GTsub, 1, i, j, r, g, b, sd0);
                    double e = er(r, g, b);
                    u3 += e;
                    u5 += e;
                    u3ev += 1;
                ...
                float r3, g3, b3;
                double sd3;
                sampleGrid(gtIt, W, GTsub, 3, i, j, r3, g3, b3, sd3);
                float r5, g5, b5;
                double sd5;
                sampleGrid(gtIt, W, GTsub, 5, i, j, r5, g5, b5, sd5);
                u3 += er(r3, g3, b3);
                u3ev += 9;
                u5 += er(r5, g5, b5);
                u5ev += 25;
        ...
        for (int t = 0; t < NT; ++t) printf("   progressive T=%-4.0f: %.1f evals/px   err=%.2f\n", Th[t], prEv[t] / cnt, prErr[t] / cnt);
    }
