...
static uint64_t rnd64() {
    uint64_t x = 0; for (int i = 0; i < 4; ++i) x = (x << 16) ^ (uint64_t)(rand() & 0xffff); return x;
}
...
static inline void mulhigh_addmul(uint64_t* r, const uint64_t* a, const uint64_t* b, int L, int GUARD) {
    ...
    for (int i = 0; i < L; ++i) {
        ...
        int n = L - jstart;
        int t = i + jstart - lo;
        mp_limb_t carry = mpn_addmul_1((mp_limb_t*)(r + t), (const mp_limb_t*)(b + jstart),
                                       (mp_size_t)n, (mp_limb_t)a[i]);
        int u = t + n; mp_limb_t cc = carry;
        while (cc && u <= nout) { unsigned char c = _addcarry_u64(0, r[u], cc, &r[u]); cc = c; ++u; }
    }
}
...
static inline void sqrhigh_addmul(uint64_t* r, const uint64_t* a, int L, int GUARD) {
    ...
    for (int i = 0; i < L - 1; ++i) {
        int jstart = i + 1; if (i + jstart < lo) jstart = lo - i; if (jstart < i + 1) jstart = i + 1;
        int n = L - jstart; if (n <= 0) continue;
        int t = i + jstart - lo;
    ...
    for (int i = 0; i < L; ++i) {
        int col = 2 * i; if (col + 1 < lo) continue;
...
static void study(int L, long iters) {
    ...
    for (int i = 0; i < L; ++i) { a[i] = rnd64(); b[i] = rnd64(); }

    auto t0 = Clock::now();
    for (long k = 0; k < iters; ++k) { mpn_mul_n((mp_limb_t*)full.data(), (mp_limb_t*)a.data(), (mp_limb_t*)b.data(), L); a[0] ^= full[L]; }
    auto t1 = Clock::now();
    double full_ns = ns_per(t0, t1, iters);

    // (2) mpn_sqr
    t0 = Clock::now();
    for (long k = 0; k < iters; ++k) { mpn_sqr((mp_limb_t*)full.data(), (mp_limb_t*)a.data(), L); a[0] ^= full[L]; }
    t1 = Clock::now();
    double sqr_ns = ns_per(t0, t1, iters);

    // (3) schoolbook high-half
    t0 = Clock::now();
    for (long k = 0; k < iters; ++k) { mulhigh_school(hi.data(), a.data(), b.data(), L); a[0] ^= hi[2]; }
    t1 = Clock::now();
    double hh_ns = ns_per(t0, t1, iters);

    // (3b) tuned ADX high-half (MULX + ADCX/ADOX), GUARD=2
    t0 = Clock::now();
    for (long k = 0; k < iters; ++k) { mulhigh_adx(hx.data(), a.data(), b.data(), L, 2); a[0] ^= hx[2]; }
    t1 = Clock::now();
    double ax_ns = ns_per(t0, t1, iters);

    // (3c) high-half via GMP's own mpn_addmul_1 per row, GUARD=2
    t0 = Clock::now();
    for (long k = 0; k < iters; ++k) { mulhigh_addmul(hx.data(), a.data(), b.data(), L, 2); a[0] ^= hx[2]; }
    t1 = Clock::now();
    double am_ns = ns_per(t0, t1, iters);

    // (4) mpf_mul at prec = 64*L bits (what the reference build actually uses)
    mpf_t ma, mb, mr; mp_bitcnt_t p = (mp_bitcnt_t)64 * L;
    mpf_init2(ma, p); mpf_init2(mb, p); mpf_init2(mr, p);
    mpf_set_ui(ma, 3); mpf_div_ui(ma, ma, 7); mpf_set_ui(mb, 5); mpf_div_ui(mb, mb, 11);
    t0 = Clock::now();
    ...
    printf("  L=%2d (%5db): mpf %7.1f | mpn_mul_n %7.1f (%.2fx) | mpn_sqr %6.1f (%.2fx) | ADX_hi %7.1f (%.2fx) | addmul_hi %7.1f (%.2fx)\n",
           L, 64 * L, mpf_ns, full_ns, mpf_ns / full_ns, sqr_ns, mpf_ns / sqr_ns,
           ax_ns, mpf_ns / ax_ns, am_ns, mpf_ns / am_ns);
}
...
static void checkHigh(int L) {
    ...
    int badS = 0; { const int lo = L - 2; for (int col = lo; col < 2 * L; ++col) if (full[col] != hi[col - lo]) ++badS; }
    // ADX with GUARD=2: kept high L limbs are columns [L-1 .. 2L-2]; verify those.
    const int GUARD = 2; int lo = L - 1 - GUARD; if (lo < 0) lo = 0;
    mulhigh_adx(hx.data(), a.data(), b.data(), L, GUARD);
    int badA = 0; for (int col = L - 1; col < 2 * L; ++col) if (full[col] != hx[col - lo]) ++badA;
    std::vector<uint64_t> hm(2 * L + 4);
    mulhigh_addmul(hm.data(), a.data(), b.data(), L, GUARD);
    int badM = 0; for (int col = L - 1; col < 2 * L; ++col) if (full[col] != hm[col - lo]) ++badM;
    ...
    int badQ = 0; for (int col = L - 1; col < 2 * L; ++col) if (fsq[col] != hs[col - lo]) ++badQ;
    printf("  correctness L=%2d: ADX_hi %s  addmul_hi %s  sqrhigh %s\n", L,
           badA == 0 ? "EXACT" : "approx", badM == 0 ? "EXACT" : "approx", badQ == 0 ? "EXACT" : "approx");
}
...
static void complexStep(int L, long iters) {
    ...
    for (int i = 0; i < L; ++i) { a[i] = rnd64() >> 2; b[i] = rnd64() >> 2; }
    const int G = 3;
    auto t0 = Clock::now();
    ...
    auto t1 = Clock::now(); double mul2 = ns_per(t0, t1, iters);
    t0 = Clock::now();
    for (long k = 0; k < iters; ++k) {
    ...
    t1 = Clock::now(); double sqr3 = ns_per(t0, t1, iters);
