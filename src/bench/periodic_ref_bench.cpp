...
static void complex_square_add_to(Cmpf& out, const Cmpf& z, const Cmpf& c, Scratch& s) {
    mpf_add(s.a.v, z.re, z.im);
    mpf_sub(s.b.v, z.re, z.im);
    mpf_mul(s.c.v, z.re, z.im);
    mpf_mul(s.d.v, s.a.v, s.b.v);
...
static void evaluate_period(const Cmpf& c, uint64_t period, Cmpf& z, Cmpf& dz, Scratch& s) {
    complex_zero(z);
    complex_zero(dz);
...
static SearchResult find_period(const Cmpf& center, mpf_srcptr radius, uint64_t max_period) {
    Cmpf z;
    Scratch s;
    ...
    const mp_bitcnt_t guard = g_precision_bits > 64 ? g_precision_bits - 32 : g_precision_bits / 2;
...
static NewtonResult refine_nucleus(Cmpf& c, uint64_t period, uint64_t max_iterations, mpf_ptr residual) {
    Cmpf z, dz, delta, previous;
    Scratch s;
    Mpf denominator, step2, tolerance, tolerance2;

    NewtonResult result;
    result.convergence_bits = g_precision_bits > 64 ? g_precision_bits - 64 : g_precision_bits / 2;
    ...
    for (uint64_t it = 0; it < max_iterations; ++it) {
        ...
        const bool unchanged = mpf_cmp(c.re, previous.re) == 0 && mpf_cmp(c.im, previous.im) == 0;
...
class ShadowStorage {
  public:
    ShadowStorage(size_t count, bool deep) : doubles_(count), floatexp_(deep ? count : 0), deep_(deep) {}
...
static uint64_t build_reference(size_t count, const Cmpf& c, Cmpf (&z)[2], Scratch& s, ShadowStorage* shadows) {
    complex_zero(z[0]);
    complex_zero(z[1]);
    uint64_t checksum = 0;
    for (size_t i = 0; i < count; ++i) {
        ...
        checksum ^= static_cast<uint64_t>(mpf_sgn(z[current].re) + 2) + (static_cast<uint64_t>(mpf_sgn(z[current].im) + 2) << 3) + i;
    }
...
static uint64_t replay_period(const ShadowStorage& period, size_t mxit) {
    ...
    for (size_t i = 0; i < mxit; ++i) {
        ...
        checksum ^= static_cast<uint64_t>(std::signbit(z.real())) + (static_cast<uint64_t>(std::signbit(z.imag())) << 1) + i;
        if (period.deep()) {
            const FeShadow& fe = period.fe_value(index);
            checksum ^= static_cast<uint64_t>(fe.re.e) + (static_cast<uint64_t>(fe.im.e) << 1) + (static_cast<uint64_t>(std::signbit(fe.re.m)) << 2) + (static_cast<uint64_t>(std::signbit(fe.im.m)) << 3);
        }
...
static long double estimated_gmp_orbit_bytes() {
    const uint64_t limbs = (static_cast<uint64_t>(g_precision_bits) + GMP_NUMB_BITS - 1) / GMP_NUMB_BITS + 1;
    const long double mpf_bytes = static_cast<long double>(sizeof(__mpf_struct)) + limbs * static_cast<long double>(sizeof(mp_limb_t));
...
static BenchmarkTimes benchmark_references(const Cmpf& view_center, const Cmpf& nucleus, uint64_t period, size_t mxit, int repetitions, bool deep) {
    BenchmarkTimes best;
    Cmpf z[2];
    Scratch s;
    ...
    for (int rep = 0; rep < repetitions; ++rep) {
        {
            ...
            best.full_shadow_allocate = std::min(best.full_shadow_allocate, end_allocate - start_allocate);
            best.full_gmp_only = std::min(best.full_gmp_only, end_gmp_only - end_allocate);
            best.full_build_store = std::min(best.full_build_store, end_build_store - end_gmp_only);
        }

        {
            ...
            const double end_allocate = now_seconds();
            best.checksum ^= build_reference(static_cast<size_t>(period), nucleus, z, s, nullptr);
            const double end_gmp_only = now_seconds();
            best.checksum ^= build_reference(static_cast<size_t>(period), nucleus, z, s, &one_period);
            const double end_build_store = now_seconds();
            const uint64_t checksum = replay_period(one_period, mxit);
            const double end_replay = now_seconds();
            best.period_shadow_allocate = std::min(best.period_shadow_allocate, end_allocate - start_allocate);
            best.period_gmp_only = std::min(best.period_gmp_only, end_gmp_only - end_allocate);
            best.period_build_store = std::min(best.period_build_store, end_build_store - end_gmp_only);
...
static float smooth_escape(double zrad, long n) {
    return (float)((double)n + 1.0 - std::log(std::log(zrad) / 2.0 / std::log(2.0)) / std::log(2.0));
}
...
static float perturb_escape(const ShadowStorage& ss, bool periodic, size_t period, double dcr, double dci, size_t mxit, double R2) {
...
static float oracle_escape(const Cmpf& c_nucleus, double dcr, double dci, size_t mxit, double R2, Scratch& s) {
    Cmpf c, z;
    mpf_set_d(s.a.v, dcr);
    mpf_add(c.re, c_nucleus.re, s.a.v);
    mpf_set_d(s.a.v, dci);
    mpf_add(c.im, c_nucleus.im, s.a.v);
    complex_zero(z);
...
static int run_verify(const Cmpf& center, const Cmpf& nucleus, uint64_t period, size_t mxit, int W, int H, double zoom_double, int oracle_step) {
    ...
    long fp_class = 0, fp_ext = 0, fp_big = 0;
    double fp_sum = 0, fp_max = 0;
    long fo_class = 0, fo_ext = 0;
    double fo_sum = 0, fo_max = 0;
    long po_class = 0, po_ext = 0;
    double po_sum = 0, po_max = 0;
    ...
    for (int i = 0; i < H; ++i) {
        for (int j = 0; j < W; ++j) {
            ...
            if ((vf < 0) != (vp < 0))
                ...
                double d = std::fabs(vf - vp);
                fp_sum += d;
                ++fp_ext;
                if (d > fp_max) fp_max = d;
                if (d > 0.01) ++fp_big;
            }
            if (oracle_step > 0 && (i % oracle_step == 0) && (j % oracle_step == 0)) {
                ...
                if ((vf < 0) != (vo < 0)) {
                    ++fo_class;
                    if (fo_class <= 10) std::printf("    [oracle class-mismatch] pixel(%d,%d) full=%.4f periodic=%.4f oracle=%.4f\n", i, j, vf, vp, vo);
                } else if (vo >= 0) {
                    double d = std::fabs(vf - vo);
                    fo_sum += d;
                    ++fo_ext;
                    if (d > fo_max) fo_max = d;
                }
                if ((vp < 0) != (vo < 0))
                    ++po_class;
                else if (vo >= 0) {
                    double d = std::fabs(vp - vo);
                    po_sum += d;
                    ++po_ext;
                    if (d > po_max) po_max = d;
                }
            }
        }
    }
    ...
    const bool periodic_no_worse = (po_class == fo_class && po_ext == fo_ext && po_max <= fo_max + 1e-6);
    ...
    std::printf("  => %s\n", pass ? "PASS (periodic reference reproduces the full reference EXACTLY and is no worse vs oracle)" : "CHECK (see mismatches above)");
    return pass ? 0 : 3;
}
...
static mp_bitcnt_t default_precision(uint64_t scale_exp) {
    const long double bits = 2.0L * static_cast<long double>(scale_exp) * std::log2(10.0L) + 256.0L;
    const long double limit = static_cast<long double>(std::numeric_limits<mp_bitcnt_t>::max());
...
int main(int argc, char** argv) {
    if (argc < 5 || (std::strcmp(argv[1], "find") != 0 && std::strcmp(argv[1], "bench") != 0 && std::strcmp(argv[1], "verify") != 0)) {
        print_usage(argv[0]);
        return 1;
    }
    ...
    if (bench_mode) {
        uint64_t mxit_u64 = 0;
        if (!parse_u64(argv[5], mxit_u64) || mxit_u64 == 0 || mxit_u64 > static_cast<uint64_t>(std::numeric_limits<size_t>::max())) {
        ...
        uint64_t v = 0;
        if (!parse_u64(argv[5], v) || v == 0 || v > static_cast<uint64_t>(std::numeric_limits<size_t>::max())) {
            std::fprintf(stderr, "invalid mxit: %s\n", argv[5]);
            return 1;
        }
        ...
        if (!(verify_zoom > 0.0)) {
            std::fprintf(stderr, "invalid zoom: %s\n", argv[8]);
            return 1;
        }
    ...
    if (max_period == 0 || repetitions_u64 == 0 || repetitions_u64 > 100 || precision_u64 < 128 || precision_u64 > static_cast<uint64_t>(std::numeric_limits<mp_bitcnt_t>::max())) {
        std::fprintf(stderr, "maxPeriod must be positive, reps must be 1..100, and precisionBits >= 128\n");
        return 1;
    }
    ...
    try {
        Cmpf center, nucleus;
        if (mpf_set_str(center.re, argv[2], 10) != 0 || mpf_set_str(center.im, argv[3], 10) != 0) {
        ...
        std::printf("mode=%s  zoom=1e%llu  precision=%llu bits  maxPeriod=%llu\n", bench_mode ? "bench" : "find", static_cast<unsigned long long>(scale_exp), static_cast<unsigned long long>(g_precision_bits), static_cast<unsigned long long>(max_period));
        ...
        if (search.stop != SearchStop::Found) {
            if (search.stop == SearchStop::EscapedDisk) {
                std::printf("period search: no nucleus detected; the complete propagated "
                            "view disk escaped at iteration %llu (%.6f s)\n",
                            static_cast<unsigned long long>(search.iterations), search_seconds);
            } else {
                std::printf("period search: no nucleus detected through maxPeriod=%llu "
                            "(%.6f s); a higher-period nucleus is not ruled out\n",
                            static_cast<unsigned long long>(max_period), search_seconds);
            }
            if (bench_mode) std::printf("benchmark skipped: periodic storage requires a nucleus in view\n");
            return bench_mode ? 2 : 0;
        }

        std::printf("period search: period=%llu  iterations=%llu  time=%.6f s\n", static_cast<unsigned long long>(search.period), static_cast<unsigned long long>(search.iterations), search_seconds);

        Mpf residual;
        const double newton_start = now_seconds();
        const NewtonResult newton = refine_nucleus(nucleus, search.period, 200, residual.v);
        const double newton_seconds = now_seconds() - newton_start;
        std::printf("Newton: iterations=%llu  converged=%s  target=%llu bits  time=%.6f s\n", static_cast<unsigned long long>(newton.iterations), newton.converged ? "yes" : "no", static_cast<unsigned long long>(newton.convergence_bits), newton_seconds);
        ...
        if (verify_mode) return run_verify(center, nucleus, search.period, mxit, verify_w, verify_h, verify_zoom, oracle_step);

        const bool deep_shadows = scale_exp > 280;
        const long double shallow_sample_bytes = sizeof(std::complex<double>);
        const long double deep_sample_bytes = sizeof(std::complex<double>) + sizeof(FeShadow);
        const long double full_shallow_bytes = shallow_sample_bytes * static_cast<long double>(mxit);
        const long double period_shallow_bytes = shallow_sample_bytes * static_cast<long double>(search.period);
        const long double full_deep_bytes = deep_sample_bytes * static_cast<long double>(mxit);
        const long double period_deep_bytes = deep_sample_bytes * static_cast<long double>(search.period);
        const long double mib = 1024.0L * 1024.0L;
        std::printf("\nreference-orbit microbenchmark: mxit=%llu  period=%llu  reps=%llu\n", static_cast<unsigned long long>(mxit), static_cast<unsigned long long>(search.period), static_cast<unsigned long long>(repetitions_u64));
        std::printf("constant GMP orbit state: two rotating complex buffers (4 mpf_t), "
                    "estimated %.3Lf KiB\n"
                    ...
                    estimated_gmp_orbit_bytes() / 1024.0L, sizeof(std::complex<double>), full_shallow_bytes / mib, period_shallow_bytes / mib, sizeof(std::complex<double>) + sizeof(FeShadow), full_deep_bytes / mib, period_deep_bytes / mib, deep_shadows ? "deep" : "shallow", static_cast<long double>(mxit) / static_cast<long double>(search.period));

        const BenchmarkTimes timing = benchmark_references(center, nucleus, search.period, mxit, static_cast<int>(repetitions_u64), deep_shadows);
        std::printf("best timings:\n"
                    ...
                    timing.full_shadow_allocate, timing.full_gmp_only, timing.full_build_store, timing.period_shadow_allocate, timing.period_gmp_only, timing.period_build_store, timing.replay, timing.period_gmp_only == 0.0 ? 0.0 : timing.full_gmp_only / timing.period_gmp_only, timing.period_build_store == 0.0 ? 0.0 : timing.full_build_store / timing.period_build_store);
