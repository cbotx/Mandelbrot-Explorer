...
static void initPalette() {
    ...
    for (int c = 0; c < 3; ++c) {
        ...
        pal2[c][0] = 0;
        for (int i = 0; i < P; ++i)
            pal2[c][i + 1] = pal2[c][i] + 0.5 * (pal1[c][i] + pal1[c][i + 1]);
        palMean[c] = pal1[c][P] / P;
    }
}
static double phase(float sac) { return sac * (density / 20.0) * P; }
static int palIndex(double u) {
    int i = (int)std::floor(u) % P;
    return i < 0 ? i + P : i;
}
static void pointColor(float v, double rgb[3]) {
    if (v < 0) { rgb[0] = rgb[1] = rgb[2] = 0; return; }
...
static double box2d(int c, double center, double dx, double dy) {
    ...
    return std::clamp((prefix2(c, center + ax + ay)
                     - prefix2(c, center - ax + ay)
                     - prefix2(c, center + ax - ay)
                     + prefix2(c, center - ax - ay)) / (wx * wy), 0.0, 1.0);
}
static bool ext(float v) { return v != EMPTYPIXEL && v >= 0; }
static void gradients(float v, float l, float r, float u, float d, double& dx, double& dy) {
    double f = phase(v);
    if (ext(l) && ext(r)) dx = 0.5 * (phase(r) - phase(l));
    else if (ext(r)) dx = phase(r) - f;
    else if (ext(l)) dx = f - phase(l);
    else dx = 0;
    if (ext(u) && ext(d)) dy = 0.5 * (phase(d) - phase(u));
    else if (ext(d)) dy = phase(d) - f;
    else if (ext(u)) dy = f - phase(u);
    else dy = 0;
}
static void old1dColor(float v, float l, float r, float u, float d, double rgb[3]) {
    if (v < 0) { rgb[0] = rgb[1] = rgb[2] = 0; return; }
    double dx, dy; gradients(v, l, r, u, d, dx, dy);
    double width = std::sqrt(dx * dx + dy * dy), center = phase(v);
    for (int c = 0; c < 3; ++c) {
        double lin;
        if (width < 1) lin = srgbToLinear(palette[c][palIndex(center)]);
        else if (width >= P) lin = palMean[c];
        else lin = (prefix1(c, center + width * 0.5) - prefix1(c, center - width * 0.5)) / width;
...
static void quadColor(float v, float l, float r, float u, float d,
                      float ul, float ur, float dl, float dr, int sub, double rgb[3]) {
    if (v < 0) { rgb[0] = rgb[1] = rgb[2] = 0; return; }
    double f = phase(v), gx, gy; gradients(v, l, r, u, d, gx, gy);
    double hxx = ext(l) && ext(r) ? phase(r) - 2.0 * f + phase(l) : 0.0;
    double hyy = ext(u) && ext(d) ? phase(d) - 2.0 * f + phase(u) : 0.0;
    double hxy = ext(ul) && ext(ur) && ext(dl) && ext(dr)
        ? 0.25 * (phase(dr) - phase(dl) - phase(ur) + phase(ul)) : 0.0;
    double lin[3] = {};
    for (int iy = 0; iy < sub; ++iy) for (int ix = 0; ix < sub; ++ix) {
        double x = (ix + 0.5) / sub - 0.5, y = (iy + 0.5) / sub - 0.5;
        double center = f + gx * x + gy * y
            + 0.5 * hxx * x * x + hxy * x * y + 0.5 * hyy * y * y;
...
static void blockColor(const float* it, int W, int sub, int y, int x, double rgb[3]) {
    ...
    if (it[center - (stride + 1) * cc] == EMPTYPIXEL) {
        pointColor(it[center], rgb); return;
    }
    double sum[3] = {};
    for (int a = -cc; a <= cc; ++a)
        for (int b = -cc; b <= cc; ++b) {
            double p[3]; pointColor(it[center + a * stride + b], p);
...
static void uniform3FromGt(const float* it, int W, int gtSub, int y, int x, double rgb[3]) {
    ...
    for (int a = -1; a <= 1; ++a) for (int b = -1; b <= 1; ++b) {
        double p[3]; pointColor(it[center + a * step * stride + b * step], p);
        for (int c = 0; c < 3; ++c) sum[c] += srgbToLinear(p[c]);
    }
...
static void gauss2FromGt(const float* it, int W, int gtSub, int y, int x, double rgb[3]) {
    ...
    int center = (y * gtSub + cc) * stride + x * gtSub + cc;
    double sum[3] = {};
    for (int a : {-step, step}) for (int b : {-step, step}) {
        double p[3]; pointColor(it[center + a * stride + b], p);
        for (int c = 0; c < 3; ++c) sum[c] += srgbToLinear(p[c]);
    }
...
static void gauss3FromGt(const float* it, int W, int gtSub, int y, int x, double rgb[3]) {
    ...
    for (int a = 0; a < 3; ++a) for (int b = 0; b < 3; ++b) {
        double p[3]; pointColor(it[center + off[a] * stride + off[b]], p);
        for (int c = 0; c < 3; ++c) sum[c] += wt[a] * wt[b] * srgbToLinear(p[c]);
    }
    for (int c = 0; c < 3; ++c) rgb[c] = linearToSrgb(sum[c]);
}
static void corner3FromGt(const float* it, int W, int gtSub, int y, int x,
                          double rgb[3], double& phaseSpan) {
    int stride = W * gtSub, cc = gtSub / 2, step = gtSub / 3;
    int center = (y * gtSub + cc) * stride + x * gtSub + cc;
    double sum[3] = {}, lo = phase(it[center]), hi = lo;
    for (int a : {-step, step}) for (int b : {-step, step}) {
        float v = it[center + a * stride + b];
        double p[3]; pointColor(v, p);
        for (int c = 0; c < 3; ++c) sum[c] += srgbToLinear(p[c]);
        if (v >= 0) { lo = std::min(lo, phase(v)); hi = std::max(hi, phase(v)); }
    }
    for (int c = 0; c < 3; ++c) rgb[c] = linearToSrgb(sum[c] * 0.25);
...
static void highres3Color(const float* it, int W, int y, int x, double rgb[3]) {
    ...
    double sum[3] = {};
    for (int a = 0; a < 3; ++a) for (int b = 0; b < 3; ++b) {
        double p[3]; pointColor(it[(y * 3 + a) * stride + x * 3 + b], p);
        for (int c = 0; c < 3; ++c) sum[c] += srgbToLinear(p[c]);
    }
    for (int c = 0; c < 3; ++c) rgb[c] = linearToSrgb(sum[c] / 9.0);
}
static void highres2Color(const float* it, int W, int y, int x, double rgb[3]) {
    ...
    for (int a = 0; a < 2; ++a) for (int b = 0; b < 2; ++b) {
        double p[3]; pointColor(it[(y * 2 + a) * stride + x * 2 + b], p);
        for (int c = 0; c < 3; ++c) sum[c] += srgbToLinear(p[c]);
    }
    for (int c = 0; c < 3; ++c) rgb[c] = linearToSrgb(sum[c] * 0.25);
}
static void writeBmp(const char* path, const std::vector<uint8_t>& rgb, int W, int H) {
    ...
    memcpy(ih + 12, &planes, 2); memcpy(ih + 14, &bpp, 2); memcpy(ih + 20, &ds, 4);
...
struct Error {
    ...
    void add(const double a[3], const double b[3]) {
        ...
        for (int c = 0; c < 3; ++c) {
            ...
            sum += d; sum2 += d * d; max = std::max(max, d); e += d;
        }
...
int main(int argc, char** argv) {
    ...
    const char* cx = argv[1]; const char* cy = argv[2]; const char* scale = argv[3];
    int mxit = atoi(argv[4]); density = atof(argv[5]);
    int W = argc > 6 ? atoi(argv[6]) : 240, H = argc > 7 ? atoi(argv[7]) : 160;
    int gtSub = argc > 8 ? atoi(argv[8]) : 9; if (!(gtSub & 1)) ++gtSub;
    int precision = (int)((std::max(0.0, std::log10(atof(scale))) + 30) * std::log2(10.0)) + 64;
    mpf_set_default_prec(precision);
    initPalette();

    mpf_t mcx, mcy, msc;
    mpf_init_set_str(mcx, cx, 10); mpf_init_set_str(mcy, cy, 10); mpf_init_set_str(msc, scale, 10);
    int method = ColoringMethod::STRIPE_AVERAGE | ColoringMethod::SUPER_SAMPLING;

    size_t ng = (size_t)W * H * gtSub * gtSub;
    float* gt = new float[ng]; std::fill(gt, gt + ng, (float)EMPTYPIXEL);
    _putenv_s("MANDEL_SAC_SS_STEPS", "0.000001");
    { Mandel m(W, H, mxit, gtSub, gt); m.setPrecision(precision); m.setDensity((float)density);
      m.Compute(mcx, mcy, msc, mxit, method); }

    size_t na = (size_t)W * H * 25;
    float* ad = new float[na]; std::fill(ad, ad + na, (float)EMPTYPIXEL);
    _putenv_s("MANDEL_SAC_SS_STEPS", "0");
    { Mandel m(W, H, mxit, 5, ad); m.setPrecision(precision); m.setDensity((float)density);
      m.Compute(mcx, mcy, msc, mxit, method); }

    size_t nh = (size_t)W * H * 9;
    float* hi = new float[nh]; std::fill(hi, hi + nh, (float)EMPTYPIXEL);
    { Mandel m(W * 3, H * 3, mxit, 1, hi); m.setPrecision(precision);
      m.Compute(mcx, mcy, msc, mxit, ColoringMethod::STRIPE_AVERAGE); }
    size_t nh2 = (size_t)W * H * 4;
    float* hi2 = new float[nh2]; std::fill(hi2, hi2 + nh2, (float)EMPTYPIXEL);
    { Mandel m(W * 2, H * 2, mxit, 1, hi2); m.setPrecision(precision);
      m.Compute(mcx, mcy, msc, mxit, ColoringMethod::STRIPE_AVERAGE); }

    std::vector<uint8_t> imgGt((size_t)W * H * 3), imgPoint(imgGt.size()),
        imgOld(imgGt.size()), img2d(imgGt.size()), imgQ2(imgGt.size()),
        imgQ4(imgGt.size()), imgQ8(imgGt.size()), imgG2(imgGt.size()),
        imgU3(imgGt.size()), imgG3(imgGt.size()), imgHi2(imgGt.size()),
        imgHi3(imgGt.size()), imgAd(imgGt.size());
    Error ep, eo, e2, eq2, eq4, eq8, eg2, eu3, eg3, ea;
    Error ehi2, ehi3;
    static const double progT[] = { 0.05, 0.1, 0.25, 0.5, 1.0, 2.0 };
    Error prog[6]; long refined[6] = {};
    long adCount = 0;
    int gs = W * gtSub, gc = gtSub / 2, as = W * 5, ac = 2;
    for (int y = 0; y < H; ++y) for (int x = 0; x < W; ++x) {
        int gi = (y * gtSub + gc) * gs + x * gtSub + gc;
        float v = gt[gi], l = x ? gt[gi - gtSub] : EMPTYPIXEL,
              r = x + 1 < W ? gt[gi + gtSub] : EMPTYPIXEL,
              u = y ? gt[gi - gs * gtSub] : EMPTYPIXEL,
              d = y + 1 < H ? gt[gi + gs * gtSub] : EMPTYPIXEL;
        float ul = x && y ? gt[gi - gs * gtSub - gtSub] : EMPTYPIXEL,
              ur = x + 1 < W && y ? gt[gi - gs * gtSub + gtSub] : EMPTYPIXEL,
              dl = x && y + 1 < H ? gt[gi + gs * gtSub - gtSub] : EMPTYPIXEL,
              dr = x + 1 < W && y + 1 < H ? gt[gi + gs * gtSub + gtSub] : EMPTYPIXEL;
        double cg[3], cp[3], co[3], c2[3], cq2[3], cq4[3], cq8[3],
            cg2[3], cu3[3], cg3[3], chi2[3], chi3[3], cc3[3], ca[3], span;
        ...
        pointColor(v, cp); old1dColor(v, l, r, u, d, co); box2dColor(v, l, r, u, d, c2);
        ...
        if (flagged(ad, W, 5, y, x)) { blockColor(ad, W, 5, y, x, ca); ++adCount; }
        else {
            float av = ad[ai], al = x ? ad[ai - 5] : EMPTYPIXEL,
                  ar = x + 1 < W ? ad[ai + 5] : EMPTYPIXEL,
                  au = y ? ad[ai - as * 5] : EMPTYPIXEL,
                  avd = y + 1 < H ? ad[ai + as * 5] : EMPTYPIXEL;
            box2dColor(av, al, ar, au, avd, ca);
        }
        ep.add(cp, cg); eo.add(co, cg); e2.add(c2, cg);
        eq2.add(cq2, cg); eq4.add(cq4, cg); eq8.add(cq8, cg);
        eg2.add(cg2, cg); eu3.add(cu3, cg); eg3.add(cg3, cg);
        ehi2.add(chi2, cg); ehi3.add(chi3, cg); ea.add(ca, cg);
        for (int k = 0; k < 6; ++k) {
        ...
        double* colors[] = {
            cg, cp, co, c2, cq2, cq4, cq8, cg2, cu3, cg3, chi2, chi3, ca
        };
        std::vector<uint8_t>* imgs[] = {
            &imgGt, &imgPoint, &imgOld, &img2d, &imgQ2, &imgQ4, &imgQ8,
            &imgG2, &imgU3, &imgG3, &imgHi2, &imgHi3, &imgAd
        };
        for (int k = 0; k < 13; ++k) for (int c = 0; c < 3; ++c)
            (*imgs[k])[((size_t)y * W + x) * 3 + c] = (uint8_t)std::clamp(colors[k][c] + 0.5, 0.0, 255.0);
    }
    printf("W=%d H=%d GT=%dx density=%.1f  adaptive5 flagged=%.2f%%\n",
           W, H, gtSub, density, 100.0 * adCount / (W * H));
    ep.print("point"); eo.print("old1d"); e2.print("box2d");
    eq2.print("quad2"); eq4.print("quad4"); eq8.print("quad8");
    eg2.print("gauss2"); eu3.print("uniform3"); eg3.print("gauss3");
    ehi2.print("highres2"); ehi3.print("highres3"); ea.print("adaptive5");
    puts("progressive centre+4 corners -> full 3x:");
    for (int k = 0; k < 6; ++k) {
        char name[32]; snprintf(name, sizeof(name), "phase>%.2f", progT[k]);
        prog[k].print(name);
    ...
    auto timeFilter = [&](int mode) {
        ...
        for (int rep = 0; rep < 5; ++rep)
            for (int y = 1; y + 1 < H; ++y) for (int x = 1; x + 1 < W; ++x) {
                int gi = (y * gtSub + gc) * gs + x * gtSub + gc;
                float v = gt[gi], l = gt[gi - gtSub], r = gt[gi + gtSub],
                      u = gt[gi - gs * gtSub], d = gt[gi + gs * gtSub],
                      ul = gt[gi - gs * gtSub - gtSub], ur = gt[gi - gs * gtSub + gtSub],
                      dl = gt[gi + gs * gtSub - gtSub], dr = gt[gi + gs * gtSub + gtSub];
                double out[3];
                if (mode == 0) old1dColor(v, l, r, u, d, out);
                else if (mode == 1) box2dColor(v, l, r, u, d, out);
                else quadColor(v, l, r, u, d, ul, ur, dl, dr, 4, out);
                sink = sink + out[0];
            }
        double ms = std::chrono::duration<double, std::milli>(
            std::chrono::steady_clock::now() - start).count() / 5.0;
        return ms;
    };
    printf("color-only time/frame: old1d=%.2fms box2d=%.2fms quad4=%.2fms (single thread)\n",
           timeFilter(0), timeFilter(1), timeFilter(2));
    writeBmp("feather_gt.bmp", imgGt, W, H); writeBmp("feather_point.bmp", imgPoint, W, H);
    writeBmp("feather_old1d.bmp", imgOld, W, H); writeBmp("feather_box2d.bmp", img2d, W, H);
    writeBmp("feather_quad2.bmp", imgQ2, W, H); writeBmp("feather_quad4.bmp", imgQ4, W, H);
    ...
    delete[] gt; delete[] ad; delete[] hi; delete[] hi2;
